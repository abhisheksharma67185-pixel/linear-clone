...
export const shopifyAdminPlugin: SitePlugin = {
  ...
  setState(_state: Record<string, unknown>) { // eslint-disable-line @typescript-eslint/no-unused-vars
    // Not implemented — use mutations instead
  },

  reset(_seed?: number) { // eslint-disable-line @typescript-eslint/no-unused-vars
    store.reset();
  },

  getMutations(): MutationDefinition[] {
    return [
      { name: "createProduct", description: "Create a new product", params: { title: "string", price: "string", status: "string" } },
      { name: "updateProduct", description: "Update an existing product", params: { id: "string", fields: "object" } },
      ...
      { name: "capturePayment", description: "Capture a pending payment", params: { id: "string" } },
      { name: "refundOrder", description: "Refund a paid order", params: { id: "string" } },
      { name: "addOrderNote", description: "Add a note to an order", params: { id: "string", message: "string" } },
      { name: "createCustomer", description: "Create a new customer", params: { firstName: "string", lastName: "string", email: "string" } },
      { name: "updateCustomer", description: "Update a customer", params: { id: "string", fields: "object" } },
      { name: "createDiscount", description: "Create a discount", params: { title: "string", type: "string", value: "string" } },
      { name: "updateDiscount", description: "Update a discount", params: { id: "string", fields: "object" } },
      { name: "deleteDiscount", description: "Delete a discount", params: { id: "string" } },
      { name: "updateSettings", description: "Update store settings", params: { fields: "object" } },
    ];
  },
  ...
  executeMutation(name: string, args: unknown): MutationResult {
    ...
    switch (name) {
      ...
      case "updateProduct":
        return store.updateProduct(a.id as string, a.fields as Parameters<typeof store.updateProduct>[1]);
      case "deleteProduct":
      ...
      case "updateCustomer":
        return store.updateCustomer(a.id as string, a.fields as Parameters<typeof store.updateCustomer>[1]);
      case "createDiscount":
        return store.createDiscount(a as Parameters<typeof store.createDiscount>[0]);
      case "updateDiscount":
        return store.updateDiscount(a.id as string, a.fields as Parameters<typeof store.updateDiscount>[1]);
      case "deleteDiscount":
  ...
  getConfigSchema(): ConfigSchema {
    return {
      ...
      site: {
        outOfStockProducts: { type: "string[]", default: [], description: "Product IDs with zero inventory" },
        paymentFailureRate: { type: "number", default: 0, description: "Payment capture failure rate" },
        discountExpired: { type: "boolean", default: false, description: "Force all discounts expired" },
        highTrafficMode: { type: "boolean", default: false, description: "Show high traffic warnings" },
      },
    };
  },

  applyConfig(_config: Record<string, unknown>) { // eslint-disable-line @typescript-eslint/no-unused-vars
