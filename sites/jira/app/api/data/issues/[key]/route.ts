...
export async function GET(request: NextRequest, { params }: { params: Promise<{ key: string }> }) {
  ...
  if (populate) {
    ...
    if (fields.has("users")) {
      ...
      populated.assignee = raw.assigneeId ? userMap.get(raw.assigneeId as string) ?? null : null;
    ...
    if (fields.has("sprints")) {
      ...
      populated.sprint = raw.sprintId ? sprintMap.get(raw.sprintId as string) ?? null : null;
    }
    if (fields.has("epics")) {
      const epicMap = new Map(store.getEpics().map((e) => [e.id, e]));
      populated.epic = raw.epicId ? epicMap.get(raw.epicId as string) ?? null : null;
    }
