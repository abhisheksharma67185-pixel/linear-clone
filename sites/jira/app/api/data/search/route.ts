...
function parseSearchQuery(raw: string): { text: string; filters: Record<string, string> } {
  const filters: Record<string, string> = {}
  // Extract qualifier:"value" or qualifier:'value' patterns
  const qualifierRe = /\b(status|assignee|priority|type|project|sprint|epic):\s*"([^"]+)"/gi
  const text = raw.replace(qualifierRe, (_, key, value) => {
...
export async function GET(request: NextRequest) {
  ...
  const rawIssues = store.getIssues().filter((i) => {
    ...
    if (filters.status) {
      const issueStatus = (issue.status as string).toLowerCase().replace(/_/g, " ")
      if (issueStatus !== filters.status.toLowerCase()) return false
    }

    // Filter by assignee qualifier (match by display name)
    if (filters.assignee) {
      const assignee = issue.assigneeId ? userMap.get(issue.assigneeId as string) : null
      if (!assignee) return false
      const assigneeName = ((assignee as Record<string, unknown>).displayName ?? (assignee as Record<string, unknown>).name) as string
      if (assigneeName.toLowerCase() !== filters.assignee.toLowerCase()) return false
    }

    // Filter by priority qualifier
    if (filters.priority) {
      if ((issue.priority as string).toLowerCase() !== filters.priority.toLowerCase()) return false
    }

    // Filter by type qualifier
    if (filters.type) {
      if ((issue.type as string).toLowerCase() !== filters.type.toLowerCase()) return false
    }
    ...
    if (filters.project) {
      ...
      const matchesProject =
        (proj.key as string).toLowerCase() === filters.project.toLowerCase() ||
        (proj.name as string).toLowerCase().includes(filters.project.toLowerCase())
      if (!matchesProject) return false
    }

    // Filter by sprint qualifier
    if (filters.sprint) {
      const sprint = issue.sprintId ? sprintMap.get(issue.sprintId as string) : null
      if (!sprint) return false
      if (!(sprint as unknown as Record<string, unknown>).name?.toString().toLowerCase().includes(filters.sprint.toLowerCase())) return false
    }
    ...
    if (filters.epic) {
      ...
      if (!(epic as unknown as Record<string, unknown>).name?.toString().toLowerCase().includes(filters.epic.toLowerCase())) return false
    }
  ...
  const projects = lowerText ? allProjects.filter((p) => {
  ...
  const users = lowerText ? allUsers.filter((u) => {
