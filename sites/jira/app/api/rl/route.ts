...
function getContextData(): Record<string, unknown> {
  const match = _rlState.currentPage.match(/^\/(issues|projects|sprints|epics|boards)\/(.+)$/);
  if (match) {
    const [, entity, id] = match;
    switch (entity) {
      case "issues":
        return { issue: store.getIssueById(id) ?? store.getIssueByKey(id) ?? null };
      case "projects":
        return { project: store.getProjectById(id) ?? store.getProjectByKey(id) ?? null };
      case "sprints":
...
function getAvailableActions(): string[] {
  ...
  if (_rlState.currentPage === "/sprints" || _rlState.currentPage.match(/^\/sprints\/.+$/)) {
    return [...base, "create_sprint", "start_sprint", "complete_sprint", "move_issue_to_sprint"];
  }
  if (_rlState.currentPage === "/epics" || _rlState.currentPage.match(/^\/epics\/.+$/)) {
...
function getObservation() {
  ...
  return {
    ...
    summary: {
      ...
      todoIssues: issues.filter((i: Record<string, unknown>) => i.status === "to_do").length,
...
function executeAction(action: Record<string, unknown>): {
  ...
  switch (action.action) {
    ...
    case "create_sprint": {
      const fields = action.fields as Record<string, unknown> | undefined;
      if (fields) {
        const result = store.createSprint(fields as Parameters<typeof store.createSprint>[0]);
        reward = result.success ? 0.5 : -0.5;
        success = result.success;
      } else {
        reward = -0.5;
    ...
    case "create_epic": {
      const fields = action.fields as Record<string, unknown> | undefined;
      if (fields) {
        const result = store.createEpic(fields as Parameters<typeof store.createEpic>[0]);
        reward = result.success ? 0.5 : -0.5;
        success = result.success;
      } else {
        reward = -0.5;
    ...
    case "create_filter": {
      const fields = action.fields as Record<string, unknown> | undefined;
      if (fields) {
        const result = store.createFilter(fields as Parameters<typeof store.createFilter>[0]);
        reward = result.success ? 0.3 : -0.5;
        success = result.success;
      } else {
        reward = -0.5;
