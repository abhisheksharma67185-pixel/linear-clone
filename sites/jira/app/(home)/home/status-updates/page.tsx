...
const projectUpdates = [
  {
    ...
    comments: [
      { author: "Omar Darboe", initials: "OD", color: "bg-green-200", time: "edited 2 days ago", text: "So exciting team! Lovely work" },
      { author: "Jane Rotanson", initials: "JR", color: "bg-blue-200", time: "3 days ago", text: "Huge accomplishment! Excited to test this in person." },
    ],
  },
  {
    ...
    learnings: "We found that physical training fostered a sense of teamwork & cooperation among the crew.",
    comments: [],
  },
  {
...
const statusStats = [
  { count: 1, label: "On track", change: "-2 from last week", color: "text-green-600" },
  { count: 1, label: "At risk", change: "+1 from last week", color: "text-yellow-600" },
  { count: 1, label: "Off track", change: "No change", color: "text-red-600" },
  { count: 0, label: "No update", change: "No change", color: "text-muted-foreground" },
  { count: 0, label: "Cancelled", change: "No change", color: "text-muted-foreground" },
  { count: 1, label: "Completed 🎉", change: "+1 from last week", color: "text-muted-foreground" },
]

const goalStats = [
  { count: 1, label: "On track", change: "+1 from last month", color: "text-green-600" },
  { count: 0, label: "At risk", change: "No change", color: "text-yellow-600" },
  { count: 1, label: "Off track", change: "No change", color: "text-red-600" },
  { count: 0, label: "No update", change: "-1 from last month", color: "text-muted-foreground" },
  { count: 0, label: "Cancelled", change: "No change", color: "text-muted-foreground" },
  { count: 0, label: "Completed 🎉", change: "No change", color: "text-muted-foreground" },
]
      ...
      <div className="px-5 py-4">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Avatar className="size-7"><AvatarFallback className={`${update.color} text-[10px] font-semibold`}>{update.initials}</AvatarFallback></Avatar>
            <div>
              <span className="text-sm font-medium">{update.author}</span>
              <span className="ml-2 text-xs text-muted-foreground">{update.time}</span>
            </div>
          </div>
          {update.status && update.period && (
            <div className="flex items-center gap-1.5 text-xs">
              <span className={`rounded px-1.5 py-0.5 font-bold ${update.statusColor}`}>{update.status}</span>
        ...
        {update.learnings && (
          <div className="mt-4">
            ...
            <div className="flex items-start gap-2 rounded-lg border bg-muted/30 px-4 py-3">
              <span>💡</span>
              <p className="text-sm text-muted-foreground">{update.learnings}</p>
            </div>
          </div>
        )}
          ...
          <div className="mt-4 space-y-3 border-t pt-4">
            {update.comments.map((comment, i) => (
              <div key={i} className="flex items-start gap-2">
                <Avatar className="size-6"><AvatarFallback className={`${comment.color} text-[9px] font-semibold`}>{comment.initials}</AvatarFallback></Avatar>
                <div>
                  <p className="text-xs">
        ...
        <div className="mt-3 flex items-center gap-2 border-t pt-3">
          ...
          <p className="text-sm text-muted-foreground">Add a comment... ask a question</p>
        </div>
      </div>
    </div>
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-background">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        {Array.from({ length: 80 }).map((_, i) => {
          const colors = ["bg-blue-400", "bg-pink-300", "bg-yellow-400", "bg-green-400", "bg-purple-400", "bg-cyan-300", "bg-orange-300", "bg-red-300"]
      ...
      <button
        onClick={onClose}
        className="absolute right-6 top-6 z-10 rounded p-1.5 text-muted-foreground hover:bg-accent"
      >
      ...
      <div className="relative z-10 w-full max-w-2xl rounded-xl border bg-background px-8 py-12 text-center shadow-lg">
        ...
        <div className="mx-auto mb-6 flex size-32 items-center justify-center rounded-full bg-gradient-to-b from-emerald-300 to-emerald-500">
          <svg className="size-16 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
        ...
        <p className="text-sm text-muted-foreground">
          Thanks for <span className="text-blue-600 underline">showing teams that you&apos;re paying attention</span> by engaging with status updates. See you next week!
        </p>
      </div>
    </div>
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background">
      ...
      <div className="absolute right-6 top-6 flex items-center gap-2">
        <button className="rounded p-1.5 text-muted-foreground hover:bg-accent">
          <svg className="size-5" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="1.5" /><circle cx="12" cy="12" r="1.5" /><circle cx="12" cy="19" r="1.5" /></svg>
        </button>
        <button
          onClick={onClose}
      ...
      <div className="w-full max-w-2xl rounded-xl border bg-background px-8 py-12 text-center shadow-lg">
        ...
        <h2 className="mb-4 text-xl font-bold">You don&apos;t have any updates due in the next 7 days</h2>

        <p className="mb-4 text-sm text-muted-foreground">
          Expecting to see updates? Try changing{" "}
          <Button variant="outline" size="sm" className="mx-1">Update Filters</Button>
        </p>
  ...
  return (
    ...
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => setCatchUpOpen(true)}>Catch up</Button>
          <Button variant="outline" size="sm" onClick={() => setWriteOpen(true)}>Write updates</Button>
        </div>
      </div>
        ...
        <div className="flex-1">
          {tab === "projects" ? (
            ...
            <>
              ...
              <div className="mb-6 rounded-lg border bg-muted/30 px-5 py-4">
                <div className="flex items-start gap-3">
                  <svg className="mt-0.5 size-5 text-green-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><path d="M9 12l2 2 4-4" /></svg>
                  <div>
              ...
              <p className="mb-4 text-sm text-muted-foreground">You&apos;re following 2 active goals, here&apos;s the breakdown.</p>
              ...
              <div className="rounded-lg border">
                <div className="border-b px-5 py-3">
                  ...
                  <p className="text-sm font-medium">Operate 42 commercial flights to space</p>
                </div>
                <div className="px-5 py-4">
                  <div className="mb-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Avatar className="size-7"><AvatarFallback className="bg-indigo-200 text-[10px] font-semibold">AH</AvatarFallback></Avatar>
                      <div>
                        <span className="text-sm font-medium">Andy Harrell</span>
                        <span className="ml-2 text-xs text-muted-foreground">4 days ago</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs">
                      <span className="rounded bg-green-100 px-1.5 py-0.5 font-bold text-green-800">ON TRACK</span>
                      <span className="text-muted-foreground">for</span>
                      <span className="flex items-center gap-1 rounded border px-1.5 py-0.5">
                        <svg className="size-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
                        Jan 2025
                  ...
                  <div className="space-y-1.5">
                    <p className="text-sm text-muted-foreground">🚀 Kicking off our goal to launch 42 commercial space flights by end of Q2 2025, generating $500 million in revenue.</p>
                    <p className="text-sm text-muted-foreground">🏨 Partnership negotiations with Planetary Hotels underways.</p>
                  </div>
                </div>
              </div>
            </>
