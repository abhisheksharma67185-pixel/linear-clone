    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="relative mb-8 overflow-hidden rounded-xl bg-gradient-to-r from-[#0052CC] via-[#2684FF] to-[#4C9AFF] px-8 py-8 text-white">
        ...
        <div className="absolute right-6 top-1/2 -translate-y-1/2 opacity-90">
          <svg className="w-52 h-32" viewBox="0 0 220 130" fill="none">
            {/* Desk */}
            <rect x="40" y="85" width="140" height="6" rx="3" fill="white" fillOpacity="0.35" />
            <rect x="60" y="91" width="6" height="30" rx="2" fill="white" fillOpacity="0.25" />
            <rect x="154" y="91" width="6" height="30" rx="2" fill="white" fillOpacity="0.25" />

            {/* Monitor */}
            <rect x="80" y="45" width="60" height="38" rx="4" fill="white" fillOpacity="0.3" />
            <rect x="84" y="49" width="52" height="28" rx="2" fill="white" fillOpacity="0.15" />
            <rect x="105" y="83" width="10" height="5" rx="1" fill="white" fillOpacity="0.25" />
            {/* Screen content lines */}
            <rect x="89" y="55" width="30" height="3" rx="1.5" fill="white" fillOpacity="0.3" />
            <rect x="89" y="62" width="42" height="3" rx="1.5" fill="white" fillOpacity="0.2" />
            <rect x="89" y="69" width="25" height="3" rx="1.5" fill="white" fillOpacity="0.25" />
            {/* Blue accent on screen */}
            <rect x="122" y="55" width="10" height="3" rx="1.5" fill="#60A5FA" fillOpacity="0.6" />

            {/* Person — sitting */}
            {/* Head */}
            <circle cx="55" cy="42" r="10" fill="white" fillOpacity="0.4" />
            {/* Body */}
            <rect x="47" y="52" width="16" height="20" rx="5" fill="white" fillOpacity="0.35" />
            {/* Arm reaching to keyboard */}
            <path d="M63 60 Q72 62 80 68" stroke="white" strokeOpacity="0.35" strokeWidth="4" strokeLinecap="round" />
            {/* Keyboard */}
            <rect x="78" y="78" width="30" height="6" rx="2" fill="white" fillOpacity="0.2" />

            {/* Coffee mug */}
            <rect x="150" y="72" width="12" height="13" rx="3" fill="white" fillOpacity="0.25" />
            <path d="M162 75 Q167 75 167 80 Q167 85 162 85" stroke="white" strokeOpacity="0.2" strokeWidth="2" fill="none" />
            {/* Steam */}
            <path d="M154 70 Q155 66 156 68 Q157 64 158 67" stroke="white" strokeOpacity="0.2" strokeWidth="1.5" strokeLinecap="round" fill="none" />

            {/* Plant */}
            <rect x="175" y="72" width="10" height="14" rx="3" fill="white" fillOpacity="0.2" />
            ...
            {/* Leaf detail */}
            <path d="M180 66 L180 74" stroke="white" strokeOpacity="0.2" strokeWidth="1.5" />

            {/* Floating decorative elements */}
            <circle cx="30" cy="25" r="3" fill="white" fillOpacity="0.15" />
            <circle cx="195" cy="30" r="4" fill="white" fillOpacity="0.12" />
            <rect x="15" y="60" width="12" height="3" rx="1.5" fill="white" fillOpacity="0.1" transform="rotate(-20 21 61.5)" />
            <rect x="200" y="55" width="10" height="3" rx="1.5" fill="white" fillOpacity="0.1" transform="rotate(15 205 56.5)" />
          </svg>
        </div>

        <Link href="/products" className="absolute bottom-3 right-4 z-10 flex items-center gap-1 text-xs opacity-80 hover:opacity-100 transition-opacity">
      ...
      <div>
        ...
        {(() => {
          const displayIssues = nextTab === "worked"
            ? recentIssues
            : [...recentIssues].reverse()

          return displayIssues.length > 0 ? (
            <div>
              <p className="text-xs text-muted-foreground mb-3">{nextTab === "worked" ? "This week" : "Recently viewed"}</p>
                ...
                {displayIssues.map((issue) => (
                  ...
                  >
                    <div className="shrink-0">
                      {issue.type === "bug" ? (
                        <svg className="size-6 text-red-500" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="10" /></svg>
                      ) : issue.type === "story" ? (
                        <svg className="size-6 text-green-600" viewBox="0 0 24 24" fill="currentColor"><path d="M5 2h14a1 1 0 011 1v18a1 1 0 01-1 1H5a1 1 0 01-1-1V3a1 1 0 011-1zm1 2v16h12V4H6z" /></svg>
                      ) : (
                        <svg className="size-6 text-blue-600" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" /></svg>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {issue.type.charAt(0).toUpperCase() + issue.type.slice(1)} &middot; {projectMap.get(issue.projectId)?.name ?? "Project"} &middot; {issue.key}
                      </p>
                    </div>

                    <div className="flex items-center gap-3 shrink-0">
                      <span className={`rounded px-2 py-0.5 text-[10px] font-bold uppercase ${STATUS_BADGE[issue.status] || STATUS_BADGE.to_do}`}>
              ...
              <Link
                href="/dashboard"
                className="mt-3 inline-block rounded-md border px-4 py-2 text-sm font-medium hover:bg-accent transition-colors"
              >
                View all
              </Link>
            </div>
