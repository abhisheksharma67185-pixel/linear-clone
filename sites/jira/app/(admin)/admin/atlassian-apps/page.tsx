...
const apps = [
  ...
  { name: "Jira Administration", slug: "jira-administration", url: "https://abhisheksharma67185.atlassian.net", plan: "", users: null, icon: "⚙" },
  { name: "Jira", slug: "jira", url: "https://abhisheksharma67185.atlassian.net", plan: "Premium", users: 1, icon: "◆" },
  { name: "Projects", slug: "projects", url: "https://abhisheksharma67185.atlassian.net", plan: "Free", users: 1, icon: "✦" },
]
...
export default function AtlassianAppsPage() {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (filterRef.current && !filterRef.current.contains(event.target as Node)) setActiveFilter(null)
      if (headerMenuRef.current && !headerMenuRef.current.contains(event.target as Node)) setHeaderMenu(false)
    ...
    <div className="p-8 max-w-5xl">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button className="bg-blue-600 text-white hover:bg-blue-700" onClick={() => setAddAppOpen(true)}>Add app</Button>
          <div className="relative" ref={headerMenuRef}>
            <button onClick={() => setHeaderMenu(!headerMenu)} className="rounded-md border p-2 text-muted-foreground hover:bg-accent">
            ...
            {headerMenu && (
              <div className="absolute right-0 top-full mt-1 w-48 rounded-lg border bg-background shadow-lg z-20">
                <button className="w-full px-4 py-2.5 text-left text-sm hover:bg-accent">Link Bitbucket</button>
                <button className="w-full px-4 py-2.5 text-left text-sm hover:bg-accent rounded-b-lg">Update landing app</button>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="mb-4 flex items-center gap-2" ref={filterRef}>
        ...
        <div className="relative">
          <button onClick={() => setActiveFilter(activeFilter === "apps" ? null : "apps")} className={`flex items-center gap-1 rounded-md border px-3 py-2 text-sm ${activeFilter === "apps" ? "border-blue-600 text-blue-600" : "hover:bg-accent"}`}>
          ...
          {activeFilter === "apps" && (
            <div className="absolute left-0 top-full mt-1 w-56 rounded-lg border bg-background shadow-lg z-10">
              <div className="p-2">
                <div className="relative"><Input placeholder="Search" className="h-8 text-xs pr-8" /><svg className="absolute right-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg></div>
              </div>
              <div className="max-h-64 overflow-y-auto px-2 pb-2">
                {appFilterOptions.map((app) => (
                  <label key={app} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-accent cursor-pointer">
                    <input type="checkbox" className="size-4 rounded border" />
                    <AppIcon icon={app === "Goals" ? "◎" : app === "Jira" ? "◆" : app === "Jira Administration" ? "⚙" : "✦"} />
                    <span>{app}</span>
                  </label>
                ))}
              </div>
        ...
        {/* Plans filter */}
        <div className="relative">
          <button onClick={() => setActiveFilter(activeFilter === "plans" ? null : "plans")} className={`flex items-center gap-1 rounded-md border px-3 py-2 text-sm ${activeFilter === "plans" ? "border-blue-600 text-blue-600" : "hover:bg-accent"}`}>
            Plans <svg className="size-3" viewBox="0 0 16 16" fill="currentColor"><path d="M4 6l4 4 4-4" /></svg>
          </button>
          {activeFilter === "plans" && (
            <div className="absolute left-0 top-full mt-1 w-48 rounded-lg border bg-background shadow-lg z-10">
              <div className="p-2">
                <div className="relative"><Input placeholder="Search" className="h-8 text-xs pr-8" /><svg className="absolute right-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg></div>
              </div>
              <p className="px-3 py-1 text-xs font-bold text-muted-foreground uppercase">Edition</p>
              <div className="max-h-64 overflow-y-auto px-2 pb-2">
                {planFilterOptions.map((plan) => (
                  <label key={plan} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-accent cursor-pointer">
                    <input type="checkbox" className="size-4 rounded border" />
                    <span>{plan}</span>
                  </label>
                ))}
              </div>
              <div className="border-t px-3 py-2 text-xs text-muted-foreground text-right">3 of 3</div>
            </div>
          )}
        </div>
      </div>
          ...
          <thead>
            <tr className="border-b bg-muted/30">
              <th className="px-4 py-2.5 text-left font-medium">
                <div className="flex items-center gap-1">App <svg className="size-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 19V5" /><path d="M5 12l7-7 7 7" /></svg></div>
              </th>
              <th className="px-4 py-2.5 text-left font-medium w-[100px]">Plan</th>
              <th className="px-4 py-2.5 text-left font-medium w-[80px]">Users</th>
              <th className="px-4 py-2.5 text-left font-medium w-[120px]">Actions</th>
            ...
            {apps.map((app) => (
              <tr key={app.name} className="border-b last:border-b-0 hover:bg-accent/30 transition-colors">
                <td className="px-4 py-3">
                  <Link href={`/admin/atlassian-apps/${app.slug}`} className="flex items-center gap-3">
                ...
                <td className="px-4 py-3 relative">
                  <button onClick={() => setRowMenu(rowMenu === app.slug ? null : app.slug)} className="rounded p-1 text-muted-foreground hover:bg-accent">
                    <svg className="size-4" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="1.5" /><circle cx="12" cy="12" r="1.5" /><circle cx="12" cy="19" r="1.5" /></svg>
                  </button>
                  {rowMenu === app.slug && (
                    <div className="absolute right-0 top-full mt-1 w-48 rounded-lg border bg-background shadow-lg z-20">
                      <Link href="/admin/users" className="block px-4 py-2.5 text-sm hover:bg-accent">Manage users</Link>
                      <Link href={`/admin/atlassian-apps/${app.slug}`} className="block px-4 py-2.5 text-sm hover:bg-accent">Manage app</Link>
                      <div className="border-t" />
                      <Link href={`/admin/atlassian-apps/${app.slug}`} className="px-4 py-2.5 text-sm hover:bg-accent flex items-center justify-between">
                        Open {app.name}
                      ...
                      <Link href={`/admin/atlassian-apps/${app.slug}`} className="px-4 py-2.5 text-sm hover:bg-accent rounded-b-lg flex items-center justify-between">
                        {app.name} settings
                        <svg className="size-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>
                      </Link>
                    </div>
      ...
      <div className="rounded-lg border p-6">
        <div className="flex items-start gap-4">
          <div className="flex-1">
            ...
            <p className="mb-1 text-sm font-semibold">Make your work more visible with Jira and Jira Service Management</p>
