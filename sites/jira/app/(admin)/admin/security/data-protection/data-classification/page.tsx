    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center max-w-lg mx-auto">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          ...
          <Link href="/admin/security/security-guide" className="text-blue-600 hover:underline">How data classification works</Link>
        </p>
        ...
        {showBenefits && (
          <div className="mt-8 w-full rounded-lg border bg-card p-6 text-left">
            <h3 className="text-base font-semibold mb-4">Benefits of Data Classification</h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-start gap-3">
                <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full bg-green-100">
                  <svg className="size-4 text-green-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>
                </div>
                <div>
                  <p className="text-sm font-medium">Data governance</p>
                  <p className="text-xs text-muted-foreground">Define classification levels and enforce data handling policies across your organization.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full bg-blue-100">
                  <svg className="size-4 text-blue-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0110 0v4" /></svg>
                </div>
                <div>
                  <p className="text-sm font-medium">Compliance</p>
                  <p className="text-xs text-muted-foreground">Meet regulatory requirements like GDPR, HIPAA, and SOC 2 with proper data categorization.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full bg-purple-100">
                  <svg className="size-4 text-purple-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>
                </div>
                <div>
                  <p className="text-sm font-medium">User awareness</p>
                  <p className="text-xs text-muted-foreground">Help users understand data sensitivity when creating and sharing content.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full bg-yellow-100">
                  <svg className="size-4 text-yellow-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4" /><path d="M12 8h.01" /></svg>
                </div>
                <div>
                  <p className="text-sm font-medium">Risk reduction</p>
                  <p className="text-xs text-muted-foreground">Prevent accidental data exposure by labeling sensitive information clearly.</p>
                </div>
              </div>
            </div>
          </div>
