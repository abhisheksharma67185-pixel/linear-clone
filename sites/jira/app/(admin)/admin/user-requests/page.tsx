...
export default function UserRequestsPage() {
  ...
  return (
    <div className="p-8">
      ...
      <p className="mb-6 text-sm text-muted-foreground">Manage your users&apos; app access requests and discover what else they&apos;re showing interest in.</p>
      ...
      {activeTab === "access" && (
        <>
          <div className="mb-4 flex items-center justify-between">
            <p className="text-sm text-muted-foreground">Approve or deny access requests from your users. Each user granted access counts toward your subscription. <button className="text-blue-600 hover:underline">How requests for access work</button></p>
            <Link href="/admin/app-access-settings">
              <Button variant="outline" size="sm">App access settings</Button>
            </Link>
          </div>
          ...
          <div className="mb-4 relative inline-block">
            ...
            >
              Site <svg className="size-3" viewBox="0 0 16 16" fill="currentColor"><path d="M4 6l4 4 4-4" /></svg>
            </button>
            {siteDropdownOpen && (
              <>
                ...
                <div className="absolute left-0 top-full mt-1 z-50 w-56 rounded-lg border bg-background shadow-lg">
                  {["All sites", "abhisheksharma67185"].map((site) => (
                    <button
                      key={site}
                      className={`w-full text-left px-4 py-2.5 text-sm hover:bg-accent/50 transition-colors first:rounded-t-lg last:rounded-b-lg ${selectedSite === site ? "font-medium" : ""}`}
                      onClick={() => {
          ...
          {selectedSite !== "All sites" && (
            <p className="mb-4 text-xs text-muted-foreground">Site: {selectedSite}</p>
          )}
          ...
          <div className="rounded-lg border">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="font-medium">Who needs access</TableHead>
              ...
              <TableBody>
                <TableRow>
                  <TableCell colSpan={4} className="h-20 text-center text-sm text-muted-foreground italic">
      ...
      {activeTab === "interests" && (
        <div className="py-12 text-center">
          <p className="text-sm text-muted-foreground">No user interests to display.</p>
        </div>
      )}
