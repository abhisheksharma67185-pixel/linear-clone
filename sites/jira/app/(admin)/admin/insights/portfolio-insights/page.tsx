...
function ActionsDropdown({ appName, goToUrl }: { appName: string; goToUrl: string }) {
    ...
    <div className="relative inline-flex justify-end" ref={ref}>
      <button
        ...
        className={`rounded-md border p-1 text-muted-foreground transition-colors ${
          open ? "border-blue-600 bg-accent" : "border-transparent hover:bg-accent"
        }`}
      >
...
function MoreActionsDropdown() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1 w-56 rounded-md border bg-popover py-1 shadow-md">
          <button
...
export default function PortfolioInsightsPage() {
  ...
  const [detectedOpenFilter, setDetectedOpenFilter] = useState<FilterName | null>(null)
  const [detectedSelectedApps, setDetectedSelectedApps] = useState<Set<string>>(new Set())
  const [detectedSelectedDeployments, setDetectedSelectedDeployments] = useState<Set<string>>(new Set())

  // Hidden apps view filters
  const [hiddenSearchUrl, setHiddenSearchUrl] = useState("")
  const [hiddenOpenFilter, setHiddenOpenFilter] = useState<FilterName | null>(null)
  const [hiddenSelectedApps, setHiddenSelectedApps] = useState<Set<string>>(new Set())
  const [hiddenSelectedDeployments, setHiddenSelectedDeployments] = useState<Set<string>>(new Set())
  ...
  const connectivityOptions = [...new Set(APPS_DATA.map((a) => a.connectivityStatus).filter(Boolean))]
  const insightOptions = [...new Set(APPS_DATA.map((a) => a.insightStatus).filter(Boolean))]

  // Filter the data
  const filteredApps = APPS_DATA.filter((app) => {
    if (searchUrl && !app.url.toLowerCase().includes(searchUrl.toLowerCase())) return false
    if (selectedApps.size > 0 && !selectedApps.has(app.name)) return false
    if (selectedDeployments.size > 0 && !selectedDeployments.has(app.deployment)) return false
    if (selectedConnectivity.size > 0 && app.connectivityStatus && !selectedConnectivity.has(app.connectivityStatus)) return false
    if (selectedInsight.size > 0 && app.insightStatus && !selectedInsight.has(app.insightStatus)) return false
    return true
  }).sort((a, b) =>
    ...
    return (
      <div className="p-8 max-w-5xl">
            ...
            <Dialog open={feedbackOpen} onOpenChange={setFeedbackOpen}>
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Help us improve Portfolio insights. Share your experience, report bugs, or suggest new features.
                  </DialogDescription>
                </DialogHeader>
                {feedbackSubmitted ? (
                  <div className="flex flex-col items-center gap-2 py-4">
                    ...
                    <p className="text-xs text-muted-foreground">We appreciate you taking the time to help us improve.</p>
                  </div>
                ) : (
                  <>
                    <div className="flex flex-col gap-2">
                      <label className="text-xs font-medium" htmlFor="feedback-hidden">
                        Your feedback
                      </label>
                      <textarea
                        id="feedback-hidden"
                        ...
                        className="min-h-[100px] rounded-md border bg-transparent px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent resize-none"
                      />
                    </div>
                    <DialogFooter>
                      <Button
            ...
            <Dialog open={connectOpen} onOpenChange={setConnectOpen}>
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Connect your Atlassian Data Center instance to get insights and recommendations on performance and cloud readiness.
                  </DialogDescription>
                </DialogHeader>
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium" htmlFor="dc-url-hidden">
                      Data Center URL
                    </label>
                    <Input
                      id="dc-url-hidden"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium" htmlFor="dc-token-hidden">
                      Access token
                    </label>
                    <Input
                      id="dc-token-hidden"
        ...
        <div className="flex gap-6 border-b mb-4">
          ...
          <button
            className="pb-2.5 text-sm font-medium border-b-2 border-blue-600 text-blue-600 transition-colors"
          >
            Detected apps
          </button>
        </div>
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Atlassian apps that we detected based on app links and license records.
        </p>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <FilterDropdown
            ...
            onToggle={(v) => setHiddenSelectedApps(toggleInSet(hiddenSelectedApps, v))}
          ...
          <FilterDropdown
            ...
            onToggle={(v) => setHiddenSelectedDeployments(toggleInSet(hiddenSelectedDeployments, v))}
        ...
        <>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <FilterDropdown
              ...
              onToggle={(v) => setSelectedDeployments(toggleInSet(selectedDeployments, v))}
            ...
            <FilterDropdown
              ...
              onToggle={(v) => setSelectedConnectivity(toggleInSet(selectedConnectivity, v))}
            ...
            <FilterDropdown
              ...
              onToggle={(v) => setSelectedInsight(toggleInSet(selectedInsight, v))}
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
            <span>
              Showing {filteredApps.length} result{filteredApps.length !== 1 ? "s" : ""} out of {APPS_DATA.length} item{APPS_DATA.length !== 1 ? "s" : ""}
            </span>
            <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M21 12a9 9 0 1 1-6.219-8.56" />
            </svg>
                ...
                {filteredApps.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-4 py-12 text-center">
                      <p className="text-sm text-muted-foreground italic">No apps match your filters</p>
                    </td>
                  </tr>
                ) : (
                  filteredApps.map((app) => (
                    <tr key={app.name} className="border-b last:border-b-0 hover:bg-accent/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm">{app.connectivityStatus}</td>
                      <td className="px-4 py-3 text-sm">{app.insightStatus}</td>
                      <td className="px-4 py-3 text-right">
                        <ActionsDropdown appName={app.name} goToUrl={app.goToUrl} />
                      </td>
                    </tr>
                  ))
                )}
        ...
        <>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <FilterDropdown
              ...
              onToggle={(v) => setDetectedSelectedApps(toggleInSet(detectedSelectedApps, v))}
            ...
            <FilterDropdown
              ...
              onToggle={(v) => setDetectedSelectedDeployments(toggleInSet(detectedSelectedDeployments, v))}
              isOpen={detectedOpenFilter === "deployment"}
              onOpenChange={(o) => setDetectedOpenFilter(o ? "deployment" : null)}
            />
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr>
                  <td colSpan={4} className="px-4 py-12 text-center">
                    <p className="text-sm text-muted-foreground italic">
                      No detected apps yet. We&apos;ll notify you if we find anything
                    </p>
                  </td>
                </tr>
              </tbody>
