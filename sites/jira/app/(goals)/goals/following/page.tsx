...
function getTimelineRange(viewBy: ViewBy) {
  ...
  let end: Date
  if (viewBy === "Days") end = new Date(start.getFullYear(), start.getMonth() + 3, 0)
  else if (viewBy === "Weeks") end = new Date(start.getFullYear(), start.getMonth() + 6, 0)
  else if (viewBy === "Quarters") end = new Date(start.getFullYear() + 2, start.getMonth(), 0)
...
function getMonthColumns(start: Date, end: Date) {
  ...
  while (cur <= end) {
    cols.push({ year: cur.getFullYear(), month: cur.getMonth(), label: cur.toLocaleString("default", { month: "short" }) })
...
const STATUS_BAR_COLOR: Record<string, string> = {
  "ON TRACK": "#36b37e", "AT RISK": "#ffab00", "OFF TRACK": "#ff5630",
  "DONE": "#0052cc", "PENDING": "#8993a4",
}

function formatRangeLabel(start: Date, end: Date) {
  const fmt = (d: Date) => d.toLocaleString("default", { day: "numeric", month: "short", year: "numeric" })
...
function TimelineView({ goals, viewBy }: { goals: GoalRow[]; viewBy: ViewBy }) {
  ...
  const goalsWithDates = goals.filter((g) => parseTargetDate(g.targetDate) !== null)
  if (goalsWithDates.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <svg className="mb-4 size-20 text-muted-foreground/30" viewBox="0 0 80 80" fill="none"><circle cx="36" cy="36" r="26" stroke="currentColor" strokeWidth="3" strokeDasharray="4 3" /><circle cx="36" cy="36" r="14" stroke="currentColor" strokeWidth="3" /><line x1="56" y1="56" x2="72" y2="72" stroke="currentColor" strokeWidth="4" strokeLinecap="round" /><line x1="28" y1="36" x2="44" y2="36" stroke="currentColor" strokeWidth="3" strokeLinecap="round" /><line x1="36" y1="28" x2="36" y2="44" stroke="currentColor" strokeWidth="3" strokeLinecap="round" /></svg>
        <p className="text-sm text-muted-foreground">No goals with target dates found.</p>
      </div>
    )
  }
  if (viewBy === "Months") {
    ...
    const getBarStyle = (goal: GoalRow) => {
      ...
      const startOffset = Math.max(0, (barStart.getTime() - start.getTime()) / (1000 * 60 * 60 * 24))
      const endOffset = Math.min(totalDays, (barEnd.getTime() - start.getTime()) / (1000 * 60 * 60 * 24))
      const leftPct = (startOffset / totalDays) * 100
      const widthPct = Math.max(1, ((endOffset - startOffset) / totalDays) * 100)
      return { left: `${leftPct}%`, width: `${widthPct}%` }
    }
    const todayOffset = (today.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)
    const todayPct = Math.min(100, Math.max(0, (todayOffset / totalDays) * 100))
    return (
      <div className="rounded-lg border overflow-hidden">
        <div className="flex">
          <div className="w-52 shrink-0 border-r">
            <div className="h-10 border-b bg-muted/30 px-4 flex items-center text-xs font-medium text-muted-foreground">Name</div>
            {goalsWithDates.map((goal) => (
              <div key={goal.id} className="h-12 border-b last:border-b-0 px-4 flex items-center gap-2 hover:bg-accent/40 cursor-pointer">
          ...
          <div className="flex-1 overflow-x-auto">
            <div className="flex border-b bg-muted/30 relative" style={{ minWidth: cols.length * colWidth }}>
              {cols.map((col) => (
                <div key={`${col.year}-${col.month}`} className="shrink-0 border-r last:border-r-0 h-10 flex items-center justify-center text-xs font-medium text-muted-foreground" style={{ width: colWidth }}>
                  {col.label}
                </div>
              ))}
            </div>
            <div className="relative" style={{ minWidth: cols.length * colWidth }}>
              <div className="absolute top-0 bottom-0 z-10 w-px bg-blue-500 opacity-70" style={{ left: `${todayPct}%` }} />
              {goalsWithDates.map((goal) => {
                const barStyle = getBarStyle(goal)
                return (
                  <div key={goal.id} className="relative h-12 border-b last:border-b-0 flex items-center">
                    {cols.map((col, i) => (
                      <div key={i} className="absolute top-0 bottom-0 border-r border-[#f4f5f7] dark:border-border" style={{ left: `${((i + 1) / cols.length) * 100}%` }} />
                    ))}
                    {barStyle && (
                      <div className="absolute h-6 rounded-full flex items-center px-3"
                        style={{ left: barStyle.left, width: barStyle.width, backgroundColor: STATUS_BAR_COLOR[goal.status] ?? "#8993a4" }}>
                        <span className="text-[11px] text-white font-medium truncate">{goal.name}</span>
                      </div>
                    )}
                  </div>
                )
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      <p className="text-sm text-muted-foreground">Switch to Months view for timeline rendering.</p>
    </div>
  )
}
...
function DateRangePicker({ startDate, endDate, onConfirm, onClose }: {
  ...
  function getDaysInMonth(year: number, month: number) {
    ...
    for (let d = 1; d <= new Date(year, month + 1, 0).getDate(); d++) rows.push(d)
    return rows
  }
  function isSameDay(a: Date, b: Date) { return a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate() }
  function inRange(d: Date) { return d > selStart && d < selEnd }
  function navigate(which: "start" | "end", delta: number, years = false) {
    const setter = which === "start" ? setStartMonth : setEndMonth
    setter((m) => { const n = new Date(m); if (years) n.setFullYear(n.getFullYear() + delta); else n.setMonth(n.getMonth() + delta); return n })
  }
  function handleDayClick(year: number, month: number, day: number) {
    const clicked = new Date(year, month, day)
    if (clicked <= selStart || (clicked < selEnd && clicked > selStart)) setSelStart(clicked)
      ...
      <div className="w-[280px]">
        <div className="flex items-center justify-between mb-3">
          <div className="flex gap-0.5">
            <button onClick={() => navigate(which, -1, true)} className="rounded p-1 hover:bg-[#f4f5f7] text-[#626f86]"><svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="11 17 6 12 11 7" /><polyline points="18 17 13 12 18 7" /></svg></button>
            <button onClick={() => navigate(which, -1)} className="rounded p-1 hover:bg-[#f4f5f7] text-[#626f86]"><svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6" /></svg></button>
          </div>
          <span className="text-[14px] font-semibold text-[#172b4d]">{label}</span>
          <div className="flex gap-0.5">
            <button onClick={() => navigate(which, 1)} className="rounded p-1 hover:bg-[#f4f5f7] text-[#626f86]"><svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6" /></svg></button>
            <button onClick={() => navigate(which, 1, true)} className="rounded p-1 hover:bg-[#f4f5f7] text-[#626f86]"><svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="13 17 18 12 13 7" /><polyline points="6 17 11 12 6 7" /></svg></button>
          </div>
        </div>
        <div className="grid grid-cols-7 mb-1">{DAYS.map((d) => <div key={d} className="text-center text-[11px] font-medium text-[#626f86] py-1">{d}</div>)}</div>
        <div className="grid grid-cols-7 gap-y-0.5">
          {days.map((day, i) => {
            if (!day) return <div key={i} />
            const d = new Date(month.getFullYear(), month.getMonth(), day)
            const isStart = isSameDay(d, selStart); const isEnd = isSameDay(d, selEnd); const between = inRange(d)
            return (
              <button key={i} onClick={() => handleDayClick(month.getFullYear(), month.getMonth(), day)}
                className={`relative h-8 w-full rounded-full text-[13px] transition-colors ${isStart || isEnd ? "bg-[#0052cc] text-white font-semibold" : ""} ${between ? "bg-[#e9f2ff] rounded-none text-[#172b4d]" : ""} ${!isStart && !isEnd && !between ? "text-[#172b4d] hover:bg-[#f4f5f7]" : ""}`}>
                {day}
              </button>
            )
          })}
  ...
  return (
    <>
      ...
      <div className="absolute left-0 top-full z-50 mt-2 rounded-lg border border-[#dfe1e6] bg-white shadow-[0_8px_24px_rgba(9,30,66,0.15)] p-5">
        <div className="flex gap-8 mb-3"><p className="w-[280px] text-center text-[15px] font-bold text-[#172b4d]">Start</p><p className="w-[280px] text-center text-[15px] font-bold text-[#172b4d]">End</p></div>
        <div className="flex gap-8">{renderCalendar(startMonth, "start")}{renderCalendar(endMonth, "end")}</div>
        <div className="mt-4 flex justify-end">
          <button onClick={() => onConfirm(selStart, selEnd)} className="rounded-md bg-[#0052cc] hover:bg-[#0747a6] px-6 py-2 text-[13px] font-semibold text-white transition-colors">
...
export default function FollowingPage() {
  ...
  const handleSaveFieldValue = useCallback(async (goalId: string, fieldId: string, value: unknown) => {
    ...
    setGoals((prev) => prev.map((g) =>
      g.id === goalId ? { ...g, fieldValues: { ...g.fieldValues, [fieldId]: value } } : g
    ))
  ...
  const handleCreateGoal = async () => {
    ...
    setCreateOpen(false); setGoalName("")
  ...
  const handleRenameField = async () => {
    ...
    const err = validateFieldName(renameValue, existingNames.filter((_, i) => customFields[i]?.id !== renameFieldId))
    if (err) { setRenameError(err); return }
    try {
      await renameField(renameFieldId, renameValue.trim())
      showToast("Field renamed")
      setRenameOpen(false); setRenameFieldId(""); setRenameValue(""); setRenameError(null)
    } catch (e: unknown) {
  ...
  const handleDeleteField = async (id: string) => {
    ...
    } catch { showToast("Failed to delete field") }
  ...
  const SORT_OPTIONS = ["Name", "Status", "Target date", "Following", "Last updated", "Follower count"]
  ...
  const sortedGoals = [...filteredGoals].sort((a, b) => {
    ...
    else if (timelineSortBy === "Target date") cmp = a.targetDate.localeCompare(b.targetDate)
    else if (timelineSortBy === "Following") cmp = Number(b.following) - Number(a.following)
  ...
  const gridTemplate = enabledCols.map((c) =>
  ...
  const copyLink = () => { navigator.clipboard.writeText(window.location.href).then(() => showToast("Link copied to clipboard")) }
  const exportCSV = () => {
    const headers = ["Name", "Status", "Progress", "Target Date", "Owner", "Following"]
    const rows = sortedGoals.map((g) => [`"${g.name.replace(/"/g, '""')}"`, g.status, `${g.progress}%`, g.targetDate, `"${g.owner.name.replace(/"/g, '""')}"`, g.following ? "Following" : "Not following"])
    const csv = [headers.join(","), ...rows.map((r) => r.join(","))].join("\n")
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" })
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a"); a.href = url; a.download = "following-goals.csv"; a.click()
    ...
    <div className="p-6">
      ...
      <div className="mb-4 flex items-center gap-2">
        <div className="flex items-center gap-1 rounded-md border px-2.5 py-1 text-sm">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger render={<button className="flex items-center gap-1 text-sm">{followingFilter}<svg className="size-3 text-muted-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9" /></svg></button>} />
          ...
          <button className="ml-0.5 rounded-sm text-muted-foreground hover:text-foreground"><svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg></button>
        </div>
        <div className="relative">
          <button onClick={() => setShowAddFilter(!showAddFilter)} className={`flex items-center gap-1 text-sm transition-colors ${showAddFilter ? "text-blue-600 font-medium" : "text-muted-foreground hover:text-foreground"}`}>
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {viewMode === "timeline" && (
            <>
              <div className="relative">
                <button onClick={() => setShowDatePicker((v) => !v)} className={`flex items-center gap-1.5 rounded-md border px-3 py-1 text-sm transition-colors ${showDatePicker ? "border-blue-500 text-blue-600" : "text-muted-foreground hover:bg-accent"}`}>
                ...
                {showDatePicker && <DateRangePicker startDate={tlStart} endDate={tlEnd} onConfirm={(s, e) => { setCustomStart(s); setCustomEnd(e); setShowDatePicker(false) }} onClose={() => setShowDatePicker(false)} />}
              </div>
              <div className="relative">
                <button onClick={() => setShowViewBy((v) => !v)} className="flex items-center gap-1.5 rounded-md border px-3 py-1 text-sm text-muted-foreground hover:bg-accent">
                  View by {viewBy}<svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9" /></svg>
                </button>
                {showViewBy && (
                  <>
                    <div className="fixed inset-0 z-40" onClick={() => setShowViewBy(false)} />
                    <div className="absolute right-0 top-full z-50 mt-1 w-36 rounded-md border bg-popover shadow-md py-1">
                      {VIEW_BY_OPTIONS.map((opt) => (
                        <button key={opt} onClick={() => { setViewBy(opt); setShowViewBy(false) }} className={`flex w-full items-center gap-2 px-3 py-2 text-sm transition-colors hover:bg-accent ${viewBy === opt ? "text-blue-600 font-medium" : "text-foreground"}`}>
                          {viewBy === opt && <svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>}
          ...
          <div className="flex rounded-md border">
            <button title="Display as list" onClick={() => setViewMode("list")} className={`px-2 py-1 rounded-l-md border-r transition-colors ${viewMode === "list" ? "bg-blue-50 dark:bg-blue-900/20" : "text-muted-foreground hover:bg-accent"}`}>
            ...
            <button title="Display as timeline" onClick={() => setViewMode("timeline")} className={`px-2 py-1 rounded-r-md transition-colors ${viewMode === "timeline" ? "bg-blue-50 dark:bg-blue-900/20" : "text-muted-foreground hover:bg-accent"}`}>
              <svg className={`size-4 ${viewMode === "timeline" ? "text-blue-600" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="10" x2="16" y2="10" /><line x1="3" y1="14" x2="21" y2="14" /><line x1="3" y1="18" x2="13" y2="18" /></svg>
            </button>
          </div>
          ...
          {viewMode === "list" ? (
            <button className="flex items-center gap-1.5 rounded-md border px-3 py-1 text-sm text-muted-foreground hover:bg-accent">
              Sort by following <svg className="size-3.5 ml-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9" /></svg>
            </button>
          ) : (
            <div className="flex items-center gap-0">
              <div className="relative">
                <button onClick={() => { setShowSortMenu((v) => !v); setSortSearch("") }} className={`flex items-center gap-1.5 rounded-l-md border border-r-0 px-3 py-1 text-sm transition-colors ${showSortMenu ? "border-blue-500 text-blue-600 bg-blue-50" : "text-muted-foreground hover:bg-accent"}`}>
                  Sort by {timelineSortBy}
                </button>
                {showSortMenu && (
                  <>
                    <div className="fixed inset-0 z-40" onClick={() => setShowSortMenu(false)} />
                    <div className="absolute right-0 top-full z-50 mt-1 w-52 rounded-md border border-[#dfe1e6] bg-white dark:bg-popover shadow-[0_8px_24px_rgba(9,30,66,0.15)]">
                      <div className="relative border-b border-[#dfe1e6] px-3 py-2">
                        <input autoFocus value={sortSearch} onChange={(e) => setSortSearch(e.target.value)} placeholder="Select..." className="w-full bg-transparent text-[13px] text-[#172b4d] outline-none placeholder:text-[#626f86]" />
                        <svg className="absolute right-3 top-1/2 -translate-y-1/2 size-4 text-[#626f86]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
                      </div>
                      <div className="px-3 py-1.5"><p className="text-[11px] font-semibold text-[#626f86] uppercase tracking-wider">Sort by</p></div>
                      <div className="pb-1">
                        {SORT_OPTIONS.filter((o) => !sortSearch || o.toLowerCase().includes(sortSearch.toLowerCase())).map((opt) => (
                          <button key={opt} onClick={() => { setTimelineSortBy(opt); setShowSortMenu(false) }} className={`flex w-full items-center px-3 py-2 text-[13px] transition-colors hover:bg-[#f4f5f7] ${timelineSortBy === opt ? "bg-[#e9f2ff] text-[#0052cc] font-medium" : "text-[#172b4d]"}`}>{opt}</button>
                        ))}
                      </div>
                    </div>
                  </>
              ...
              <button onClick={() => setTimelineSortAsc((v) => !v)} title="Reverse sort order" className="group/rsort relative rounded-r-md border px-2 py-1 text-muted-foreground hover:bg-accent transition-colors">
                <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  {timelineSortAsc ? <polyline points="6 9 12 15 18 9" /> : <polyline points="18 15 12 9 6 15" />}
                </svg>
              </button>
            </div>
          )}
                      ...
                      <div className="flex items-center gap-1 ml-2 shrink-0">
                        ...
                        {col.locked ? (
                          ...
                          <button
                            onClick={() => {
                              if (col.custom) {
                                setCustomColEnabled((prev) => ({ ...prev, [col.id]: !col.enabled }))
                              } else {
                                setBaseColState((prev) => ({ ...prev, [col.id]: !col.enabled }))
                              }
                            }}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem onClick={copyLink} className="flex items-center gap-2.5 cursor-pointer">
                <svg className="size-4 text-muted-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" /></svg>
                Copy link
              </DropdownMenuItem>
              <DropdownMenuItem onClick={exportCSV} className="flex items-center gap-2.5 cursor-pointer">
                <svg className="size-4 text-muted-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" y1="15" x2="12" y2="3" /></svg>
                Export CSV
              </DropdownMenuItem>
              <DropdownMenuItem onClick={(e) => { e.preventDefault(); setWrapText((v) => !v) }} className="flex items-center justify-between gap-2.5 cursor-pointer">
                <span className="flex items-center gap-2.5">
                  <svg className="size-4 text-muted-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><path d="M3 6h18" /><path d="M3 12h15a3 3 0 0 1 0 6h-4" /><polyline points="14 15 11 18 14 21" /><path d="M3 18h5" /></svg>
                  Wrap text
                </span>
                <div className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ${wrapText ? "bg-[#36b37e]" : "bg-[#97a0af]"}`}>
                  <span className={`inline-block size-3.5 rounded-full bg-white shadow-sm transition-transform ${wrapText ? "translate-x-[18px]" : "translate-x-[3px]"}`} />
                  {!wrapText && <svg className="absolute right-1 size-2.5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>}
                  {wrapText && <svg className="absolute left-1.5 size-2.5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}
                </div>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      {viewMode === "list" ? (
        <div className="rounded-lg border overflow-x-auto">
          {/* Header */}
          <div className="grid gap-4 border-b px-4 py-2.5 text-xs font-medium text-muted-foreground min-w-0" style={{ gridTemplateColumns: gridTemplate }}>
            {enabledCols.map((col) => (
              <span key={col.id} className={`truncate ${col.custom && col.fieldType === "Number" ? "text-right" : ""}`}>{col.label}</span>
            ))}
          </div>
          {/* Rows */}
          {filteredGoals.map((goal) => (
            <div
              ...
              className={`grid gap-4 border-b last:border-b-0 px-4 py-3 hover:bg-accent/50 transition-colors cursor-pointer min-w-0 ${wrapText ? "items-start" : "items-center"}`}
              style={{ gridTemplateColumns: gridTemplate }}
            >
              {enabledCols.map((col) => {
                if (col.id === "name") return (
                  <div key="name" className="flex items-start gap-2 min-w-0" onClick={(e) => e.stopPropagation()}>
                    <svg className="size-4 text-muted-foreground/50 shrink-0 mt-0.5 cursor-pointer" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" onClick={() => router.push(`/goals/${goal.id}`)}><circle cx="12" cy="12" r="10" /><circle cx="12" cy="12" r="3" /></svg>
                    <span className={`text-sm cursor-pointer hover:underline ${wrapText ? "whitespace-normal break-words" : "truncate"}`} onClick={() => router.push(`/goals/${goal.id}`)}>{goal.name}</span>
                  </div>
                )
                ...
                if (col.id === "progress") return (
                  <div key="progress" className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                ...
                if (col.id === "target_date") return (
                  <div key="target_date" className="flex items-center gap-1.5 text-xs text-muted-foreground" onClick={(e) => e.stopPropagation()}>
                ...
                if (col.id === "following") return (
                  <div key="following" onClick={(e) => e.stopPropagation()}>
                    {goal.following ? <span className="text-xs text-muted-foreground">Following</span> : <span className="text-xs text-muted-foreground">—</span>}
                  </div>
                )
                ...
                return <div key={col.id} className="text-xs text-muted-foreground" onClick={(e) => e.stopPropagation()}>—</div>
              })}
            </div>
          ))}
          {filteredGoals.length === 0 && <div className="py-12 text-center text-sm text-muted-foreground">No goals found.</div>}
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => { setRenameOpen(false); setRenameError(null) }}>
          ...
          <div className="relative z-10 w-full max-w-[360px] rounded-lg border bg-popover shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2 border-t px-5 py-4">
              ...
              <button onClick={handleRenameField} disabled={!renameValue.trim()} className="rounded-[3px] bg-[#0052cc] px-4 py-1.5 text-[13px] font-medium text-white hover:bg-[#0747a6] disabled:opacity-50">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 flex items-center gap-2 rounded-lg bg-[#172b4d] px-4 py-2.5 text-[13px] font-medium text-white shadow-lg animate-in fade-in slide-in-from-bottom-2">
          <svg className="size-4 text-green-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>
          {toast}
        </div>
      )}
    </div>
