    ...
    <div className="p-6">
      ...
      <div className="mb-5 flex items-center gap-1 border-b">
        <button
          ...
          className={`pb-2 px-2 text-sm font-medium transition-colors ${activeTab === "all" ? "border-b-2 border-blue-600 text-blue-600" : "text-muted-foreground hover:text-foreground"}`}
        >
          All goals
        </button>
        {customViews.map((v) => (
        ...
        <div className="rounded-lg border">
          ...
          {filteredGoals.map((goal) => (
            <div key={goal.id} className="grid grid-cols-[1fr_90px_120px_110px_70px_80px] gap-4 border-b last:border-b-0 px-4 py-3 items-center hover:bg-accent/50 transition-colors cursor-pointer">
              ...
              <div>
                <span className="rounded px-2 py-0.5 text-[10px] font-bold uppercase bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-300">
                  {goal.status}
                </span>
              </div>
              <div className="flex items-center gap-2">
