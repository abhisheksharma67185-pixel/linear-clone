...
const seedUpdates: UpdateEntry[] = [
  {
    id: "upd-1", goalId: "goal-1", goalName: "Increase platform uptime to 99.9%",
    author: { name: "Abhishek Sharma", initials: "AS" }, status: "ON TRACK",
    date: "2026-04-10T10:00:00Z",
    body: "Uptime hit 99.85% in March. Migrated two more services to the redundant cluster. On track for 99.9% by June.",
  },
  {
    id: "upd-2", goalId: "goal-8", goalName: "Ship redesigned onboarding flow",
    author: { name: "Taylor Brown", initials: "TB" }, status: "ON TRACK",
  ...
  {
    id: "upd-3", goalId: "goal-5", goalName: "Achieve SOC 2 Type II compliance",
    author: { name: "Abhishek Sharma", initials: "AS" }, status: "AT RISK",
  ...
  {
    id: "upd-4", goalId: "goal-2", goalName: "Reduce customer churn by 15%",
    author: { name: "Sam Williams", initials: "SW" }, status: "AT RISK",
  ...
  {
    id: "upd-5", goalId: "goal-6", goalName: "Grow monthly active users to 50K",
    author: { name: "Sam Williams", initials: "SW" }, status: "OFF TRACK",
...
function statusBadge(status: string) {
  const cls =
    ...
    status === "AT RISK" ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300" :
    status === "OFF TRACK" ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400" :
    status === "DONE" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" :
    "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300"
  return <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold uppercase ${cls}`}>{status}</span>
}

function statusDot(status: string) {
  const cls =
    status === "ON TRACK" ? "bg-green-500" :
    status === "AT RISK" ? "bg-yellow-400" :
...
export default function StatusUpdatesPage() {
  ...
  const activeGoals = followedGoals.filter((g) => g.status !== "DONE" && g.status !== "CANCELLED")
  ...
  const noUpdate = followedGoals.filter((g) => g.status === "PENDING" || g.status === "PAUSED").length
  ...
  return (
    <div className="flex gap-0">
      ...
      <div className="flex-1 p-6 min-w-0">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => setCatchUpOpen(true)}>Catch up</Button>
            <Button variant="outline" onClick={() => setWriteOpen(true)}>Write updates</Button>
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="mt-3 flex items-center gap-3">
                <Button className="bg-green-600 text-white hover:bg-green-700" size="sm" onClick={() => router.push("/goals")}>
                  Create your first goal
                </Button>
                <button className="text-sm text-muted-foreground hover:underline" onClick={() => router.push("/goals")}>More about goals</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 flex items-center justify-center gap-4">
          <button onClick={prevMonth} className="rounded p-1 text-muted-foreground hover:bg-accent">
          ...
          <button onClick={nextMonth} className="rounded p-1 text-muted-foreground hover:bg-accent">
        ...
        <p className="mb-4 text-sm font-medium text-muted-foreground">
          You&apos;re following {activeGoals.length} active goal{activeGoals.length !== 1 ? "s" : ""}, here&apos;s the breakdown.
        </p>

        {/* Status grid */}
        <div className="mb-8 grid grid-cols-3 gap-px rounded-lg border overflow-hidden">
          {[
            { count: onTrack, label: "On track", sub: onTrack > 0 ? `+${onTrack} from last month` : "No change", color: "text-green-600", dot: "bg-green-500" },
            { count: atRisk, label: "At risk", sub: atRisk > 0 ? `${atRisk} goal${atRisk !== 1 ? "s" : ""}` : "No change", color: "text-yellow-600", dot: "bg-yellow-400" },
            { count: offTrack, label: "Off track", sub: offTrack > 0 ? `${offTrack} goal${offTrack !== 1 ? "s" : ""}` : "No change", color: "text-red-600", dot: "bg-red-500" },
            { count: noUpdate, label: "No update", sub: noUpdate > 0 ? `${noUpdate} pending` : "No change", color: "text-gray-500", dot: "bg-gray-400" },
            { count: cancelled, label: "Cancelled", sub: "No change", color: "text-gray-500", dot: "bg-gray-400" },
            { count: completed, label: "Completed", sub: completed > 0 ? `${completed} done` : "No change", color: "text-gray-500", dot: "bg-gray-400" },
          ].map((item) => (
        ...
        {monthUpdates.length > 0 ? (
              ...
              <div key={update.id} className="rounded-lg border p-5">
                ...
                <div className="flex items-start gap-3">
                  <Avatar className="size-8">
                    <AvatarFallback className="bg-purple-600 text-xs text-white">{update.author.initials}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1">
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      <span className="text-sm font-medium">{update.author.name}</span>
                      <span className="text-xs text-muted-foreground">{timeAgo(update.date)}</span>
          ...
          <div className="flex flex-col items-center py-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-1">No updates for {MONTHS[currentMonth]}</h3>
            <p className="text-sm text-muted-foreground max-w-md">
              Goal owners need to know someone&apos;s listening to invest time in writing monthly updates.
            </p>
            <Button className="mt-4" variant="outline" onClick={() => setWriteOpen(true)}>
              Write an update
            </Button>
          </div>
        )}
      ...
      {/* Right sidebar */}
      <div className="hidden xl:block w-64 shrink-0 border-l p-6">
        {/* Your goals */}
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              {followedGoals.slice(0, 5).map((g) => (
                <button key={g.id} onClick={() => router.push(`/goals/${g.id}`)} className="flex items-center gap-2 w-full text-left hover:bg-accent rounded-md px-1 py-1 -mx-1 transition-colors">
                  {statusDot(g.status)}
                  <span className="text-sm truncate flex-1">{g.name}</span>
                </button>
              ))}
              {followedGoals.length > 5 && (
                <button onClick={() => router.push("/goals/following")} className="text-xs text-blue-600 hover:underline">
          ...
          {completedGoals.length > 0 ? (
            <div className="space-y-2">
              {completedGoals.map((g) => (
                <button key={g.id} onClick={() => router.push(`/goals/${g.id}`)} className="flex items-center gap-2 w-full text-left hover:bg-accent rounded-md px-1 py-1 -mx-1 transition-colors">
                  <svg className="size-4 text-green-500 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>
                  <span className="text-sm truncate flex-1">{g.name}</span>
                </button>
              ))}
            </div>
      ...
      {/* Toast */}
      {writeToast && (
        <div className="fixed top-4 right-4 z-[100] rounded-lg border bg-background px-4 py-3 shadow-lg text-sm flex items-center gap-2 animate-in fade-in slide-in-from-top-2">
...
function GreenCheckmark({ className }: { className?: string }) {
  return (
    <div className={`relative ${className}`}>
      {/* Sparkles */}
      <svg className="absolute -top-3 -left-2 size-5 text-emerald-400 animate-pulse" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 2l1.5 4.5L18 8l-4.5 1.5L12 14l-1.5-4.5L6 8l4.5-1.5z" />
      </svg>
      <svg className="absolute -top-1 -right-3 size-4 text-emerald-300 animate-pulse" style={{ animationDelay: "0.3s" }} viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 2l1.5 4.5L18 8l-4.5 1.5L12 14l-1.5-4.5L6 8l4.5-1.5z" />
      </svg>
      <svg className="absolute bottom-2 -left-4 size-3 text-emerald-400 animate-pulse" style={{ animationDelay: "0.6s" }} viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 2l1.5 4.5L18 8l-4.5 1.5L12 14l-1.5-4.5L6 8l4.5-1.5z" />
      </svg>
      <svg className="absolute bottom-0 right-0 size-3 text-yellow-400 animate-pulse" style={{ animationDelay: "0.4s" }} viewBox="0 0 24 24" fill="currentColor">
        <circle cx="12" cy="12" r="5" />
      </svg>
      {/* Main circle */}
      <div className="size-28 rounded-full bg-gradient-to-br from-emerald-400 to-green-500 flex items-center justify-center shadow-lg shadow-green-200 dark:shadow-green-900/30">
...
function CatchUpOverlay({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 backdrop-blur-sm animate-in fade-in duration-300">
      ...
      {/* Close button */}
      <button onClick={onClose} className="fixed top-4 right-4 z-[70] rounded p-2 text-muted-foreground hover:bg-accent transition-colors">
        <svg className="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
      </button>

      {/* Content card */}
      <div className="relative z-[65] w-full max-w-md rounded-xl border bg-background p-10 shadow-xl text-center animate-in zoom-in-95 duration-500">
        <div className="flex justify-center mb-6">
          <GreenCheckmark />
        </div>
        <h2 className="text-xl font-bold mb-3">You&apos;re all caught up</h2>
        <p className="text-sm text-muted-foreground leading-relaxed">
          Thanks for <span className="underline decoration-dotted cursor-help">showing teams that you&apos;re paying attention</span> by <span className="underline decoration-dotted cursor-help">engaging with status updates</span>. See you next week!
        </p>
      </div>
    </div>
  )
...
function WriteUpdatesOverlay({
  goals, onClose, onPublish,
  writeGoalId, setWriteGoalId,
  writeStatus, setWriteStatus,
  writeBody, setWriteBody,
}: {
  goals: GoalRow[]
  onClose: () => void
  onPublish: () => void
  ...
  const pendingGoals = goals.filter((g) => showPending ? true : g.status !== "PENDING" && g.status !== "PAUSED")
  const dueGoals = pendingGoals.filter((g) => g.status !== "DONE" && g.status !== "CANCELLED")
    ...
    <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-sm animate-in fade-in duration-300">
      ...
      <div className="fixed top-4 right-4 z-10 flex items-center gap-2">
        ...
        <div className="relative">
          <button onClick={() => setFilterOpen(!filterOpen)} className="rounded p-2 text-muted-foreground hover:bg-accent transition-colors">
          ...
          {filterOpen && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1 w-64 rounded-lg border bg-popover p-4 shadow-lg z-10">
                <h3 className="text-sm font-semibold mb-2">Update filtering</h3>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">
                    Show <span className="rounded border px-1.5 py-0.5 text-[10px] font-bold uppercase">PENDING</span> and <span className="rounded border px-1.5 py-0.5 text-[10px] font-bold uppercase">PAUSED</span>
                  </span>
                  <button
                  ...
                  >
                    <span className={`inline-block size-3.5 rounded-full bg-white transition-transform ${showPending ? "translate-x-[18px]" : "translate-x-1"}`} />
                  </button>
                </div>
              </div>
            </>
        ...
        <button onClick={onClose} className="rounded p-2 text-muted-foreground hover:bg-accent transition-colors">
          <svg className="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
        </button>
      </div>
      ...
      <div className="flex h-full items-center justify-center">
        {dueGoals.length === 0 && !hasGoalSelected ? (
          ...
          <div className="w-full max-w-lg text-center animate-in zoom-in-95 duration-500 px-6">
            <div className="flex justify-center mb-8">
              <GreenCheckmark />
            </div>
            <h2 className="text-xl font-bold mb-4">You don&apos;t have any updates due in the next 7 days</h2>
            <p className="text-sm text-muted-foreground mb-2">
              Expecting to see updates? Try changing{" "}
              <button onClick={() => setFilterOpen(true)} className="inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-sm font-medium hover:bg-accent transition-colors">
                Update Filters
              </button>
            </p>
            <p className="text-sm text-muted-foreground mt-4 leading-relaxed">
              ...
              <span className="text-blue-600 hover:underline cursor-pointer">
                ...
                <svg className="inline size-3 ml-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>
              </span>
            </p>
            <button onClick={onClose} className="mt-6 rounded-md border px-6 py-2 text-sm font-medium hover:bg-accent transition-colors">
              Done
            </button>
          </div>
        ) : (
          /* Has goals to update — show the write form */
          <div className="w-full max-w-lg animate-in slide-in-from-bottom-4 duration-500 px-6">
            <div className="rounded-xl border bg-background p-8 shadow-xl">
              <h2 className="text-xl font-bold mb-6">Write your update</h2>
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-medium mb-1.5 block">Goal <span className="text-red-500">*</span></label>
                  <select value={writeGoalId} onChange={(e) => setWriteGoalId(e.target.value)} className="w-full rounded-md border px-3 py-2.5 text-sm bg-background">
                    <option value="">Select a goal...</option>
                    {dueGoals.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <button key={s.value} onClick={() => setWriteStatus(s.value)}
                ...
                <div>
                  <label className="text-sm font-medium mb-1.5 block">Update <span className="text-red-500">*</span></label>
                  <textarea value={writeBody} onChange={(e) => setWriteBody(e.target.value)} placeholder="Share what's happening with this goal..." rows={4}
              ...
              <div className="flex justify-end gap-2 mt-6">
                <button onClick={onClose} className="rounded-md border px-4 py-2 text-sm font-medium hover:bg-accent transition-colors">Cancel</button>
                <button disabled={!writeGoalId || !writeBody.trim()} onClick={onPublish}
