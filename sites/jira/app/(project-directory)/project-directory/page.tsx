...
function ProjectRowMenu({ project, onArchive, onToast, starred, onToggleStar }: {
  ...
  return (
    <>
      <button
        ...
        className={`rounded p-1 transition-colors ${open ? "border border-blue-500 bg-blue-50 text-blue-600" : "text-muted-foreground hover:text-foreground hover:bg-accent"}`}
      >
      ...
      {open && (
        <div
          ...
          className="z-[9999] w-44 rounded-lg border bg-popover shadow-lg py-1"
          onClick={(e) => e.stopPropagation()}
        >
          <button
            type="button"
            onClick={() => { setOpen(false); onToggleStar(project.id); onToast(starred ? `Unstarred ${project.name}` : `Starred ${project.name}`) }}
            className="flex w-full items-center gap-2.5 px-3 py-2 text-sm hover:bg-accent transition-colors text-left"
          >
            <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill={starred ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.5">
          ...
          <button
            ...
            className="flex w-full items-center gap-2.5 px-3 py-2 text-sm hover:bg-accent transition-colors text-left"
          >
            <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg>
            Archive project
          </button>
          <button
            type="button"
            onClick={() => { setOpen(false); onToast(`"${project.name}" copied`) }}
            className="flex w-full items-center gap-2.5 px-3 py-2 text-sm hover:bg-accent transition-colors text-left"
          >
            <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
            Copy project
          </button>
        </div>
      )}
...
function SortByDropdown({ sortBy, sortAsc, onSelect, onReverse }: { sortBy: string; sortAsc: boolean; onSelect: (s: string) => void; onReverse: () => void }) {
  const [open, setOpen] = useState(false)
  ...
  return (
    <div ref={ref} className="relative flex items-center">
      <button
        ...
        className="flex items-center gap-1.5 rounded-l-lg border border-r-0 px-3 py-1.5 text-sm text-muted-foreground hover:bg-accent transition-colors whitespace-nowrap"
      >
        Sort by {sortBy}
      </button>
      <div className="group/rev relative">
        <button
          onClick={onReverse}
          className="flex items-center justify-center rounded-r-lg border px-2 py-1.5 text-muted-foreground hover:bg-accent transition-colors"
        ...
        <span className="pointer-events-none absolute bottom-full left-1/2 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-gray-900 px-2 py-1 text-[11px] text-white opacity-0 group-hover/rev:opacity-100 transition-opacity z-50">
          Reverse sort order
        </span>
      </div>
      {open && (
        <div role="menu" className="absolute left-0 top-full z-50 mt-1 w-44 rounded-lg border bg-popover shadow-lg py-1">
          {options.map((opt) => (
            <button
              key={opt}
              role="menuitem"
...
const ALL_COLUMNS = [
  ...
  { id: "start_date",       label: "Start date",       locked: false, icon: <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
  ...
  { id: "last_updated",     label: "Last updated",     locked: false, icon: <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
  { id: "goal",             label: "Goal",             locked: false, icon: <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg> },
  { id: "tags",             label: "Tags",             locked: false, icon: <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="4" y1="9" x2="20" y2="9"/><line x1="4" y1="15" x2="20" y2="15"/><line x1="10" y1="3" x2="8" y2="21"/><line x1="16" y1="3" x2="14" y2="21"/></svg> },
  { id: "team",             label: "Team",             locked: false, icon: <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> },
  { id: "contributors",     label: "Contributors",     locked: false, icon: <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg> },
  { id: "related_projects", label: "Related projects", locked: false, icon: <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg> },
...
function ColToggle({ on, onClick }: { on: boolean; onClick: () => void }) {
  return (
    ...
    >
      {/* circle knob */}
      <span className={`absolute top-1 size-5 rounded-full bg-white shadow transition-all ${on ? "left-[calc(100%-1.5rem)]" : "left-1"}`} />
      {/* icon inside knob */}
      {on ? (
        <svg className="absolute left-[calc(100%-1.5rem)] top-1 size-5 p-1 text-green-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
      ) : (
        <svg className="absolute left-1 top-1 size-5 p-1 text-gray-800" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
      )}
    </button>
  )
}

function ColumnsPopover({ open, setOpen, visibleColumns, setVisibleColumns, customColumns, onAddColumn }: {
  ...
  const allCols = [
    ...
    ...customColumns.map((c) => ({
      ...c, locked: false,
      icon: <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="21" x2="9" y2="9"/></svg>,
    })),
  ]
  ...
  return (
    <>
      <div ref={ref} className="relative">
        ...
        {open && (
          <div role="dialog" className="absolute right-0 top-full z-50 mt-1 w-72 rounded-lg border bg-popover shadow-xl">
            {/* Search */}
            <div className="border-b p-2">
              <div className="relative">
                <input
                  ...
                  autoFocus
                  className="w-full rounded-md border bg-background py-1.5 pl-3 pr-8 text-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-ring"
                />
                <svg className="absolute right-2.5 top-1/2 -translate-y-1/2 size-4 text-muted-foreground pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              </div>
            </div>
            {/* Column list */}
            <div className="max-h-72 overflow-y-auto py-1">
              ...
              {filtered.length === 0 && (
                <p className="px-3 py-2 text-sm text-muted-foreground">No columns match.</p>
              )}
            </div>
            {/* Footer */}
      ...
      {createFieldOpen && (
        <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-[12vh] bg-black/50" onClick={() => setCreateFieldOpen(false)}>
          <div data-testid="create-field-modal" className="w-full max-w-[440px] rounded-lg border bg-popover p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button data-testid="cancel-create-field-btn" onClick={() => setCreateFieldOpen(false)} className="rounded-md border px-4 py-2 text-sm hover:bg-accent">Cancel</button>
              <button data-testid="submit-create-field-btn" onClick={handleCreateField} disabled={!newFieldName.trim()} className="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 disabled:opacity-50">Create field</button>
            </div>
          </div>
        </div>
      )}
...
function MoreMenu({ open, setOpen, onExport, onCopyLink, wrapText, onToggleWrapText, isTimeline }: {
  open: boolean; setOpen: (v: boolean) => void
  onExport: () => void; onCopyLink: () => void
  wrapText: boolean; onToggleWrapText: () => void
  isTimeline: boolean
}) {
  const ref = useRef<HTMLDivElement>(null)
  useEffect(() => {
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div role="menu" className="absolute right-0 top-full z-50 mt-1 w-44 rounded-lg border bg-popover shadow-lg py-1">
          <button role="menuitem" onClick={() => { onCopyLink(); setOpen(false) }} className="flex w-full items-center px-3 py-2 text-sm hover:bg-accent text-left">Copy link</button>
          <button role="menuitem" onClick={() => { onExport(); setOpen(false) }} className="flex w-full items-center px-3 py-2 text-sm hover:bg-accent text-left">Export CSV</button>
          {!isTimeline && (
            <div
              role="menuitem"
            ...
            >
              ...
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); onToggleWrapText() }}
              ...
              >
                <span className={`absolute top-0.5 size-4 rounded-full bg-white shadow transition-all ${wrapText ? "left-[calc(100%-1.25rem)]" : "left-0.5"}`} />
                {wrapText
                  ? <svg className="absolute right-1 size-2.5 text-green-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
                  : <svg className="absolute left-1 size-2.5 text-gray-800" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                }
              </button>
            </div>
          )}
    ...
    <div className="flex flex-col gap-2 w-[260px]">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <button onClick={() => setViewDate(addMonths(viewDate, -12))} className="p-1 rounded hover:bg-accent text-muted-foreground text-sm leading-none">«</button>
          <button onClick={() => setViewDate(addMonths(viewDate, -1))} className="p-1 rounded hover:bg-accent text-muted-foreground text-sm leading-none">‹</button>
        </div>
        <span className="text-sm font-medium">{MONTHS_LONG[month]} {year}</span>
        <div className="flex items-center">
          <button onClick={() => setViewDate(addMonths(viewDate, 1))} className="p-1 rounded hover:bg-accent text-muted-foreground text-sm leading-none">›</button>
          <button onClick={() => setViewDate(addMonths(viewDate, 12))} className="p-1 rounded hover:bg-accent text-muted-foreground text-sm leading-none">»</button>
        </div>
      ...
      <div className="grid grid-cols-7 text-center">
        {DOW_SHORT.map(d => <span key={d} className="text-[10px] text-muted-foreground font-medium py-1">{d}</span>)}
        {cells.map((day, i) => {
...
function DateRangePicker({ start, end, onConfirm }: {
  start: Date; end: Date; onConfirm: (s: Date, e: Date) => void
}) {
  ...
  return (
    <div className="absolute left-0 top-full z-50 mt-1 rounded-xl border bg-popover shadow-xl p-5 flex flex-col gap-4">
      <div className="flex gap-8">
        {renderCalendarPanel(startView, setStartView, selStart, setSelStart, "Start")}
      ...
      <div className="flex justify-end">
        <button
          ...
          className="rounded-md bg-blue-600 px-4 py-1.5 text-sm text-white hover:bg-blue-700 transition-colors font-medium"
        >
...
export default function ProjectDirectoryPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Record<string, boolean>>({
    name: true, status: true, start_date: false, target_date: true,
    owner: true, following: true, last_updated: true,
    goal: false, tags: false, team: false, contributors: false,
    related_projects: false, follower_count: false,
  })
  ...
  useEffect(() => {
    ...
    const esc = (e: KeyboardEvent) => { if (e.key === "Escape") setMoreFiltersOpen(false) }
    document.addEventListener("mousedown", h)
  ...
  const [reportingLinePopoverOpen, setReportingLinePopoverOpen] = useState(false)
  const reportingLineRef = useRef<HTMLDivElement>(null)
  useEffect(() => {
    if (!reportingLinePopoverOpen) return
    const h = (e: MouseEvent) => { if (reportingLineRef.current && !reportingLineRef.current.contains(e.target as Node)) setReportingLinePopoverOpen(false) }
    document.addEventListener("mousedown", h)
    return () => document.removeEventListener("mousedown", h)
  ...
  useEffect(() => {
    ...
    const h = (e: MouseEvent) => { if (addTabRef.current && !addTabRef.current.contains(e.target as Node)) setAddTabOpen(false) }
    const esc = (e: KeyboardEvent) => { if (e.key === "Escape") setAddTabOpen(false) }
    document.addEventListener("mousedown", h)
    document.addEventListener("keydown", esc)
    return () => { document.removeEventListener("mousedown", h); document.removeEventListener("keydown", esc) }
  }, [addTabOpen])
  ...
  useEffect(() => {
    ...
    const h = (e: MouseEvent) => { if (tabContextRef.current && !tabContextRef.current.contains(e.target as Node)) setTabContextMenu(null) }
    document.addEventListener("mousedown", h)
    return () => document.removeEventListener("mousedown", h)
  }, [tabContextMenu])
  ...
  useEffect(() => {
    ...
    const esc = (e: KeyboardEvent) => { if (e.key === "Escape") { setCreateViewOpen(false); setViewName(""); setViewNameError(null) } }
    document.addEventListener("keydown", esc)
  ...
  const anyChipActive = activeChips.size > 0 || starredChipActive || reportingLineChipActive
    ...
    <div className="p-6">
        ...
        <div className="flex items-center">
          {([
            ...
            <button
              key={tab.key}
              type="button"
              onClick={() => { setActiveTab(tab.key); setActiveViewId(null) }}
          ...
          {savedViews.map((view) => (
            <div
              ...
              onContextMenu={(e) => { e.preventDefault(); setTabContextMenu({ id: view.id, x: e.clientX, y: e.clientY }) }}
              data-testid={`view-tab-${view.id}`}
            >
              <button
                type="button"
                onClick={() => switchToView(view)}
                className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 transition-colors max-w-[160px] ${
              ...
              <span
                ...
                className="absolute right-1 top-1/2 -translate-y-1/2 flex size-4 items-center justify-center rounded-sm hover:bg-muted text-muted-foreground hover:text-foreground cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity"
              >
                <svg className="size-3 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
              </span>
            </div>
          ))}
          ...
          <div ref={addTabRef} className="relative -mb-px">
            <button
              ...
              onClick={() => { setAddTabOpen((v) => !v); setViewSearch("") }}
              className="flex items-center justify-center px-3 py-2 border-b-2 border-transparent text-muted-foreground hover:text-foreground transition-colors"
              title="Add tab"
            >
              <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            </button>
            {addTabOpen && (
              <div data-testid="add-tab-dropdown" className="absolute left-0 top-full z-50 mt-1 w-72 rounded-lg border bg-popover shadow-xl">
                <div className="p-2 border-b">
                  <div className="relative">
                    <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 size-4 text-muted-foreground pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                    <input
                      type="text"
                      ...
                      autoFocus
                      className="w-full rounded-md border bg-background py-2 pl-9 pr-3 text-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
                <div className="py-1">
                  {/* Saved views list */}
                  {savedViews.filter((v) => v.name.toLowerCase().includes(viewSearch.toLowerCase())).map((view) => (
                    <button
                      key={view.id}
                      type="button"
                    ...
                    >
                      <svg className="size-4 text-muted-foreground shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M3 15h18M9 3v18"/></svg>
                      {view.name}
                    </button>
                  ))}
                  {/* Create view option */}
                  {"Create view".toLowerCase().includes(viewSearch.toLowerCase()) && (
                    <button
                      type="button"
                      data-testid="dropdown-create-view-btn"
                      onClick={() => { setAddTabOpen(false); setCreateViewOpen(true) }}
                      className="flex w-full items-center gap-2.5 px-3 py-2.5 text-sm hover:bg-accent transition-colors text-left"
                    >
                      <svg className="size-4 text-muted-foreground shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M3 15h18M9 3v18"/></svg>
                      Create view
                  ...
                  {viewSearch && !savedViews.some((v) => v.name.toLowerCase().includes(viewSearch.toLowerCase())) && !"Create view".toLowerCase().includes(viewSearch.toLowerCase()) && (
                    <p className="px-3 py-2 text-sm text-muted-foreground">No views found</p>
                  )}
                </div>
              </div>
            )}
          ...
          {(() => {
            ...
            return (
              <>
                <button
                  type="button"
                  onClick={() => { switchToView(view); setTabContextMenu(null) }}
                  className="flex w-full items-center px-3 py-1.5 text-sm hover:bg-accent text-left"
                >
                  Switch to view
                </button>
                <button
                  ...
                  className="flex w-full items-center px-3 py-1.5 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 text-left"
                >
                  Delete view
                </button>
              </>
      ...
      <div ref={filterBarRef} className="mb-4 flex items-center gap-2">
        <div className="flex flex-1 flex-wrap items-center gap-2 min-w-0">
              ...
              <div key={config.key} className="relative">
                {isChip ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-full border border-gray-200 px-3 py-1.5 text-sm text-muted-foreground hover:bg-accent transition-colors dark:border-gray-700"
                  >
                    {config.icon}
                    {config.label}
                  </button>
          ...
          {starredChipActive && (
            ...
            >
              ...
              <span
                ...
                onClick={(e) => { e.stopPropagation(); setStarredChipActive(false) }}
                className="ml-0.5 flex size-4 items-center justify-center rounded-full hover:bg-blue-200 dark:hover:bg-blue-800 text-blue-600 cursor-pointer"
              >
                <svg className="size-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </span>
          ...
          {reportingLineChipActive && (
            <div ref={reportingLineRef} className="relative">
              <button
                ...
                className="flex items-center gap-1.5 rounded-full border border-blue-500 bg-blue-50 px-3 py-1 text-sm text-blue-700 dark:bg-blue-900/20 dark:text-blue-400"
              >
                <svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                <span>Reporting line</span>
                <span
                  role="button"
                  aria-label="Remove Reporting line filter"
                  onClick={(e) => { e.stopPropagation(); setReportingLineChipActive(false); setReportingLinePopoverOpen(false) }}
                  className="ml-0.5 flex size-4 items-center justify-center rounded-full hover:bg-blue-200 dark:hover:bg-blue-800 text-blue-600 cursor-pointer"
                >
                  <svg className="size-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
                </span>
              ...
              {reportingLinePopoverOpen && (
                <div data-testid="reporting-popover" className="absolute left-0 top-full z-50 mt-2 w-72 rounded-lg border bg-popover shadow-xl p-4">
                  <h3 className="text-sm font-semibold mb-1">Stay across the projects your reports work on</h3>
                  <p className="text-xs text-muted-foreground mb-3">Connect your identity provider to get started</p>
                  <a
          ...
          <div ref={moreFiltersRef} className="relative">
            ...
            {moreFiltersOpen && (
              <div data-testid="more-filters-dropdown" className="absolute left-0 top-full z-50 mt-1 w-48 rounded-lg border bg-popover shadow-lg py-1">
                {!starredChipActive && (
                  <button
                    type="button"
                    data-testid="more-filter-starred"
                    onClick={() => { setStarredChipActive(true); setMoreFiltersOpen(false) }}
                    className="flex w-full items-center gap-2.5 px-3 py-2 text-sm hover:bg-accent transition-colors text-left"
                  >
                    <svg className="size-4 text-muted-foreground shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                    Starred
                  </button>
                )}
                {!reportingLineChipActive && (
                  <button
                    ...
                    onClick={() => { setReportingLineChipActive(true); setReportingLinePopoverOpen(true); setMoreFiltersOpen(false) }}
                    className="flex w-full items-center gap-2.5 px-3 py-2 text-sm hover:bg-accent transition-colors text-left"
                  >
                    <svg className="size-4 text-muted-foreground shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                    Reporting line
                  </button>
                )}
                {starredChipActive && reportingLineChipActive && (
                  <p className="px-3 py-2 text-xs text-muted-foreground">No more filters</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="flex shrink-0 items-center gap-2 ml-2">
          <button data-testid="reset-btn" onClick={clearAllFilters} className="text-sm text-muted-foreground hover:text-foreground transition-colors">Reset</button>
          <button
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
              ...
              <div ref={tlRangePickerRef} className="relative">
                ...
                {tlRangePickerOpen && (
                  <DateRangePicker
                    ...
                    onConfirm={(s, e) => { setTlRangeStart(s); setTlRangeEnd(e); setTlRangePickerOpen(false) }}
                  />
                )}
              </div>
          ...
          {viewMode === "list" && (
            <>
              <SortByDropdown sortBy={sortBy} sortAsc={sortAsc} onSelect={(s) => { setSortBy(s); setSortAsc(true) }} onReverse={() => setSortAsc((v) => !v)} />
              <ColumnsPopover open={columnsOpen} setOpen={setColumnsOpen} visibleColumns={visibleColumns} setVisibleColumns={setVisibleColumns} customColumns={customColumns} onAddColumn={(col) => setCustomColumns((prev) => [...prev, col])} />
            </>
          )}
          ...
          {filteredProjects.map((project) => {
            ...
            return (
              <div
                ...
                className="group grid grid-cols-[1fr_110px_110px_90px_100px_110px] gap-4 border-b last:border-b-0 px-4 py-3 items-center hover:bg-accent/50 transition-colors cursor-pointer"
              >
                <Link href={`/projects/${project.key}/board`} onClick={(e) => e.stopPropagation()} className="flex items-center gap-2 hover:text-blue-600">
                  ...
                  <span className={`text-sm ${wrapText ? "whitespace-normal break-words" : "truncate"}`}>{project.name}</span>
                </Link>
                  ...
                  <div className="relative flex items-center" onClick={(e) => e.stopPropagation()}>
                    {isFollowing ? (
                      <div className="group/unfollow relative">
                        <span className="text-xs text-muted-foreground group-hover/unfollow:hidden">Following</span>
                        <div className="relative hidden group-hover/unfollow:block">
                          <button
                            type="button"
                            onClick={() => { toggleFollow(project.id); showToast(`Unfollowed ${project.name}`) }}
                            className="peer rounded px-2 py-0.5 text-xs font-semibold bg-red-500 text-white hover:bg-red-600 transition-colors whitespace-nowrap"
                          >
                            Unfollow
                          </button>
                          <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 w-max max-w-[200px] rounded bg-gray-800 px-2 py-1 text-[11px] text-white opacity-0 peer-hover:opacity-100 transition-opacity z-50 whitespace-nowrap">
                      ...
                      <button
                        type="button"
                        onClick={() => { toggleFollow(project.id); showToast(`Now following ${project.name}`) }}
                        className="rounded px-2 py-0.5 text-xs font-semibold border border-blue-500 text-blue-600 hover:bg-blue-50 transition-colors whitespace-nowrap"
                      >
                        Follow
                      </button>
                    )}
      ...
      {createViewOpen && (
        <div
          ...
          onClick={() => { setCreateViewOpen(false); setViewName(""); setViewNameError(null) }}
        >
          <div className="w-full max-w-[480px] rounded-lg bg-white dark:bg-background shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <h2 className="text-xl font-semibold text-foreground mb-1">Create view</h2>
              ...
              <div>
                ...
                <input
                  ...
                  value={viewName}
                  onChange={(e) => { setViewName(e.target.value); setViewNameError(null) }}
                  onKeyDown={(e) => { if (e.key === "Enter") handleCreateView() }}
                  autoFocus
                  className={`w-full rounded-md border px-3 py-2 text-sm outline-none bg-background transition-colors ${
                    viewNameError || (!viewName.trim() && viewName !== "")
                      ? "border-red-500 focus:ring-2 focus:ring-red-400"
                      : viewName.trim() ? "border-input focus:ring-2 focus:ring-blue-500" : "border-red-500 focus:ring-2 focus:ring-red-400"
                  }`}
                />
                {!viewName.trim() && (
                  <p data-testid="view-name-error" className="mt-1.5 flex items-center gap-1.5 text-sm text-red-600">
                    <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
              ...
              <div className="mt-4 flex items-center gap-2 text-sm text-muted-foreground" data-testid="visibility-note">
                <svg className="size-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
