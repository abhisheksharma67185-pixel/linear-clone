...
export default function ProjectStatusUpdatesPage() {
  ...
  const [newUpdateStatus, setNewUpdateStatus] = useState<"ON TRACK" | "AT RISK" | "OFF TRACK">("ON TRACK")
  const [toast, setToast] = useState<string | null>(null)
  const [statusFilter, setStatusFilter] = useState<string | null>(null)

  const showToast = (msg: string) => { setToast(msg); setTimeout(() => setToast(null), 3000) }

  const weekLabel = weekOffset === 0 ? "Last week" : weekOffset === 1 ? "This week" : weekOffset === -1 ? "Two weeks ago" : `${-weekOffset} weeks ago`
  ...
  const filteredUpdates = statusFilter ? updates.filter((u) => u.status === statusFilter) : updates
  ...
  return (
    <div className="p-6">
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 rounded-lg border bg-background px-4 py-3 shadow-lg text-sm animate-in fade-in slide-in-from-top-2">
          {toast}
        </div>
      )}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="mt-3 flex items-center gap-3">
              <Button className="bg-blue-600 text-white hover:bg-blue-700" size="sm" onClick={() => setCreateOpen(true)}>Create project</Button>
      ...
      <div className="mb-8 grid grid-cols-3 gap-px rounded-lg border overflow-hidden">
        {[
          { count: 1, label: "On track", sub: "-2 from last week", color: "text-green-600", filterKey: "ON TRACK" },
          { count: 1, label: "At risk", sub: "+1 from last week", color: "text-yellow-600", filterKey: "AT RISK" },
          { count: 1, label: "Off track", sub: "No change", color: "text-red-600", filterKey: "OFF TRACK" },
          { count: 0, label: "No update", sub: "No change", color: "text-gray-500", filterKey: null },
          { count: 0, label: "Cancelled", sub: "No change", color: "text-gray-500", filterKey: null },
          { count: 1, label: "Completed", sub: "+1 from last week", color: "text-gray-500", filterKey: null },
        ].map((item) => (
          <button
            key={item.label}
            onClick={() => {
              if (!item.filterKey) return
              setStatusFilter(statusFilter === item.filterKey ? null : item.filterKey)
              showToast(statusFilter === item.filterKey ? `${item.label} filter cleared` : `Filtered to ${item.label}`)
            }}
            className={`flex items-start gap-3 border-b border-r p-4 last:border-r-0 text-left transition-colors ${statusFilter === item.filterKey ? "bg-blue-50 dark:bg-blue-900/20" : "hover:bg-accent"}`}
          >
          ...
          <div className="flex items-start gap-3">
            ...
            >
              <Avatar className="size-8">
                <AvatarFallback className="bg-indigo-600 text-xs text-white">{update.authorInitials}</AvatarFallback>
              </Avatar>
            </button>
            <div className="flex-1">
              <div className="mb-1 flex items-center gap-2">
                ...
                <button
                  onClick={() => { setStatusFilter(update.status); showToast(`Filtered to ${update.status}`) }}
              ...
              {update.bodyLines.map((line, i) => (
                <p key={i} className="mt-1 text-sm text-muted-foreground leading-relaxed">{line}</p>
              ))}
            </div>
          </div>
        </div>
      ...
      {/* Catch up overlay */}
      {catchUpOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setCatchUpOpen(false)}>
          <div className="w-full max-w-[480px] rounded-lg border bg-background p-6 shadow-2xl text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-2 text-sm text-muted-foreground">You&apos;ve reviewed every update from projects you follow for {weekLabel.toLowerCase()}.</p>
            <Button className="mt-4" onClick={() => setCatchUpOpen(false)}>Close</Button>
          </div>
        </div>
      )}
      ...
      {/* Write update modal */}
      {writeOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[12vh] bg-black/50" onClick={() => setWriteOpen(false)}>
          <div className="relative w-full max-w-[520px] rounded-lg border bg-popover p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold mb-3">Write your update</h2>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button onClick={() => setWriteOpen(false)} className="rounded-md border px-4 py-2 text-sm hover:bg-accent">Cancel</button>
              <button onClick={handleWriteUpdate} disabled={!newUpdateBody.trim()} className="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 disabled:opacity-50">Publish update</button>
            </div>
          </div>
        </div>
      )}
