  ...
  return (
    <Link
      ...
      style={{
        ...
        borderRadius: 3, marginBottom: 2, textDecoration: "none",
...
type Prefs = { watching: boolean; reporter: boolean; assignee: boolean; mentions: boolean; own_changes: boolean }
...
export default function NotificationSettingsPage() {
  ...
  const [prefs, setPrefs] = useState<Prefs>({
    watching: true, reporter: true, assignee: true, mentions: true, own_changes: false,
  })
  ...
  return (
    <div style={{ display: "flex", padding: "32px 40px", gap: 0, minHeight: "100vh", background: "white" }}>
      {/* Sidebar */}
      <nav style={{ width: 220, flexShrink: 0 }}>
        ...
        <SidebarLink href="/jira/settings/general" active={pathname === "/jira/settings/general"}>
          General settings
        </SidebarLink>
        <SidebarLink href="/jira/settings/notifications" active={pathname === "/jira/settings/notifications"}>
      ...
      <div style={{ flex: 1, maxWidth: 640 }}>
        <h1 style={{ fontSize: 24, fontWeight: 500, color: "#172B4D", marginBottom: 8, marginTop: 0 }}>
          Notification settings
        </h1>

        {/* Sub-nav tabs */}
        <div style={{ display: "flex", borderBottom: "1px solid #DFE1E6", marginBottom: 32 }}>
          {TABS.map((tab, i) => (
            <button
              key={tab}
              ...
              style={{
                ...
                borderBottom: i === activeTab ? "2px solid #0052CC" : "2px solid transparent",
                padding: "10px 0", marginRight: 24,
                fontSize: 14, fontWeight: i === activeTab ? 500 : 400,
        ...
        {activeTab === 0 && (
          <div>
            {/* Master email toggle */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, paddingBottom: 24, borderBottom: "1px solid #F4F5F7" }}>
              <div>
                <p style={{ fontSize: 14, fontWeight: 500, color: "#172B4D", margin: "0 0 2px 0" }}>
            ...
            {emailEnabled && (
              <div>
                <p style={{ fontSize: 13, fontWeight: 500, color: "#172B4D", marginBottom: 12 }}>
                ...
                {NOTIFICATION_TYPES.map((type) => (
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => setPrefs((p) => ({ ...p, [type.key]: e.target.checked }))}
                      style={{ width: 16, height: 16, accentColor: "#0052CC" }}
                    />
                    <span style={{ fontSize: 14, color: "#172B4D" }}>{type.label}</span>
                  </label>
                ))}
                ...
                <button
                  onClick={handleSave}
                  style={{
                    marginTop: 24, background: "#0052CC", color: "white",
        ...
        {activeTab === 1 && (
          <div>
            <p style={{ fontSize: 14, color: "#6B778C" }}>Alert notification preferences coming soon.</p>
          </div>
        )}
      </div>
    </div>
