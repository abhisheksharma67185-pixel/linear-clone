/**
 * Jira site adapter — wires @simbench/core engine to Jira's store.
...
const jiraAdapter: SiteAdapter = {
  ...
  collections: ["issues", "projects", "sprints", "epics", "users", "boards", "filters"],
  singletons: [],
};
...
registerPredicate("issue_has_fields", (snapshot: GenericSnapshot, check: EvalCheck) => {
  ...
  const issues = snapshot.issues as { id: string; key?: string; summary?: string }[];
  ...
  if (!issue && expected.summary) {
    issue = issues?.find(
      (i) => i.summary?.toLowerCase() === String(expected.summary).toLowerCase(),
    );
  }
...
registerPredicate("issue_exists_with_key", (snapshot: GenericSnapshot, check: EvalCheck) => {
...
registerPredicate("issue_has_status", (snapshot: GenericSnapshot, check: EvalCheck) => {
  const issues = snapshot.issues as { id: string; key?: string; status: string }[];
...
registerPredicate("issue_has_priority", (snapshot: GenericSnapshot, check: EvalCheck) => {
  const issues = snapshot.issues as { id: string; key?: string; priority: string }[];
  const issue = issues?.find((i) => i.id === check.id || i.key === check.id);
  return issue?.priority === check.expected;
});

registerPredicate("issue_has_type", (snapshot: GenericSnapshot, check: EvalCheck) => {
  const issues = snapshot.issues as { id: string; key?: string; type: string }[];
  const issue = issues?.find((i) => i.id === check.id || i.key === check.id);
  return issue?.type === check.expected;
});

registerPredicate("issue_in_sprint", (snapshot: GenericSnapshot, check: EvalCheck) => {
  const issues = snapshot.issues as { id: string; key?: string; sprintId: string | null }[];
  const issue = issues?.find((i) => i.id === check.id || i.key === check.id);
  return issue?.sprintId === check.expected;
});

registerPredicate("issue_in_epic", (snapshot: GenericSnapshot, check: EvalCheck) => {
  const issues = snapshot.issues as { id: string; key?: string; epicId: string | null }[];
  const issue = issues?.find((i) => i.id === check.id || i.key === check.id);
  return issue?.epicId === check.expected;
});

registerPredicate("issue_has_story_points", (snapshot: GenericSnapshot, check: EvalCheck) => {
  const issues = snapshot.issues as { id: string; key?: string; storyPoints: number | null }[];
  const issue = issues?.find((i) => i.id === check.id || i.key === check.id);
...
registerPredicate("issue_has_assignee", (snapshot: GenericSnapshot, check: EvalCheck) => {
  const issues = snapshot.issues as { id: string; key?: string; assigneeId: string | null }[];
  const issue = issues?.find((i) => i.id === check.id || i.key === check.id);
  return issue?.assigneeId === check.expected;
});

registerPredicate("sprint_has_state", (snapshot: GenericSnapshot, check: EvalCheck) => {
...
registerPredicate("sprint_issue_count", (snapshot: GenericSnapshot, check: EvalCheck) => {
...
registerPredicate("project_has_fields", (snapshot: GenericSnapshot, check: EvalCheck) => {
  const expected = check.expected as Record<string, unknown> | undefined;
  if (!expected) return false;
  const projects = snapshot.projects as { id: string; key?: string; name?: string }[];
...
registerPredicate("epic_has_status", (snapshot: GenericSnapshot, check: EvalCheck) => {
