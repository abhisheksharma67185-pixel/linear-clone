...
export default function SummaryPage() {
  ...
  const FILTER_OPTIONS = ["Assignee", "Created", "Due date", "Parent", "Priority", "Updated", "Work type"]
  const filteredFilterOptions = filterSearch ? FILTER_OPTIONS.filter((f) => f.toLowerCase().includes(filterSearch.toLowerCase())) : FILTER_OPTIONS
  ...
  const doneStates = workflow.filter((w) => w.toLowerCase().includes("done") || w.toLowerCase().includes("closed") || w.toLowerCase().includes("deployed"))
  const inProgressStates = workflow.filter((w) => w.toLowerCase().includes("progress") || w.toLowerCase().includes("review") || w.toLowerCase().includes("doing") || w.toLowerCase().includes("development"))

  const completedCount = projectIssues.filter((i) => doneStates.includes(i.status)).length
  const inProgressCount = projectIssues.filter((i) => inProgressStates.includes(i.status)).length
  ...
  const priorityCounts = useMemo(() => {
    ...
    for (const i of projectIssues) counts[i.priority] = (counts[i.priority] ?? 0) + 1
    return counts
  }, [projectIssues])
  ...
  const recentIssues = useMemo(
    () => [...projectIssues].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, 5),
    [projectIssues]
  )
  ...
  function donutPath(startAngle: number, endAngle: number, r: number, cx: number, cy: number) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 pt-4 pb-1">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            {/* Team members */}
            <button className="flex size-8 items-center justify-center rounded border hover:bg-accent transition-colors" title="Team members">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger render={<button aria-label="More actions" className="flex size-8 items-center justify-center rounded border hover:bg-accent transition-colors" />}>
                <svg className="size-4 text-muted-foreground" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="1.5" /><circle cx="12" cy="12" r="1.5" /><circle cx="12" cy="19" r="1.5" /></svg>
              </DropdownMenuTrigger>
          ...
          <div className="flex items-center gap-1">
            {/* Share */}
            <button className="flex size-8 items-center justify-center rounded border hover:bg-accent transition-colors" title="Share">
            ...
            {/* Automation */}
            <button className="flex size-8 items-center justify-center rounded border hover:bg-accent transition-colors" title="Automation">
            ...
            {/* Chat/comment */}
            <button className="flex size-8 items-center justify-center rounded border hover:bg-accent transition-colors" title="Chat">
            ...
            {/* Full screen */}
            <button className="flex size-8 items-center justify-center rounded border hover:bg-accent transition-colors" title="Full screen">
      ...
      <div className="px-6 border-b">
        <div className="flex items-center gap-0">
          {[
            { label: "Summary", href: `/projects/${project.key}/summary`, active: true, icon: <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /></svg> },
          ...
          <div className="relative ml-1">
            <button
              ...
              className={`flex items-center justify-center size-8 rounded transition-colors ${viewsOpen ? "bg-blue-100 text-blue-600 dark:bg-blue-900/40" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}
            >
            ...
            {viewsOpen && (
              <>
                ...
                <div className="absolute left-0 top-full z-50 mt-2 flex rounded-lg border bg-popover shadow-xl">
                  {/* Views list */}
                  <div className="w-[200px] py-2 border-r">
                    <p className="px-3 py-1 text-[11px] font-semibold text-muted-foreground">Views</p>
                    {VIEWS.map((view) => (
                      <button
                        key={view.name}
                      ...
                      >
                        <svg className="size-4 text-muted-foreground shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          {view.icon === "archive" && <><path d="M21 8v13H3V8" /><path d="M1 3h22v5H1z" /><path d="M10 12h4" /></>}
                          {view.icon === "calendar" && <><rect x="3" y="4" width="18" height="18" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" /></>}
                          {view.icon === "deploy" && <><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></>}
                          {view.icon === "dev" && <><polyline points="16 18 22 12 16 6" /><polyline points="8 6 2 12 8 18" /></>}
                          {view.icon === "form" && <><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><path d="M14 2v6h6M9 13h6M9 17h3" /></>}
                          {view.icon === "goal" && <><circle cx="12" cy="12" r="10" /><circle cx="12" cy="12" r="6" /><circle cx="12" cy="12" r="2" /></>}
                          {view.icon === "list" && <><path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01" /></>}
                          {view.icon === "page" && <><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><path d="M14 2v6h6M16 13H8M16 17H8M10 9H8" /></>}
                          {view.icon === "release" && <><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><path d="M12 15V3" /></>}
                          {view.icon === "report" && <><path d="M22 12h-4l-3 9L9 3l-3 9H2" /></>}
                          {view.icon === "security" && <><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></>}
                          {view.icon === "shortcut" && <><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" /></>}
                          {view.icon === "timeline" && <><path d="M5 12h14M12 5l7 7-7 7" /></>}
                        </svg>
                        {view.name}
                      </button>
                    ))}
                  ...
                  <div className="w-[220px] p-4">
                    <div className="rounded-lg bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 p-4 mb-3 flex items-center justify-center h-[100px]">
                      <svg className="size-10 text-blue-400/50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><path d="M14 2v6h6" /></svg>
                    </div>
                    <p className="text-sm font-medium mb-1">Archived work items</p>
                    <p className="text-xs text-muted-foreground mb-3">View all the work items archived in your space items.</p>
                    <button className="text-xs text-blue-600 font-medium hover:underline">Add to navigation</button>
                  </div>
                </div>
              </>
            )}
      ...
      <div className="px-6 py-2.5 border-b">
        <div className="relative inline-block">
          <Button
            variant="ghost"
            size="sm"
            className={`h-7 text-xs gap-1.5 ${filterOpen ? "bg-accent" : ""}`}
            onClick={() => { setFilterOpen(!filterOpen); setFilterSearch("") }}
          >
            <svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3" /></svg>
            Filter
          </Button>
          {filterOpen && (
            <>
              <div className="fixed inset-0 z-40" onClick={() => setFilterOpen(false)} />
              <div className="absolute left-0 top-full z-50 mt-1 w-[220px] rounded-lg border bg-popover shadow-lg">
                {/* Search */}
                ...
                <div className="py-1 max-h-[240px] overflow-y-auto">
                  {filteredFilterOptions.map((f) => (
                    <button
                      ...
                      className="flex w-full items-center gap-2.5 px-3 py-2 text-sm hover:bg-accent transition-colors text-left"
                    >
                      <span className={`flex size-4 items-center justify-center rounded border text-[10px] transition-colors ${activeFilters.includes(f) ? "bg-blue-600 border-blue-600 text-white" : "border-gray-300 dark:border-gray-600"}`}>
                        {activeFilters.includes(f) && <svg className="size-2.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><polyline points="20 6 9 17 4 12" /></svg>}
                      </span>
                      {f}
                    </button>
                  ))}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            { label: "completed", sublabel: "in the last 7 days", value: completedCount, color: "text-green-600" },
            { label: "updated", sublabel: "in the last 7 days", value: projectIssues.length, color: "text-blue-600" },
            { label: "created", sublabel: "in the last 7 days", value: projectIssues.length, color: "text-teal-600" },
            { label: "due soon", sublabel: "in the next 7 days", value: Math.min(2, projectIssues.length), color: "text-yellow-600" },
          ].map((stat) => (
        ...
        <div className="grid grid-cols-2 gap-6 mb-8">
          ...
          <div className="rounded-lg border p-5">
            <h3 className="text-sm font-semibold mb-1">Status overview</h3>
            <p className="text-xs text-muted-foreground mb-4">
              Get a snapshot of the status of your work items. <Link href={`/projects/${project.key}/backlog`} className="text-blue-600 hover:underline">View all work items</Link>
            </p>
            <div className="flex items-center gap-6">
              ...
              <div className="relative">
                <svg width="140" height="140" viewBox="0 0 140 140">
                  {completedCount > 0 && (
                    <path d={donutPath(0, doneAngle, 55, 70, 70)} fill="none" stroke="#22c55e" strokeWidth="20" strokeLinecap="round" />
                  )}
                  {inProgressCount > 0 && (
                    <path d={donutPath(doneAngle, doneAngle + ipAngle, 55, 70, 70)} fill="none" stroke="#3b82f6" strokeWidth="20" strokeLinecap="round" />
                  )}
                  {todoCount > 0 && (
                    <path d={donutPath(doneAngle + ipAngle, doneAngle + ipAngle + todoAngle, 55, 70, 70)} fill="none" stroke="#e5e7eb" strokeWidth="20" strokeLinecap="round" />
                  )}
                  {projectIssues.length === 0 && (
                    <circle cx="70" cy="70" r="55" fill="none" stroke="#e5e7eb" strokeWidth="20" />
                  )}
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-2xl font-bold">{projectIssues.length}</span>
                  <span className="text-[10px] text-muted-foreground">Total Work Item{projectIssues.length !== 1 ? "s" : ""}</span>
                </div>
              </div>
              {/* Legend */}
          ...
          <div className="rounded-lg border p-5">
            ...
            <div className="space-y-3">
              {recentIssues.map((issue) => {
                ...
                const timeDiff = Date.now() - new Date(issue.updatedAt).getTime()
                const hours = Math.floor(timeDiff / 3600000)
                const timeAgo = hours < 1 ? "just now" : hours < 24 ? `about ${hours} hour${hours > 1 ? "s" : ""} ago` : `${Math.floor(hours / 24)} day${Math.floor(hours / 24) > 1 ? "s" : ""} ago`
                return (
                  <div key={issue.id} className="flex items-start gap-2.5">
                    <Avatar className="size-6 mt-0.5 shrink-0">
                      ...
                      <AvatarFallback className="text-[8px] bg-blue-500 text-white">{(reporter?.name ?? "U").charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div className="min-w-0">
                      <p className="text-xs">
                        <span className="font-medium">{reporter?.name ?? "Someone"}</span>
                        {" created "}
                        <Link href={`/issue/${issue.key}`} className="font-medium text-blue-600 hover:underline">{issue.key}</Link>
                      </p>
        ...
        <div className="grid grid-cols-2 gap-6 mb-8">
          ...
          <div className="rounded-lg border p-5">
            ...
            <p className="text-xs text-muted-foreground mb-4">
              Get a holistic view of how work is being prioritized. <Link href="https://support.atlassian.com/jira-software-cloud/docs/what-is-a-jira-software-project/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">How to manage priorities for spaces</Link>
            </p>
            <div className="flex items-end gap-3" style={{ height: 120 }}>
              {["highest", "high", "medium", "low", "lowest"].map((p) => {
                const barHeight = Math.max(Math.round((priorityCounts[p] / maxPriority) * 100), priorityCounts[p] > 0 ? 8 : 4)
                return (
                  <div key={p} className="flex-1 flex flex-col items-center justify-end h-full">
                    <span className="text-[10px] text-muted-foreground mb-1 tabular-nums">{priorityCounts[p]}</span>
                    <div className="w-full bg-gray-300 dark:bg-gray-600 rounded-t" style={{ height: `${barHeight}px` }} />
                    <span className="text-[10px] text-muted-foreground mt-2 capitalize">{p}</span>
                  </div>
                )
              })}
            </div>
          ...
          {/* Types of work */}
          <div className="rounded-lg border p-5">
            <h3 className="text-sm font-semibold mb-1">Types of work</h3>
            <p className="text-xs text-muted-foreground mb-4">
              Get a breakdown of work items by their types. <Link href={`/projects/${project.key}/backlog`} className="text-blue-600 hover:underline">View all items</Link>
            </p>
            <div className="space-y-2.5">
              <div className="grid grid-cols-[auto_1fr_auto] gap-x-3 text-[11px] font-semibold text-muted-foreground mb-1">
                <span>Type</span><span /><span>Distribution</span>
              </div>
              {Object.entries(typeCounts).map(([type, count]) => {
                const pct = Math.round((count / (projectIssues.length || 1)) * 100)
                const colors: Record<string, string> = { task: "bg-blue-500", story: "bg-green-500", bug: "bg-red-500", subtask: "bg-cyan-500", epic: "bg-purple-500" }
                const icons: Record<string, string> = { task: "T", story: "S", bug: "B", subtask: "ST", epic: "E" }
                return (
                  <div key={type} className="grid grid-cols-[auto_1fr_auto] gap-x-3 items-center">
                    <div className="flex items-center gap-2">
                      <span className={`flex size-4 items-center justify-center rounded-sm text-[8px] text-white font-bold ${colors[type] ?? "bg-gray-500"}`}>{icons[type] ?? "?"}</span>
        ...
        <div className="grid grid-cols-2 gap-6">
          {/* Team workload */}
          <div className="rounded-lg border p-5">
            <h3 className="text-sm font-semibold mb-1">Team workload</h3>
            <p className="text-xs text-muted-foreground mb-4">
              Monitor the capacity of your team. <Link href={`/projects/${project.key}/board`} className="text-blue-600 hover:underline">Reassign work items to get the right balance</Link>
            </p>
            <div className="space-y-2.5">
              <div className="grid grid-cols-[120px_1fr] gap-x-3 text-[11px] font-semibold text-muted-foreground mb-1">
                <span>Assignee</span><span>Work distribution</span>
              </div>
              {workload.map(({ user, id, count }) => {
                const pct = Math.round((count / maxWorkload) * 100)
                return (
                  <div key={id} className="grid grid-cols-[120px_1fr] gap-x-3 items-center">
                    <span className="text-xs truncate">{user?.name ?? "Unassigned"}</span>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-muted-foreground tabular-nums w-6">{count}</span>
                    </div>
                  </div>
                )
              })}
          ...
          {/* Epic progress */}
          <div className="rounded-lg border p-5">
            <h3 className="text-sm font-semibold mb-1">Epic progress</h3>
            {projectEpics.length > 0 ? (
              <div className="space-y-3 mt-3">
                {projectEpics.map((epic) => {
                  const epicIssues = projectIssues.filter((i) => i.epicId === epic.id)
                  const epicDone = epicIssues.filter((i) => doneStates.includes(i.status)).length
                  const pct = epicIssues.length ? Math.round((epicDone / epicIssues.length) * 100) : 0
                  return (
                    <div key={epic.id}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-medium">{epic.name}</span>
                        <span className="text-[10px] text-muted-foreground">{epicDone}/{epicIssues.length}</span>
                      </div>
                      <div className="h-2 rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden">
              ...
              <div className="flex flex-col items-center justify-center py-8">
                <svg className="size-12 text-muted-foreground/30 mb-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" /></svg>
                <p className="text-sm font-medium mb-1">Epic progress</p>
                <p className="text-xs text-muted-foreground text-center">
                  Use epics to track larger initiatives in your space. <Link href="https://support.atlassian.com/jira-software-cloud/docs/what-is-an-epic/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">What is an epic?</Link>
                </p>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setAddPeopleOpen(false)}>
          ...
          <div className="relative z-10 w-full max-w-[400px] rounded-lg border bg-popover shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <input placeholder="Enter email addresses" className="w-full rounded-md border bg-background px-3 py-2 text-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-ring/30" autoFocus />
            </div>
          ...
          <div className="relative z-10 w-full max-w-[440px] rounded-lg border bg-popover shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-5 py-4">
              ...
              <div className="rounded-lg border border-red-200 bg-red-50 dark:border-red-900/50 dark:bg-red-950/20 p-3">
                <p className="text-xs text-red-700 dark:text-red-300 font-medium">Warning: This will permanently delete all work items in this project.</p>
              </div>
            </div>
            <div className="flex justify-end gap-2 px-5 py-3 border-t">
              <Button variant="ghost" size="sm" onClick={() => setDeleteConfirmOpen(false)}>Cancel</Button>
              <Button size="sm" className="bg-red-600 text-white hover:bg-red-700" onClick={() => { setDeleteConfirmOpen(false); window.location.href = "/projects" }}>Delete</Button>
            </div>
          </div>
        </div>
      )}
