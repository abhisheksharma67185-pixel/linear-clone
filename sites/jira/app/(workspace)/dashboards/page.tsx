...
const initialDashboards: Dashboard[] = [
  {
    ...
    description: "Your personal default dashboard showing assigned issues and activity.",
  ...
  },
  {
    ...
    description: "Track active sprint progress and burndown across all projects.",
    owner: "Abhishek Sharma",
    viewers: "My organization",
...
export default function DashboardsPage() {
  ...
  const [viewerFilter, setViewerFilter] = useState<"all" | "private" | "org">("all")
  ...
  const handleEditSave = () => {
    ...
    setDashboards((prev) =>
      prev.map((d) =>
        d.id === editDash.id ? { ...d, name: editName.trim(), description: editDescription.trim() } : d
      )
    )
    setEditOpen(false)
    setEditDash(null)
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        {/* Owner filter */}
        <DropdownMenu>
          <DropdownMenuTrigger className="flex items-center gap-1.5 rounded-md border px-3 py-2 text-sm hover:bg-accent transition-colors">
          ...
          <DropdownMenuContent align="start">
            <DropdownMenuItem onClick={() => setOwnerFilter("all")}>
              All owners
              {ownerFilter === "all" && <svg className="size-4 ml-auto" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12" /></svg>}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setOwnerFilter("me")}>
              Owned by me
              {ownerFilter === "me" && <svg className="size-4 ml-auto" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12" /></svg>}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="flex items-center gap-1.5 rounded-md border px-3 py-2 text-sm hover:bg-accent transition-colors">
              Viewers{viewerFilter !== "all" ? `: ${viewerFilter === "private" ? "Private" : "Organization"}` : ""}
              <svg className="size-3.5 text-muted-foreground" viewBox="0 0 16 16" fill="currentColor">
                <path d="M4 6l4 4 4-4" />
              </svg>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start">
            <DropdownMenuItem onClick={() => setViewerFilter("all")}>
              All
              {viewerFilter === "all" && <svg className="size-4 ml-auto" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12" /></svg>}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setViewerFilter("org")}>
              My organization
              {viewerFilter === "org" && <svg className="size-4 ml-auto" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12" /></svg>}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setViewerFilter("private")}>
              Private
              {viewerFilter === "private" && <svg className="size-4 ml-auto" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12" /></svg>}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        {(ownerFilter !== "all" || viewerFilter !== "all") && (
          <button
            onClick={() => { setOwnerFilter("all"); setViewerFilter("all") }}
      ...
      {/* Active filter chips */}
      {starredCount > 0 && (
        <p className="mb-3 text-xs text-muted-foreground">{starredCount} starred dashboard{starredCount !== 1 ? "s" : ""}</p>
      )}
            ...
            {filtered.map((dashboard) => (
              <TableRow key={dashboard.id}>
                <TableCell>
                  ...
                  >
                    <svg className="size-4" viewBox="0 0 24 24" fill={dashboard.starred ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
                      <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                    </svg>
                  </button>
                </TableCell>
                <TableCell>
                  <Link href={`/dashboards/${dashboard.id}`} className="font-medium text-blue-600 hover:underline">
                    {dashboard.name}
                  </Link>
                </TableCell>
                ...
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => router.push(`/dashboards/${dashboard.id}`)}>
                        <svg className="size-4 mr-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>
                        View
                      </DropdownMenuItem>
                      ...
                      <DropdownMenuItem onClick={() => handleCopy(dashboard)}>
                        <svg className="size-4 mr-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></svg>
                        Copy
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        onClick={() => { setDeleteDash(dashboard); setDeleteOpen(true) }}
        ...
        <DialogContent className="sm:max-w-[480px]">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium mb-1.5 block">Viewers</label>
                <select
                  value={newViewers}
              ...
              <div>
                <label className="text-sm font-medium mb-1.5 block">Editors</label>
                <select
                  value={newEditors}
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="sm:max-w-[400px]">
          ...
          <p className="text-sm text-muted-foreground py-2">
            Are you sure you want to delete <span className="font-medium text-foreground">{deleteDash?.name}</span>? This action cannot be undone.
          </p>
          <DialogFooter>
