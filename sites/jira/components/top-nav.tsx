...
function SearchBar({ isBlue = true }: { isBlue?: boolean }) {
  ...
  const [activeTab, setActiveTab] = useState<"jira" | "home">("jira")
  ...
  const BoardIconSm = ({ color = "bg-violet-600" }: { color?: string }) => (
  ...
  return (
    <div ref={wrapperRef} style={{ position: "relative", width: 600, maxWidth: "100%" }}>
      {/* Trigger input */}
              ...
              {!query.trim() ? (
                <>
                  ...
                  {recentIssues.length > 0 ? recentIssues.map((issue) => (
                    <button key={issue.id} onClick={() => go(`/issue/${issue.key}`)} className="flex items-center gap-3 px-4 py-2 w-full text-left hover:bg-accent transition-colors group">
                  ...
                  {recentProjects.length > 0 ? recentProjects.map((proj) => (
                    ...
                    <>
                      ...
                      <button onClick={() => go("/projects")} className="flex items-center gap-3 px-4 py-2 w-full text-left hover:bg-accent transition-colors">
                        <ProjectIcon color="bg-violet-600" />
                      ...
                      {results.issues.map((issue) => (
                        <button key={issue.id} onClick={() => go(`/issue/${issue.key}`)} className="flex items-center gap-3 px-4 py-2.5 w-full text-left hover:bg-accent transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            <span className="text-sm truncate block">{issue.key} {issue.summary}</span>
                          </div>
                        </button>
                      ))}
                    </div>
...
function CreateIssueDialog({ isBlue = true }: { isBlue?: boolean }) {
  ...
  return (
    <>
      {/* BUG 3: plain "+ Create" button — no chevron, no dropdown */}
      <button
        onClick={() => setOpen(true)}
        style={{ background: "#0052CC", color: "white", border: "none", borderRadius: 3, padding: "0 12px", height: 32, fontSize: 14, fontWeight: 500, cursor: "pointer", whiteSpace: "nowrap" }}
