...
function HelpArticlesIcon() {
  return (
    <svg className="size-5 shrink-0 text-[#626f86]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75">
...
function KeyboardIcon() {
  return (
    <svg className="size-5 shrink-0 text-[#626f86]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75">
      ...
      <line x1="6" y1="8" x2="6.01" y2="8" strokeWidth="2.5" strokeLinecap="round" />
      <line x1="10" y1="8" x2="10.01" y2="8" strokeWidth="2.5" strokeLinecap="round" />
      <line x1="14" y1="8" x2="14.01" y2="8" strokeWidth="2.5" strokeLinecap="round" />
      <line x1="18" y1="8" x2="18.01" y2="8" strokeWidth="2.5" strokeLinecap="round" />
...
function ExternalLinkIcon() {
  return (
    <svg className="size-[15px] shrink-0 text-[#626f86]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
function FeedbackDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) { setSubmitted(false); setRating(null); setComment(""); onClose() } }}>
      <DialogContent className="sm:max-w-[440px]">
        ...
        {submitted ? (
          ...
          <div className="space-y-4 py-2">
            <div>
              <p className="mb-3 text-[13px] text-[#172b4d] dark:text-foreground font-medium">
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className="w-full rounded-[3px] border border-[#dfe1e6] bg-background px-3 py-2 text-[13px] text-foreground outline-none placeholder:text-[#626f86] focus:border-[#0052cc] focus:ring-1 focus:ring-[#0052cc] resize-none"
              />
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              <Button
                variant="outline"
                size="sm"
                onClick={() => { setRating(null); setComment(""); onClose() }}
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          {/* ── Header ── */}
          <div className="shrink-0 border-b border-[#dfe1e6] dark:border-border bg-[#f4f5f7] dark:bg-muted">
            <div className="relative flex h-12 items-center justify-center px-4">
              ...
              {view === "shortcuts" && (
                <button
                  ...
                  className="absolute left-3 flex items-center gap-1.5 rounded px-2 py-1 text-[13px] text-[#172b4d] dark:text-foreground hover:bg-[#dfe1e6] dark:hover:bg-border transition-colors"
                >
              ...
              <button
                ...
                className="absolute right-3 rounded p-1 text-[#626f86] dark:text-muted-foreground hover:bg-[#dfe1e6] dark:hover:bg-border transition-colors"
              >
          ...
          {view === "main" ? (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
              <h2 className="mb-5 text-[18px] font-bold text-[#172b4d] dark:text-foreground leading-tight">
                Keyboard shortcuts
              </h2>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-start gap-3 rounded-[3px] border border-[#dfe1e6] dark:border-border bg-[#f4f5f7] dark:bg-muted/30 px-4 py-3">
                  <p className="flex-1 text-[13px] text-[#172b4d] dark:text-foreground leading-snug">
                    Add{" "}
                    <kbd className="inline-flex items-center justify-center rounded border border-[#dfe1e6] bg-white dark:bg-background px-1.5 py-0.5 font-mono text-[11px] text-[#626f86]">
                      opt
                    </kbd>{" "}
                    as a modifier key to all shortcuts for improved accessibility
                  </p>
                  {/* Toggle */}
                  <button
                    type="button"
                    ...
                    className={`relative mt-0.5 inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0052cc] ${
                  ...
                  <div key={section.title}>
                    ...
                    <div>
                      {section.shortcuts.map((s) => (
                        <div
                          key={s.description}
                          className="flex items-center justify-between py-2 border-b border-[#f4f5f7] dark:border-border last:border-0"
                        >
                          ...
                          <div className="flex items-center gap-1 shrink-0 ml-4">
                            {optModifier && (
                              <kbd className="inline-flex min-w-[22px] items-center justify-center rounded border border-[#dfe1e6] dark:border-border bg-[#f4f5f7] dark:bg-muted px-1.5 py-0.5 font-mono text-[11px] text-[#626f86] dark:text-muted-foreground">
                                opt
                              </kbd>
                            )}
                            {s.keys.map((k, i) => (
                              <kbd
                                key={i}
                                className="inline-flex min-w-[22px] items-center justify-center rounded border border-[#dfe1e6] dark:border-border bg-[#f4f5f7] dark:bg-muted px-1.5 py-0.5 font-mono text-[11px] text-[#626f86] dark:text-muted-foreground"
                              >
                                {k}
                              </kbd>
                            ))}
          ...
          <div className="shrink-0 border-t border-[#dfe1e6] dark:border-border px-5 py-3">
            <div className="flex items-center gap-3 text-[11px] text-[#626f86] dark:text-muted-foreground">
              ...
              <span>·</span>
              <a href="/privacy/notice-of-collection" className="hover:underline">Notice of Collection</a>
              <span>·</span>
