...
export function completeCycle(id: string): Result<Cycle> {
  ...
  if (cycle.state === "upcoming") {
    return { success: false, error: "Cannot complete a cycle that has not started" };
  }
...
export function moveIssueToCycle(issueId: string, cycleId: string | null): Result<Issue> {
...
export function createLabel(fields: {
  ...
  if (_labels.some((l) => l.teamId === teamId && l.name === fields.name!.trim())) {
    return { success: false, error: `Label already exists in team: ${fields.name}` };
  }
...
export function updateLabel(
  ...
  if (fields.name !== undefined) {
    ...
    if (_labels.some((l) => l.id !== id && l.teamId === label.teamId && l.name === fields.name!.trim())) {
      return { success: false, error: `Label already exists in team: ${fields.name}` };
    }
...
export function createTeam(fields: {
  ...
  if (_teams.some((t) => t.key === normalizedKey)) {
    return { success: false, error: `Team key already exists: ${normalizedKey}` };
  }
