...
export function CreateProjectDialog({ open, onOpenChange }: CreateProjectDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        {/* Header breadcrumb */}
        <div className="flex items-center gap-1.5 border-b px-5 py-3 text-xs text-muted-foreground">
          ...
          <span className="font-medium text-foreground">ABH</span>
        ...
        <div className="flex flex-1 flex-col overflow-auto px-8 py-6">
          {/* Project icon */}
          <button type="button" className="mb-4 flex size-10 items-center justify-center rounded-lg border border-dashed border-muted-foreground/30 text-muted-foreground hover:border-muted-foreground/60 hover:text-foreground">
          ...
          <input
            ...
            className="mb-2 bg-transparent text-2xl font-medium text-foreground placeholder:text-muted-foreground/40 focus:outline-none"
          />
          ...
          <input
            ...
            className="mb-5 bg-transparent text-sm text-muted-foreground placeholder:text-muted-foreground/50 focus:outline-none"
          />
              ...
              >
                {pill.label === "Backlog" && (
                  <svg viewBox="0 0 12 12" className="size-3" fill="none">
                    <circle cx="6" cy="6" r="4.5" stroke="currentColor" strokeWidth="1.2" strokeDasharray="2.5 1.5" />
                  </svg>
                )}
                ...
                {pill.label === "Lead" && (
                  <svg viewBox="0 0 12 12" className="size-3" fill="none">
                    ...
                    <path d="M2 10.5c0-2 1.5-3.5 4-3.5s4 1.5 4 3.5" stroke="currentColor" strokeWidth="1.1" />
                  </svg>
                )}
                {pill.label === "Members" && (
                  <svg viewBox="0 0 12 12" className="size-3" fill="none">
                    <circle cx="4.5" cy="4.5" r="1.8" stroke="currentColor" strokeWidth="1.1" />
                    <path d="M1 10c0-1.8 1.4-3 3.5-3s3.5 1.2 3.5 3" stroke="currentColor" strokeWidth="1.1" />
                    <circle cx="8.5" cy="4.5" r="1.5" stroke="currentColor" strokeWidth="1.1" />
                    <path d="M8.5 7.5c1.5 0 2.5 1 2.5 2.5" stroke="currentColor" strokeWidth="1.1" />
                  </svg>
                )}
                {pill.label === "Start" && (
                  <svg viewBox="0 0 12 12" className="size-3" fill="none">
                    <rect x="1" y="1.5" width="10" height="9" rx="1.2" stroke="currentColor" strokeWidth="1.1" />
                    <path d="M4 1v1.5M8 1v1.5M1 5h10" stroke="currentColor" strokeWidth="1.1" />
                  </svg>
                )}
                {pill.label === "Target" && (
                  <svg viewBox="0 0 12 12" className="size-3" fill="none">
                ...
                {pill.label === "Labels" && (
                  <svg viewBox="0 0 12 12" className="size-3" fill="none">
                    <path d="M6.5 1.5H9.5C10.1 1.5 10.5 1.9 10.5 2.5V5.5L6 10C5.6 10.4 5 10.4 4.6 10L2 7.4C1.6 7 1.6 6.4 2 6L6.5 1.5Z" stroke="currentColor" strokeWidth="1.1" />
                    <circle cx="8" cy="4" r="1" fill="currentColor" />
                  </svg>
                )}
                {pill.label === "Dependencies" && (
                  <svg viewBox="0 0 12 12" className="size-3" fill="none">
                    <path d="M2 6h4M8 3l2 3-2 3" stroke="currentColor" strokeWidth="1.1" strokeLinecap="round" />
                    <circle cx="2" cy="6" r="1.2" stroke="currentColor" strokeWidth="1.1" />
                  </svg>
                )}
        ...
        <div className="flex items-center justify-between border-t px-8 py-3">
          ...
          <button type="button" className="flex size-5 items-center justify-center rounded text-muted-foreground hover:bg-muted hover:text-foreground">
        ...
        <div className="flex items-center justify-end gap-2 border-t px-5 py-3">
          <button
            type="button"
            onClick={handleClose}
            className="rounded-full px-4 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground"
          >
            Cancel
          </button>
          <button
