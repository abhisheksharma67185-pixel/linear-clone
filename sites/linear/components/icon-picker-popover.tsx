...
import {
  Search01Icon,
  // General & UI
  CubeIcon, Home01Icon, StarIcon, FlashIcon, Target01Icon, Chart01Icon,
  CheckListIcon, CheckmarkSquare01Icon, CheckmarkSquare02Icon,
  FilterIcon, Settings02Icon, BulbIcon, IdeaIcon,
  DiamondIcon, CrownIcon, GiftIcon, BadgeIcon, AuctionIcon,
  MedalFirstPlaceIcon, MedalSecondPlaceIcon, MedalThirdPlaceIcon,
  Award01Icon, Award02Icon, RocketIcon, Shield01Icon, LockIcon,
  HeartAddIcon, FireIcon, FlowerIcon, SnowIcon, ZapIcon, PowerIcon,
  // People
  UserIcon, UserGroupIcon,
  // Communication
  Message01Icon, ChatIcon, ChatBotIcon, ChatAddIcon, ChatEditIcon,
  ChatFeedbackIcon, ChatSearchIcon, ChatSparkIcon, ChatTranslateIcon,
  ChatUserIcon, MailOpenIcon, MailsIcon, MailboxIcon,
  PhoneCheckIcon, VoiceIcon,
  // Files
  File01Icon, FileEditIcon, FileCodeIcon, FileSearchIcon, FileStarIcon,
  FileSyncIcon, FileUploadIcon, FileDownloadIcon, FileHeartIcon,
  FileImageIcon, FileMusicIcon, FileVideoIcon, FileSecurityIcon,
  FileLinkIcon, FileClockIcon,
  // Folders
  Folder01Icon, FolderOpenIcon, FolderAddIcon, FolderCheckIcon,
  FolderCodeIcon, FolderCloudIcon, FolderGitIcon, FolderHeartIcon,
  FolderSearchIcon, FolderTreeIcon, FolderVideoIcon,
  // Charts & Analytics
  ChartIcon, ChartUpIcon, ChartDownIcon, ChartAreaIcon, ChartGanttIcon,
  ChartRadarIcon, ChartRingIcon, ChartScatterIcon, ChartBarBigIcon,
  ChartColumnIcon, ChartHistogramIcon, ChartIncreaseIcon,
  ChartDecreaseIcon, ChartEvaluationIcon, AnalyticsUpIcon, AnalyticsDownIcon,
  // Code & Tech
  CodeIcon, CodeSquareIcon, CodeCircleIcon, CodeSimpleIcon,
  TerminalIcon, DatabaseIcon, DatabaseAddIcon, DatabaseSyncIcon,
  DatabaseSettingIcon, DatabaseLightningIcon, RouterIcon,
  LaptopIcon, ChipIcon, CpuIcon, GpuIcon, HardDriveIcon,
  UsbIcon, BluetoothIcon, PrinterIcon, DroneIcon, MicrochipIcon,
  MicroscopeIcon, AtomicPowerIcon,
  // Cloud & Computer
  CloudIcon, CloudServerIcon, CloudUploadIcon, CloudDownloadIcon,
  CloudAlertIcon, CloudBigRainIcon, CloudSnowIcon,
  ComputerIcon, ComputerCheckIcon, ComputerCloudIcon,
  ComputerActivityIcon, ComputerUserIcon,
  // Books
  BookOpenCheckIcon, BookEditIcon, BookSearchIcon,
  BookHeartIcon, BookUserIcon, BookUploadIcon,
  // Nature & Earth
  Tree01Icon, EarthIcon, SolarEnergyIcon, SolarPowerIcon,
  // Maps & Navigation
  MapsIcon, CompassIcon, MapPinIcon, NavigationIcon,
  // Media & Audio
  AlbumIcon, RadioIcon, PodcastIcon, NewsIcon, RssIcon,
  // Objects & Tools
  InboxIcon, InboxCheckIcon, Calendar01Icon, Activity03Icon,
  Tag01Icon, Link01Icon, GlobeIcon, MoonIcon, Sun01Icon, TruckIcon,
  RefreshIcon, SecurityIcon, NoteIcon, TranslateIcon,
  BrainIcon, BrushIcon, PaintBucketIcon, PencilIcon,
  ToolsIcon, ToolboxIcon, PackageIcon, PackageDeliveredIcon,
  ZoomIcon, WorkIcon, GameIcon, PuzzleIcon,
  StackStarIcon, WifiCircleIcon, SdCardIcon,
  DollarCircleIcon, BankIcon,
...
export function IconPickerPopover({
  ...
  const picker = (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        {selectedIcon ? (
          <HugeiconsIcon icon={selectedIcon} className={`size-4 shrink-0 ${color.text}`} />
        ) : (
          trigger
        )}
      </PopoverTrigger>

      <PopoverContent side="bottom" align="start" sideOffset={4} className="w-80 gap-0 p-0">
        {/* Tabs */}
        <div className="flex border-b px-3 pt-2">
          {(["icons", "emojis"] as const).map((t) => (
            <button
              ...
              className={`px-2 pb-2 pt-0.5 text-xs font-medium transition-colors ${
                pickerTab === t
          ...
          {PICKER_COLORS.map((c) => (
            <button
              ...
              className={`flex size-6 shrink-0 items-center justify-center rounded-full ${c.bg} transition-all ${
                colorId === c.id
                  ? "ring-2 ring-white/30 ring-offset-1 ring-offset-popover"
        ...
        <div className="grid max-h-60 grid-cols-[repeat(14,1fr)] overflow-auto p-2 gap-0.5">
          {filtered.map((entry) => (
            <button
              key={entry.name}
              type="button"
              ...
              className={`flex size-[22px] items-center justify-center rounded transition-colors hover:bg-accent ${
                selectedIcon === entry.icon ? "bg-accent" : ""
              }`}
            >
              <HugeiconsIcon icon={entry.icon} strokeWidth={2} className="size-3.5 text-foreground/90" />
            </button>
          ))}
          {filtered.length === 0 && (
            <p className="col-span-full py-3 text-center text-xs text-muted-foreground">No icons found</p>
          )}
        </div>
      </PopoverContent>
    </Popover>
