"use client"

import * as React from "react"
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover"
import {
  FILTER_GROUPS,
  FILTER_LABEL,
  setFilterValues,
  type FilterKind,
  type FilterState,
} from "@/app/lib/my-issues-filter-state"
import type { Issue, Member, Label, Project } from "@/app/lib/mock-data"
import { StatusIcon, PriorityIcon } from "@/components/status-icons"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { cn } from "@/lib/utils"

type Status = Issue["status"]
type Priority = Issue["priority"]

const STATUS_OPTIONS: { value: Status; label: string }[] = [
  { value: "backlog", label: "Backlog" },
  { value: "todo", label: "Todo" },
  { value: "in_progress", label: "In Progress" },
  { value: "done", label: "Completed" },
  { value: "cancelled", label: "Canceled" },
]

const PRIORITY_OPTIONS: { value: Priority; label: string }[] = [
  { value: "urgent", label: "Urgent" },
  { value: "high", label: "High" },
  { value: "medium", label: "Medium" },
  { value: "low", label: "Low" },
  { value: "none", label: "No priority" },
]

const RELATION_OPTIONS = [
  { value: "parent", label: "Parent issues" },
  { value: "sub", label: "Sub-issues" },
  { value: "blocked", label: "Blocked issues" },
  { value: "blocking", label: "Blocking issues" },
  { value: "recurring", label: "Recurring issues" },
  { value: "with-relations", label: "Issues with relations" },
  { value: "duplicates", label: "Duplicates" },
] as const

function ProjectPropertyIcon({ kind }: { kind: string }) {
  const props = {
    width: 14,
    height: 14,
    viewBox: "0 0 16 16",
    fill: "none" as const,
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round" as const,
    strokeLinejoin: "round" as const,
    "aria-hidden": true,
  }
  switch (kind) {
    case "projectStatus":
      return (
        <svg {...props}>
          <circle cx="8" cy="8" r="5.5" strokeDasharray="2 1.4" />
        </svg>
      )
    case "projectPriority":
      return (
        <svg {...props} fill="currentColor" stroke="none">
          <rect x="2" y="9" width="2.5" height="5" rx="0.5" />
          <rect x="6.75" y="6" width="2.5" height="8" rx="0.5" />
          <rect x="11.5" y="3" width="2.5" height="11" rx="0.5" />
        </svg>
      )
    case "projectLead":
      return (
        <svg {...props}>
          <circle cx="8" cy="6" r="2.5" />
          <path d="M3.5 13.5 C3.5 11 5.5 9.5 8 9.5 C10.5 9.5 12.5 11 12.5 13.5" />
        </svg>
      )
    case "projectMilestone":
      return (
        <svg {...props} fill="currentColor" stroke="none">
          <path d="M8 1.5 L14 8 L8 14.5 L2 8 Z" />
        </svg>
      )
    default:
      return null
  }
}

function DateOptionIcon({ kind }: { kind: string }) {
  const props = {
    width: 14,
    height: 14,
    viewBox: "0 0 16 16",
    fill: "none" as const,
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round" as const,
    strokeLinejoin: "round" as const,
    "aria-hidden": true,
  }
  switch (kind) {
    case "dueDate":
      return (
        <svg {...props}>
          <rect x="2" y="3" width="12" height="11" rx="2.5" />
        </svg>
      )
    case "createdDate":
      return (
        <svg {...props}>
          <path d="M2.5 3 L2.5 13.5 L11 13.5 L13.5 11 L13.5 3 Z" />
          <path d="M11 13.5 L11 11 L13.5 11" />
        </svg>
      )
    case "updatedDate":
      return (
        <svg {...props}>
          <path d="M2.5 8 A5.5 5.5 0 0 1 13.5 8" />
          <path d="M11 5.5 L13.5 5.5 L13.5 3" />
          <path d="M13.5 8 A5.5 5.5 0 0 1 2.5 8" />
          <path d="M5 10.5 L2.5 10.5 L2.5 13" />
        </svg>
      )
    case "startedDate":
      return (
        <svg {...props}>
          <rect x="2" y="3" width="12" height="11" rx="2.5" />
          <path d="M7 7 L10 8.5 L7 10 Z" fill="currentColor" />
        </svg>
      )
    case "completedDate":
      return (
        <svg {...props}>
          <rect x="2" y="3" width="12" height="11" rx="2.5" />
          <path d="M5.5 8.5 L7.5 10.5 L11 7" />
        </svg>
      )
    case "triagedDate":
      return (
        <svg {...props}>
          <circle cx="8" cy="8" r="5.5" />
          <path d="M5 8 L11 8 M5 8 L7 6 M11 8 L9 6 M5 8 L7 10 M11 8 L9 10" />
        </svg>
      )
    case "timeInStatus":
      return (
        <svg {...props}>
          <circle cx="8" cy="8" r="5.5" />
          <path d="M8 5 L8 8 L10.5 9.5" />
        </svg>
      )
    default:
      return null
  }
}

function RelationIcon({ kind }: { kind: string }) {
  const props = {
    width: 14,
    height: 14,
    viewBox: "0 0 16 16",
    fill: "none" as const,
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round" as const,
    strokeLinejoin: "round" as const,
    "aria-hidden": true,
  }
  switch (kind) {
    case "parent":
      return (
        <svg {...props}>
          <rect x="2.5" y="2.5" width="11" height="11" rx="2.5" />
          <path d="M2.5 6.5 L6.5 6.5 L6.5 2.5" />
        </svg>
      )
    case "sub":
      return (
        <svg {...props}>
          <rect x="2.5" y="2.5" width="6" height="6" rx="1" />
          <rect x="7.5" y="7.5" width="6" height="6" rx="1" />
        </svg>
      )
    case "blocked":
      return (
        <svg {...props}>
          <circle cx="8" cy="8" r="5.5" />
          <path d="M4 8 L12 8" />
        </svg>
      )
    case "blocking":
      return (
        <svg {...props}>
          <circle cx="8" cy="8" r="5.5" />
          <path d="M4 8 L12 8" />
          <path d="M5 5 L11 11" />
        </svg>
      )
    case "recurring":
      return (
        <svg {...props}>
          <path d="M3 8 A5 5 0 1 1 8 13" />
          <path d="M5.5 11 L3 13 L1 11" />
        </svg>
      )
    case "with-relations":
      return (
        <svg {...props}>
          <path d="M4 14 L4 2" />
          <path d="M4 2.5 L11.5 4 L7.5 6.5 L11.5 9 L4 8.5" />
        </svg>
      )
    case "duplicates":
      return (
        <svg {...props}>
          <circle cx="6" cy="6" r="3" />
          <circle cx="10" cy="10" r="3" />
        </svg>
      )
    default:
      return null
  }
}

export interface FilterPopoverHandle {
  open(): void
}

interface Props {
  filters: FilterState
  onChange: (next: FilterState) => void
  issues: Issue[]
  members: Member[]
  labels: Label[]
  projects: Project[]
  initialKind?: FilterKind | null
  open?: boolean
  onOpenChange?: (open: boolean) => void
  trigger: React.ReactElement
}

export const MyIssuesFilterPopover = React.forwardRef<
  FilterPopoverHandle,
  Props
>(function MyIssuesFilterPopover(
  {
    filters,
    onChange,
    issues,
    members,
    labels,
    projects,
    initialKind,
    open,
    onOpenChange,
    trigger,
  },
  ref
) {
  const [internalOpen, setInternalOpen] = React.useState(false)
  const isControlled = open !== undefined
  const isOpen = isControlled ? open : internalOpen
  const setOpen = React.useCallback(
    (v: boolean) => {
      if (!isControlled) setInternalOpen(v)
      onOpenChange?.(v)
    },
    [isControlled, onOpenChange]
  )

  React.useImperativeHandle(ref, () => ({ open: () => setOpen(true) }), [setOpen])

  const [hoveredKind, setHoveredKind] = React.useState<FilterKind | null>(null)
  const [focusedIndex, setFocusedIndex] = React.useState(0)
  const [query, setQuery] = React.useState("")
  const [sideOffset, setSideOffset] = React.useState(0)
  const searchRef = React.useRef<HTMLInputElement>(null)
  const mainPanelRef = React.useRef<HTMLDivElement>(null)
  const sidePanelRef = React.useRef<HTMLDivElement>(null)
  const rowRefs = React.useRef<Map<FilterKind, HTMLButtonElement>>(new Map())

  React.useEffect(() => {
    if (isOpen) {
      setHoveredKind(initialKind ?? null)
      setQuery("")
      setFocusedIndex(0)
      const t = setTimeout(() => searchRef.current?.focus(), 30)
      return () => clearTimeout(t)
    } else {
      setHoveredKind(null)
    }
  }, [isOpen, initialKind])

  const visibleGroups = React.useMemo(() => {
    const q = query.trim().toLowerCase()
    if (!q) return FILTER_GROUPS
    return FILTER_GROUPS.map((g) => ({
      items: g.items.filter((k) => FILTER_LABEL[k].toLowerCase().includes(q)),
    })).filter((g) => g.items.length > 0)
  }, [query])

  const flatItems = React.useMemo(
    () => visibleGroups.flatMap((g) => g.items),
    [visibleGroups]
  )

  // Reset focus to first row whenever the visible set changes (e.g., user typed)
  React.useEffect(() => {
    setFocusedIndex(0)
  }, [query])

  // Measure the hovered row's vertical offset so the side panel aligns next to it.
  // Cap the offset so the side panel never extends past the main panel's bottom —
  // otherwise long pickers (e.g. Subscribers) would push the popover off-screen.
  React.useLayoutEffect(() => {
    if (!hoveredKind) {
      setSideOffset(0)
      return
    }
    const rowEl = rowRefs.current.get(hoveredKind)
    const mainEl = mainPanelRef.current
    const sideEl = sidePanelRef.current
    if (!rowEl || !mainEl) return
    const rowOffset = Math.max(
      0,
      rowEl.getBoundingClientRect().top -
        mainEl.getBoundingClientRect().top
    )
    const sideH = sideEl?.offsetHeight ?? 0
    const mainH = mainEl.offsetHeight
    const maxOffset = Math.max(0, mainH - sideH)
    setSideOffset(Math.min(rowOffset, maxOffset))
  }, [hoveredKind])

  const onPickCategory = (kind: FilterKind) => {
    if (!hasSubmenu(kind)) {
      const existing = filters[kind]
      if (existing && existing.length > 0) {
        const next = { ...filters }
        delete next[kind]
        onChange(next)
      } else {
        onChange({ ...filters, [kind]: ["__stub__"] })
      }
      setOpen(false)
      return
    }
    setHoveredKind(kind)
  }

  const onSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (flatItems.length === 0) return
    if (e.key === "ArrowDown") {
      e.preventDefault()
      setFocusedIndex((i) => (i + 1) % flatItems.length)
    } else if (e.key === "ArrowUp") {
      e.preventDefault()
      setFocusedIndex(
        (i) => (i - 1 + flatItems.length) % flatItems.length
      )
    } else if (e.key === "Enter") {
      e.preventDefault()
      const kind = flatItems[focusedIndex]
      if (kind) onPickCategory(kind)
    } else if (e.key === "ArrowRight") {
      const kind = flatItems[focusedIndex]
      if (kind && hasSubmenu(kind)) {
        e.preventDefault()
        setHoveredKind(kind)
      }
    }
  }

  const sideOpen = hoveredKind !== null && hasSubmenu(hoveredKind)
  const focusedKind = flatItems[focusedIndex]

  return (
    <Popover open={isOpen} onOpenChange={setOpen}>
      <PopoverTrigger render={trigger} />
      <PopoverContent
        align="end"
        sideOffset={6}
        className="w-auto gap-1.5 border-0 bg-transparent p-0 shadow-none ring-0"
      >
        <div
          className="flex items-start gap-1.5"
          onMouseLeave={() => setHoveredKind(null)}
        >
          {sideOpen && hoveredKind && (
            <div
              ref={sidePanelRef}
              className="bg-popover ring-foreground/10 w-[230px] overflow-hidden rounded-lg shadow-md ring-1"
              style={{ marginTop: sideOffset }}
            >
              <ValuePicker
                kind={hoveredKind}
                currentValues={filters[hoveredKind] ?? []}
                issues={issues}
                members={members}
                labels={labels}
                projects={projects}
                onChange={(values) =>
                  onChange(setFilterValues(filters, hoveredKind, values))
                }
              />
            </div>
          )}
          <div
            ref={mainPanelRef}
            className="bg-popover ring-foreground/10 flex w-[230px] flex-col overflow-hidden rounded-lg shadow-md ring-1"
          >
            <div className="border-border/50 flex h-9 items-center gap-2 border-b px-3">
              <input
                ref={searchRef}
                type="text"
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                onKeyDown={onSearchKeyDown}
                placeholder="Add Filter..."
                aria-label="Search filters"
                className="placeholder:text-muted-foreground flex-1 bg-transparent text-[13px] outline-none"
              />
              <span
                aria-hidden="true"
                className="text-muted-foreground border-border/60 rounded border px-1.5 py-0.5 font-mono text-[10px] leading-none"
              >
                F
              </span>
            </div>
            <div className="py-1.5">
              {flatItems.length === 0 ? (
                <div className="text-muted-foreground px-3 py-2 text-center text-xs">
                  No results
                </div>
              ) : (
                visibleGroups.map((g, gi) => (
                  <React.Fragment key={gi}>
                    {gi > 0 && (
                      <div className="border-border/40 my-1.5 border-t" />
                    )}
                    {g.items.map((kind) => {
                      const active =
                        filters[kind] && filters[kind]!.length > 0
                      const highlight =
                        hoveredKind === kind || focusedKind === kind
                      return (
                        <button
                          key={kind}
                          ref={(el) => {
                            if (el) rowRefs.current.set(kind, el)
                            else rowRefs.current.delete(kind)
                          }}
                          type="button"
                          onClick={() => onPickCategory(kind)}
                          onMouseEnter={() => {
                            if (hasSubmenu(kind)) setHoveredKind(kind)
                            const idx = flatItems.indexOf(kind)
                            if (idx >= 0) setFocusedIndex(idx)
                          }}
                          className={cn(
                            "flex h-7 w-full items-center gap-2 px-2.5 text-left text-[13px]",
                            highlight ? "bg-accent" : "hover:bg-accent"
                          )}
                        >
                          <span className="text-muted-foreground flex size-4 shrink-0 items-center justify-center">
                            <FilterCategoryIcon kind={kind} />
                          </span>
                          <span className="flex-1 truncate">
                            {FILTER_LABEL[kind]}
                          </span>
                          {active && (
                            <span className="text-muted-foreground text-[11px]">
                              {filters[kind]!.length}
                            </span>
                          )}
                          {hasSubmenu(kind) && (
                            <ChevronRightIcon className="text-muted-foreground/70 size-3 shrink-0" />
                          )}
                        </button>
                      )
                    })}
                  </React.Fragment>
                ))
              )}
            </div>
          </div>
        </div>
      </PopoverContent>
    </Popover>
  )
})

function ValuePicker({
  kind,
  currentValues,
  issues,
  members,
  labels,
  projects,
  onChange,
}: {
  kind: FilterKind
  currentValues: string[]
  issues: Issue[]
  members: Member[]
  labels: Label[]
  projects: Project[]
  onChange: (values: string[]) => void
}) {
  const [query, setQuery] = React.useState("")

  // Reset query when switching to a different kind so the user starts fresh
  React.useEffect(() => {
    setQuery("")
  }, [kind])

  const options = useOptions(kind, { members, labels, projects })
  const filtered = React.useMemo(() => {
    const q = query.trim().toLowerCase()
    if (!q) return options
    return options.filter((o) => o.label.toLowerCase().includes(q))
  }, [options, query])

  const counts = React.useMemo(() => {
    const m = new Map<string, number>()
    const inc = (k: string) => m.set(k, (m.get(k) ?? 0) + 1)
    for (const issue of issues) {
      if (kind === "status") inc(issue.status)
      else if (kind === "priority") inc(issue.priority)
      else if (kind === "assignee") inc(issue.assigneeId ?? "unassigned")
      else if (kind === "creator") inc(issue.creatorId)
      else if (kind === "project") inc(issue.projectId ?? "none")
      else if (kind === "labels") for (const id of issue.labelIds ?? []) inc(id)
      else if (kind === "template") inc("none")
    }
    return m
  }, [issues, kind])

  const toggle = (value: string) => {
    const set = new Set(currentValues)
    if (set.has(value)) set.delete(value)
    else set.add(value)
    onChange([...set])
  }

  return (
    <div className="flex flex-col">
      <div className="border-border/50 flex h-9 items-center gap-2 border-b px-3">
        <input
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder={
            kind === "content" ? "Filter by content..." : "Filter..."
          }
          aria-label={`Filter ${FILTER_LABEL[kind]}`}
          className="placeholder:text-muted-foreground flex-1 bg-transparent text-[13px] outline-none"
        />
      </div>
      {kind === "content" ? null : (
      <div className="max-h-[28rem] overflow-auto py-1.5">
        {filtered.length === 0 ? (
          <div className="text-muted-foreground px-3 py-2 text-center text-xs">
            No matching options
          </div>
        ) : (
          filtered.map((o) => {
            const selected = currentValues.includes(o.value)
            const count = counts.get(o.value) ?? 0
            return (
              <button
                key={o.value}
                type="button"
                onClick={() => toggle(o.value)}
                className="hover:bg-accent flex h-7 w-full items-center gap-2 px-2.5 text-left text-[13px]"
              >
                <span
                  className={cn(
                    "border-muted-foreground/40 flex size-3.5 shrink-0 items-center justify-center rounded-sm border",
                    selected && "border-primary bg-primary text-primary-foreground"
                  )}
                  aria-hidden="true"
                >
                  {selected && (
                    <svg
                      width="10"
                      height="10"
                      viewBox="0 0 10 10"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="1.6"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    >
                      <path d="M2 5 L4 7 L8 3" />
                    </svg>
                  )}
                </span>
                {o.icon}
                <span className="flex-1 truncate">{o.label}</span>
                {count > 0 && (
                  <span className="text-muted-foreground text-[11px]">
                    {count} {count === 1 ? "issue" : "issues"}
                  </span>
                )}
              </button>
            )
          })
        )}
      </div>
      )}
    </div>
  )
}

function hasSubmenu(kind: FilterKind): boolean {
  return kind !== "aiFilter" && kind !== "advancedFilter"
}

function ChevronRightIcon({ className }: { className?: string }) {
  return (
    <svg
      width="12"
      height="12"
      viewBox="0 0 12 12"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.4"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
      className={className}
    >
      <path d="M4.5 2.5 L8 6 L4.5 9.5" />
    </svg>
  )
}

function FilterCategoryIcon({ kind }: { kind: FilterKind }) {
  const filled = {
    width: 16,
    height: 16,
    viewBox: "0 0 16 16",
    fill: "currentColor",
    "aria-hidden": true,
  }
  const filled14 = {
    width: 14,
    height: 14,
    viewBox: "0 0 14 14",
    fill: "currentColor",
    "aria-hidden": true,
  }

  switch (kind) {
    case "aiFilter":
      return (
        <svg {...filled}>
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M15.2642 10.9643L12.3789 8.07906C12.9923 7.19457 13.2716 6.11809 13.1623 5.04291C13.0463 3.90157 12.5011 2.8469 11.637 2.09228C10.7729 1.33766 9.65441 0.939437 8.50784 0.978168C7.36127 1.0169 6.27221 1.4897 5.461 2.30091C4.64979 3.11212 4.17699 4.20117 4.13826 5.34774C4.09953 6.49431 4.49776 7.61279 5.25238 8.4769C6.007 9.341 7.06166 9.88621 8.20301 10.0022C9.27819 10.1115 10.3547 9.83225 11.2392 9.21884L14.1243 12.104L14.1263 12.106C14.2782 12.2542 14.482 12.3371 14.6942 12.3371C14.9064 12.3371 15.1102 12.2542 15.2621 12.106L15.2642 12.1039C15.4152 11.9527 15.5 11.7478 15.5 11.5341C15.5 11.3205 15.4152 11.1155 15.2642 10.9643ZM5.7789 5.51468C5.77894 4.94168 5.94886 4.38156 6.26721 3.90512C6.58558 3.42864 7.0381 3.05727 7.56754 2.83797C8.09697 2.61867 8.67955 2.56129 9.2416 2.67309C9.80365 2.78489 10.3199 3.06084 10.7251 3.46606C11.1303 3.87127 11.4063 4.38754 11.5181 4.94959C11.6299 5.51164 11.5725 6.09422 11.3532 6.62365C11.1339 7.15309 10.7625 7.60561 10.2861 7.92398C9.80966 8.24231 9.24957 8.41224 8.67661 8.41229C7.90837 8.41138 7.17185 8.10579 6.62862 7.56257C6.08542 7.01936 5.77983 6.28288 5.7789 5.51468Z"
          />
          <path d="M3.35661 7.75299C3.4804 7.75299 3.58698 7.84037 3.61126 7.96175L4.09841 10.3975L6.01651 10.877C6.13211 10.9059 6.21321 11.0098 6.21321 11.129C6.21321 11.2481 6.13211 11.352 6.01651 11.3809L4.09841 11.8604L3.61126 14.2962C3.58698 14.4176 3.4804 14.505 3.35661 14.505C3.23282 14.505 3.12624 14.4176 3.10196 14.2962L2.61481 11.8604L0.696707 11.3809C0.581101 11.352 0.5 11.2481 0.5 11.129C0.5 11.0098 0.581101 10.9059 0.696707 10.877L2.61481 10.3975L3.10196 7.96175C3.12624 7.84037 3.23282 7.75299 3.35661 7.75299Z" />
          <path d="M6.47291 12.4274C6.58469 12.4274 6.68392 12.499 6.71927 12.605L6.9379 13.2609L7.59379 13.4795C7.69983 13.5149 7.77136 13.6141 7.77136 13.7259C7.77137 13.8377 7.69984 13.9369 7.5938 13.9722L6.9379 14.1909L6.71927 14.8468C6.68392 14.9528 6.58469 15.0244 6.47291 15.0244C6.36113 15.0244 6.26189 14.9528 6.22654 14.8468L6.00791 14.1909L5.35201 13.9722C5.24597 13.9369 5.17445 13.8377 5.17445 13.7259C5.17445 13.6141 5.24598 13.5149 5.35202 13.4795L6.00791 13.2609L6.22654 12.605C6.26189 12.499 6.36113 12.4274 6.47291 12.4274Z" />
        </svg>
      )
    case "advancedFilter":
      return (
        <svg {...filled}>
          <path d="M6.21973 10.749V8.7793H5.5C4.24079 8.7793 3.21973 7.75823 3.21973 6.49902V4.5293H1.75C1.31922 4.5293 0.969727 4.17981 0.969727 3.74902C0.969727 3.31824 1.31922 2.96875 1.75 2.96875L12.25 2.96875C12.6808 2.96875 13.0303 3.31824 13.0303 3.74902C13.0303 4.17981 12.6808 4.5293 12.25 4.5293H4.78027V6.49902C4.78027 6.89667 5.10236 7.21875 5.5 7.21875L14.25 7.21875C14.6808 7.21875 15.0303 7.56824 15.0303 7.99902C15.0303 8.42981 14.6808 8.7793 14.25 8.7793H7.78027V10.749C7.78027 11.1467 8.10236 11.4688 8.5 11.4688H12.5C12.9308 11.4688 13.2803 11.8182 13.2803 12.249C13.2803 12.6798 12.9308 13.0293 12.5 13.0293H8.5C7.24079 13.0293 6.21973 12.0082 6.21973 10.749Z" />
        </svg>
      )
    case "status":
      return (
        <svg {...filled14}>
          <path d="M13.9408 7.91426L11.9576 7.65557C11.9855 7.4419 12 7.22314 12 7C12 6.77686 11.9855 6.5581 11.9576 6.34443L13.9408 6.08573C13.9799 6.38496 14 6.69013 14 7C14 7.30987 13.9799 7.61504 13.9408 7.91426ZM13.4688 4.32049C13.2328 3.7514 12.9239 3.22019 12.5538 2.73851L10.968 3.95716C11.2328 4.30185 11.4533 4.68119 11.6214 5.08659L13.4688 4.32049ZM11.2615 1.4462L10.0428 3.03204C9.69815 2.76716 9.31881 2.54673 8.91341 2.37862L9.67951 0.531163C10.2486 0.767153 10.7798 1.07605 11.2615 1.4462ZM7.91426 0.0591659L7.65557 2.04237C7.4419 2.01449 7.22314 2 7 2C6.77686 2 6.5581 2.01449 6.34443 2.04237L6.08574 0.059166C6.38496 0.0201343 6.69013 0 7 0C7.30987 0 7.61504 0.0201343 7.91426 0.0591659ZM4.32049 0.531164L5.08659 2.37862C4.68119 2.54673 4.30185 2.76716 3.95716 3.03204L2.73851 1.4462C3.22019 1.07605 3.7514 0.767153 4.32049 0.531164ZM1.4462 2.73851L3.03204 3.95716C2.76716 4.30185 2.54673 4.68119 2.37862 5.08659L0.531164 4.32049C0.767153 3.7514 1.07605 3.22019 1.4462 2.73851ZM0.0591659 6.08574C0.0201343 6.38496 0 6.69013 0 7C0 7.30987 0.0201343 7.61504 0.059166 7.91426L2.04237 7.65557C2.01449 7.4419 2 7.22314 2 7C2 6.77686 2.01449 6.5581 2.04237 6.34443L0.0591659 6.08574ZM0.531164 9.67951L2.37862 8.91341C2.54673 9.31881 2.76716 9.69815 3.03204 10.0428L1.4462 11.2615C1.07605 10.7798 0.767153 10.2486 0.531164 9.67951ZM2.73851 12.5538L3.95716 10.968C4.30185 11.2328 4.68119 11.4533 5.08659 11.6214L4.32049 13.4688C3.7514 13.2328 3.22019 12.9239 2.73851 12.5538ZM6.08574 13.9408L6.34443 11.9576C6.5581 11.9855 6.77686 12 7 12C7.22314 12 7.4419 11.9855 7.65557 11.9576L7.91427 13.9408C7.61504 13.9799 7.30987 14 7 14C6.69013 14 6.38496 13.9799 6.08574 13.9408ZM9.67951 13.4688L8.91341 11.6214C9.31881 11.4533 9.69815 11.2328 10.0428 10.968L11.2615 12.5538C10.7798 12.9239 10.2486 13.2328 9.67951 13.4688ZM12.5538 11.2615L10.968 10.0428C11.2328 9.69815 11.4533 9.31881 11.6214 8.91341L13.4688 9.67951C13.2328 10.2486 12.924 10.7798 12.5538 11.2615Z" />
        </svg>
      )
    case "assignee":
      return (
        <svg {...filled}>
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M12.2616 10.1807C12.5759 9.91122 13.0496 9.94763 13.3192 10.2617L14.8192 12.0117C15.0599 12.2926 15.0599 12.7074 14.8192 12.9883L13.3192 14.7383C13.0496 15.0523 12.5759 15.0888 12.2616 14.8193C11.9476 14.5498 11.9114 14.0761 12.1805 13.7617L12.619 13.25H9.81821C9.31854 13.2504 8.86663 13.5485 8.66978 14.0078L8.43931 14.5459C8.27587 14.926 7.83438 15.1024 7.45396 14.9395C7.0739 14.7761 6.89769 14.3345 7.0604 13.9541L7.29087 13.417C7.72409 12.4061 8.7185 11.7504 9.81821 11.75H12.619L12.1805 11.2383C11.9114 10.924 11.9476 10.4502 12.2616 10.1807Z"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M8.57603 8C9.16635 8.00005 9.73859 8.10941 10.2684 8.31152C10.6549 8.45932 10.8493 8.89258 10.702 9.2793C10.5544 9.6662 10.1202 9.86031 9.73325 9.71289C9.37143 9.57489 8.98057 9.50005 8.57603 9.5H7.42563C6.0104 9.50003 4.75811 10.4161 4.32896 11.7646L3.46567 14.4775C3.34008 14.8721 2.91789 15.0902 2.52329 14.9648C2.12863 14.8392 1.9104 14.4171 2.03599 14.0225L2.89927 11.3096C3.52653 9.3386 5.35724 8.00003 7.42563 8H8.57603Z"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M7.99985 1C9.51864 1 10.7499 2.23122 10.7499 3.75C10.7499 5.26878 9.51864 6.5 7.99985 6.5C6.4812 6.49985 5.24985 5.26869 5.24985 3.75C5.24985 2.23131 6.4812 1.00015 7.99985 1Z"
          />
        </svg>
      )
    case "agent":
      return (
        <svg {...filled}>
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M11.75 1C13.5449 1 15 2.45507 15 4.25V5.75C15 6.16421 14.6642 6.5 14.25 6.5C13.8358 6.5 13.5 6.16421 13.5 5.75V4.25C13.5 3.2835 12.7165 2.5 11.75 2.5H4.25C3.2835 2.5 2.5 3.2835 2.5 4.25V11.75C2.5 12.7165 3.2835 13.5 4.25 13.5H5.75C6.16421 13.5 6.5 13.8358 6.5 14.25C6.5 14.6642 6.16421 15 5.75 15H4.25C2.45507 15 1 13.5449 1 11.75V4.25C1 2.45507 2.45507 1 4.25 1H11.75Z"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M6.05273 7.29883C5.79591 6.52818 6.52818 5.79591 7.29883 6.05273L14.3252 8.39551C15.1585 8.67345 15.2406 9.82021 14.4551 10.2129L11.9209 11.4795C11.7301 11.5749 11.5749 11.7301 11.4795 11.9209L10.2129 14.4551C9.82021 15.2406 8.67345 15.1585 8.39551 14.3252L6.05273 7.29883Z"
          />
        </svg>
      )
    case "creator":
      return (
        <svg {...filled}>
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M13.2996 9.29785C13.6697 8.91896 14.2916 8.89861 14.6883 9.25195C14.7091 9.27048 14.7292 9.28979 14.7479 9.30957C15.0201 9.59815 15.0731 10.0136 14.9002 10.3516C14.8701 10.4103 14.8337 10.4674 14.7899 10.5205C14.7731 10.5409 14.755 10.5608 14.7361 10.5801L11.8338 13.5518L11.8299 13.5557L11.7029 13.6865C11.64 13.751 11.5741 13.8132 11.5076 13.874C11.3744 13.9958 11.2348 14.1114 11.0887 14.2178C11.0155 14.2711 10.9403 14.3218 10.8641 14.3711C10.56 14.5677 10.2332 14.7328 9.88947 14.8604C9.84043 14.8785 9.79023 14.8939 9.74005 14.9082C9.68944 14.9227 9.63823 14.9351 9.58673 14.9463C9.48414 14.9686 9.37955 14.9863 9.27521 14.999C9.25729 15.0012 9.23958 15.0006 9.22248 14.999C9.12205 14.9896 9.03967 14.9188 9.01154 14.8252C9.00165 14.7923 8.99745 14.7565 9.00275 14.7197L9.00763 14.6816C9.03892 14.4694 9.08364 14.2574 9.16193 14.0576C9.34063 13.6023 9.59276 13.1755 9.90802 12.7939H9.90998C9.9928 12.6939 10.0793 12.5956 10.1707 12.502L10.2391 12.4316L13.2996 9.29785Z"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M8.57502 8C9.1654 8.00001 9.73753 8.10941 10.2674 8.31152C10.6541 8.45922 10.8484 8.89245 10.701 9.2793C10.5533 9.66626 10.1192 9.86052 9.73224 9.71289C9.37036 9.57489 8.97962 9.50001 8.57502 9.5H7.42462C6.00944 9.50011 4.75705 10.4161 4.32795 11.7646L3.46466 14.4775C3.33902 14.8722 2.91695 15.0904 2.52228 14.9648C2.12775 14.8392 1.90945 14.4171 2.03498 14.0225L2.89826 11.3096C3.52547 9.33861 5.35628 8.00011 7.42462 8H8.57502Z"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M7.99982 1C9.5186 1 10.7498 2.23122 10.7498 3.75C10.7498 5.26878 9.5186 6.5 7.99982 6.5C6.48119 6.49982 5.24982 5.26867 5.24982 3.75C5.24982 2.23133 6.48119 1.00018 7.99982 1Z"
          />
        </svg>
      )
    case "priority":
      return (
        <svg {...filled}>
          <rect x="1" y="8" width="3" height="6" rx="1" />
          <rect x="6" y="5" width="3" height="9" rx="1" />
          <rect x="11" y="2" width="3" height="12" rx="1" />
        </svg>
      )
    case "labels":
    case "suggestedLabel":
      return (
        <svg {...filled}>
          <path d="M12 11.5V13H5.132v-1.5H12Zm1.5-1.5V6a1.5 1.5 0 0 0-1.346-1.492L12 4.5H5.133a.5.5 0 0 0-.303.103l-.08.076-2.382 2.834a.5.5 0 0 0-.11.234l-.008.087v.331a.5.5 0 0 0 .118.321l2.382 2.835a.5.5 0 0 0 .383.179V13l-.22-.012a2 2 0 0 1-1.16-.54l-.15-.16L1.218 9.45a2 2 0 0 1-.46-1.11L.75 8.165v-.331a2 2 0 0 1 .363-1.147l.106-.14 2.383-2.834a2 2 0 0 1 1.312-.701L5.134 3H12a3 3 0 0 1 3 3v4a3 3 0 0 1-3.002 3v-1.5c.778 0 1.417-.59 1.494-1.347L13.5 10Z" />
          <path d="M5.5 8a1 1 0 1 1 2 0 1 1 0 0 1-2 0Z" />
        </svg>
      )
    case "relations":
      return (
        <svg {...filled}>
          <path d="M3.75 1.04a.75.75 0 0 1 .75.75v12.476a.75.75 0 0 1-1.5 0V1.79a.75.75 0 0 1 .75-.75Zm1.716.46a.5.5 0 0 1 .704-.456l7.535 3.375a.5.5 0 0 1 .012.907L6.183 8.95a.5.5 0 0 1-.717-.45v-7Z" />
        </svg>
      )
    case "dates":
      return (
        <svg {...filled}>
          <path d="M11 1C13.2091 1 15 2.79086 15 5V11C15 13.2091 13.2091 15 11 15H5C2.79086 15 1 13.2091 1 11V5C1 2.79086 2.79086 1 5 1H11ZM13.5 6H2.5V11C2.5 12.3807 3.61929 13.5 5 13.5H11C12.3807 13.5 13.5 12.3807 13.5 11V6Z" />
        </svg>
      )
    case "project":
      return (
        <svg {...filled}>
          <path d="M8 1.4 L14 4.6 L14 11.4 L8 14.6 L2 11.4 L2 4.6 Z M8 3 L3.5 5.4 L8 7.8 L12.5 5.4 Z M3 6.6 L3 10.8 L7.25 13 L7.25 8.85 Z M8.75 8.85 L8.75 13 L13 10.8 L13 6.6 Z" />
        </svg>
      )
    case "projectProperties":
      return (
        <svg {...filled}>
          <path d="M11 13a1 1 0 1 1 0 2 1 1 0 0 1 0-2ZM14 13a1 1 0 1 1 0 2 1 1 0 0 1 0-2Z" />
          <path
            fillRule="evenodd"
            d="M7.331 1.07c.441-.094.897-.094 1.338 0 .498.105.967.376 1.904.916l1.354.78c.937.54 1.406.811 1.747 1.189.301.335.53.73.669 1.157.157.484.157 1.026.157 2.107v.779l-.004.076a.748.748 0 0 1-1.491-.073H13v-.782c0-.378-.002-.661-.007-.878L9.987 7.844a2.25 2.25 0 0 0-1.236 2.008v1.11L8.75 11v3.024c0 .504-.375.938-.879.943a2.671 2.671 0 0 1-.54-.04c-.498-.105-.967-.376-1.904-.916l-1.354-.78c-.937-.541-1.406-.812-1.747-1.19a3.214 3.214 0 0 1-.669-1.156C1.5 10.402 1.5 9.86 1.5 8.779V7.22c0-1.081 0-1.623.157-2.107.14-.428.368-.822.67-1.157.34-.378.81-.648 1.746-1.188l1.354-.78c.937-.541 1.406-.812 1.904-.918ZM3 7.218v1.56c0 1.182.018 1.438.084 1.642.074.228.196.439.356.617.145.16.358.303 1.382.893l1.354.782c.542.312.863.49 1.075.597V9.851A2.25 2.25 0 0 0 6.027 7.85l-3.02-1.51C3.002 6.559 3 6.842 3 7.22Zm5.357-4.683c-.235-.05-.479-.05-.714 0-.211.045-.442.158-1.467.75l-1.354.78c-.91.524-1.18.696-1.33.84l3.502 1.751c.634.317 1.38.317 2.013 0l3.5-1.75c-.15-.145-.419-.317-1.33-.842l-1.353-.78c-1.025-.59-1.256-.704-1.467-.749Z"
            clipRule="evenodd"
          />
          <path d="M11 10a1 1 0 1 1 0 2 1 1 0 0 1 0-2ZM14 10a1 1 0 1 1 0 2 1 1 0 0 1 0-2Z" />
        </svg>
      )
    case "initiative":
      return (
        <svg {...filled}>
          <path d="M8 2.25a5.75 5.75 0 1 0 0 11.5 5.75 5.75 0 0 0 0-11.5ZM.75 8a7.25 7.25 0 1 1 14.5 0A7.25 7.25 0 0 1 .75 8Z" />
          <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5ZM4 8a4 4 0 1 1 8 0 4 4 0 0 1-8 0Z" />
          <circle cx="8" cy="8" r="1.1" />
        </svg>
      )
    case "subscribers":
      return (
        <svg {...filled}>
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M8 1C10.347 1.00026 12.25 2.90296 12.25 5.25V5.92969C12.25 6.76016 12.5519 7.56241 13.0986 8.1875L13.499 8.64551C13.822 9.01469 14.001 9.48899 14.001 9.97949V11.6162C14.0009 12.2422 13.4922 12.7497 12.8662 12.75H10.7021C10.4668 14.0298 9.34788 15 8 15C6.65212 15 5.53316 14.0298 5.29785 12.75H3.13477C2.50865 12.7499 2.00014 12.2423 2 11.6162V9.97949C2 9.48895 2.17898 9.01472 2.50195 8.64551L2.90234 8.18848C3.44909 7.56333 3.75098 6.7602 3.75098 5.92969V5.25C3.75098 2.90295 5.65302 1.00026 8 1ZM6.85547 12.75C7.04848 13.1912 7.48763 13.5 8 13.5C8.51237 13.5 8.95152 13.1912 9.14453 12.75H6.85547ZM8 2.5C6.48144 2.50026 5.25098 3.73138 5.25098 5.25V5.92969C5.25098 7.12346 4.81717 8.27722 4.03125 9.17578L3.63086 9.63281C3.54706 9.72861 3.50098 9.85222 3.50098 9.97949V11.25H12.501V9.97949C12.501 9.85232 12.4538 9.72856 12.3701 9.63281L11.9697 9.17578C11.1836 8.27722 10.75 7.12362 10.75 5.92969V5.25C10.75 3.73138 9.51855 2.50026 8 2.5Z"
          />
        </svg>
      )
    case "autoClosed":
      return (
        <svg {...filled14}>
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M7 14C10.866 14 14 10.866 14 7C14 3.13401 10.866 0 7 0C3.13401 0 0 3.13401 0 7C0 10.866 3.13401 14 7 14ZM5.03033 3.96967C4.73744 3.67678 4.26256 3.67678 3.96967 3.96967C3.67678 4.26256 3.67678 4.73744 3.96967 5.03033L5.93934 7L3.96967 8.96967C3.67678 9.26256 3.67678 9.73744 3.96967 10.0303C4.26256 10.3232 4.73744 10.3232 5.03033 10.0303L7 8.06066L8.96967 10.0303C9.26256 10.3232 9.73744 10.3232 10.0303 10.0303C10.3232 9.73744 10.3232 9.26256 10.0303 8.96967L8.06066 7L10.0303 5.03033C10.3232 4.73744 10.3232 4.26256 10.0303 3.96967C9.73744 3.67678 9.26256 3.67678 8.96967 3.96967L7 5.93934L5.03033 3.96967Z"
          />
        </svg>
      )
    case "content":
      return (
        <svg {...filled}>
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M2.10972 1.93688C2 2.20775 2 2.55017 2 3.235V12.765C2 13.4498 2 13.7922 2.10972 14.0631C2.26213 14.4394 2.56058 14.7379 2.93688 14.8903C3.20775 15 3.55017 15 4.235 15H11.765C12.4498 15 12.7922 15 13.0631 14.8903C13.4394 14.7379 13.7379 14.4394 13.8903 14.0631C14 13.7922 14 13.4498 14 12.765V7V3.235C14 2.55017 14 2.20775 13.8903 1.93688C13.7379 1.56058 13.4394 1.26213 13.0631 1.10972C12.7922 1 12.4498 1 11.765 1H9.63636H4.235C3.55017 1 3.20775 1 2.93688 1.10972C2.56058 1.26213 2.26213 1.56058 2.10972 1.93688ZM12 3.5C12 3.22386 11.7761 3 11.5 3H4.5C4.22386 3 4 3.22386 4 3.5V5.5C4 5.77614 4.22386 6 4.5 6H4.60961C4.83905 6 5.03904 5.84385 5.09468 5.62127L5.42425 4.30299C5.46877 4.12492 5.62876 4 5.81231 4H7.25V11C7.25 11.5523 6.80228 12 6.25 12H5.83333C5.3731 12 5 12.3731 5 12.8333C5 12.9254 5.07462 13 5.16667 13H10.8333C10.9254 13 11 12.9254 11 12.8333C11 12.3731 10.6269 12 10.1667 12H9.75C9.19772 12 8.75 11.5523 8.75 11V4H10.1877C10.3712 4 10.5312 4.12492 10.5757 4.30299L10.9053 5.62127C10.961 5.84385 11.161 6 11.3904 6H11.5C11.7761 6 12 5.77614 12 5.5V3.5Z"
          />
        </svg>
      )
    case "links":
      return (
        <svg {...filled}>
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M11.1111 1H4.88889L4.68235 1.0053C2.6306 1.11094 1 2.78131 1 4.8264V11.1736L1.00539 11.3768C1.11275 13.3956 2.8104 15 4.88889 15H5.14815L5.25369 14.993C5.63332 14.9423 5.92593 14.6222 5.92593 14.2347C5.92593 13.8121 5.5777 13.4694 5.14815 13.4694H4.88889L4.72913 13.4641C3.51498 13.3834 2.55556 12.3887 2.55556 11.1736V4.8264L2.56094 4.66922C2.64303 3.47457 3.65392 2.53056 4.88889 2.53056H11.1111L11.2709 2.53586C12.485 2.61663 13.4515 3.61128 13.4515 4.8264V4.93025C13.503 5.30378 13.8285 5.59168 14.2222 5.59168C14.6518 5.59168 15 5.24906 15 4.8264L14.9946 4.62319C14.8872 2.6044 13.1896 1 11.1111 1ZM9.14639 7.74497C8.93387 8.01972 8.95512 8.41357 9.21015 8.6645C9.48837 8.93825 9.93944 8.93825 10.2177 8.6645L10.6687 8.22067L10.769 8.12965C11.4221 7.58353 12.4029 7.61387 13.0196 8.22067C13.6688 8.85941 13.6688 9.89501 13.0196 10.5338L12.5685 10.9776L12.5048 11.0494C12.2922 11.3241 12.3135 11.718 12.5685 11.9689C12.8467 12.2426 13.2978 12.2426 13.576 11.9689L14.0271 11.5251L14.1475 11.3998C15.2313 10.2071 15.1911 8.37468 14.0271 7.22935C12.8215 6.04313 10.8668 6.04313 9.66123 7.22936L9.21015 7.67319L9.14639 7.74497ZM11.9698 13.5396C12.1823 13.2649 12.1611 12.871 11.9061 12.6201C11.6278 12.3463 11.1768 12.3463 10.8986 12.6201L10.4475 13.0639L10.3472 13.1549C9.69406 13.701 8.71333 13.6707 8.09662 13.0639C7.44745 12.4252 7.44745 11.3896 8.09662 10.7508L8.5477 10.307L8.61146 10.2352C8.82398 9.96045 8.80273 9.5666 8.5477 9.31567C8.26948 9.04193 7.81841 9.04193 7.54019 9.31567L7.08911 9.7595L6.9687 9.88478C5.88495 11.0775 5.92509 12.9099 7.08911 14.0552C8.29471 15.2414 10.2494 15.2414 11.455 14.0552L11.9061 13.6114L11.9698 13.5396ZM12.1972 9.1321C12.501 9.43096 12.501 9.9155 12.1972 10.2144L10.1232 12.2551C9.81942 12.554 9.32696 12.554 9.02322 12.2551C8.71947 11.9563 8.71947 11.4717 9.02322 11.1728L11.0973 9.1321C11.401 8.83323 11.8935 8.83323 12.1972 9.1321Z"
          />
        </svg>
      )
    case "template":
      return (
        <svg {...filled}>
          <path d="M11.25 1A3.75 3.75 0 0 1 15 4.75v4.855c0 .466-.186.912-.516 1.24l-3.665 3.646A1.75 1.75 0 0 1 9.585 15H4.75A3.75 3.75 0 0 1 1 11.25v-6.5A3.75 3.75 0 0 1 4.75 1h6.5Zm0 1.5h-6.5A2.25 2.25 0 0 0 2.5 4.75v6.5a2.25 2.25 0 0 0 2.25 2.25h3.249L8 10.75a2.75 2.75 0 0 1 2.582-2.745L10.75 8h2.75V4.75a2.25 2.25 0 0 0-2.25-2.25Zm2.25 7h-2.75c-.69 0-1.25.56-1.25 1.25l-.001 2.75h.086a.25.25 0 0 0 .176-.073l3.665-3.645a.25.25 0 0 0 .074-.177V9.5Z" />
        </svg>
      )
    default:
      return null
  }
}

function useOptions(
  kind: FilterKind,
  { members, labels, projects }: { members: Member[]; labels: Label[]; projects: Project[] }
): { value: string; label: string; icon?: React.ReactNode }[] {
  return React.useMemo(() => {
    if (kind === "status") {
      return STATUS_OPTIONS.map((o) => ({
        value: o.value,
        label: o.label,
        icon: <StatusIcon status={o.value} className="size-3.5 shrink-0" />,
      }))
    }
    if (kind === "priority") {
      return PRIORITY_OPTIONS.map((o) => ({
        value: o.value,
        label: o.label,
        icon: <PriorityIcon priority={o.value} className="size-3.5 shrink-0" />,
      }))
    }
    if (kind === "assignee") {
      const list = [
        { value: "unassigned", label: "No assignee", icon: undefined },
        ...members.map((m) => ({
          value: m.id,
          label: m.name,
          icon: (
            <Avatar className="size-4 shrink-0">
              <AvatarImage src={m.avatar} alt={m.name} />
              <AvatarFallback className="text-[8px]">
                {m.name.charAt(0)}
              </AvatarFallback>
            </Avatar>
          ),
        })),
      ]
      return list
    }
    if (kind === "creator") {
      return members.map((m) => ({
        value: m.id,
        label: m.name,
        icon: (
          <Avatar className="size-4 shrink-0">
            <AvatarImage src={m.avatar} alt={m.name} />
            <AvatarFallback className="text-[8px]">
              {m.name.charAt(0)}
            </AvatarFallback>
          </Avatar>
        ),
      }))
    }
    if (kind === "relations") {
      return RELATION_OPTIONS.map((o) => ({
        value: o.value,
        label: o.label,
        icon: <RelationIcon kind={o.value} />,
      }))
    }
    if (kind === "template") {
      return [
        {
          value: "none",
          label: "No template",
          icon: (
            <span className="text-muted-foreground inline-flex shrink-0">
              <FilterCategoryIcon kind="template" />
            </span>
          ),
        },
      ]
    }
    if (kind === "subscribers") {
      return members.map((m) => ({
        value: m.id,
        label: m.name,
        icon: (
          <Avatar className="size-4 shrink-0">
            <AvatarImage src={m.avatar} alt={m.name} />
            <AvatarFallback className="text-[8px]">
              {m.name.charAt(0)}
            </AvatarFallback>
          </Avatar>
        ),
      }))
    }
    if (kind === "projectProperties") {
      return [
        { value: "projectStatus", label: "Project status" },
        { value: "projectPriority", label: "Project priority" },
        { value: "projectLead", label: "Project lead" },
        { value: "projectMilestone", label: "Project milestone name" },
      ].map((o) => ({
        ...o,
        icon: <ProjectPropertyIcon kind={o.value} />,
      }))
    }
    if (kind === "suggestedLabel") {
      return [
        { value: "bug", label: "Bug", color: "#ef4444" },
        { value: "feature", label: "Feature", color: "#a78bfa" },
        { value: "improvement", label: "Improvement", color: "#3b82f6" },
      ].map((o) => ({
        value: o.value,
        label: o.label,
        icon: (
          <span
            aria-hidden="true"
            className="size-2.5 shrink-0 rounded-full"
            style={{ backgroundColor: o.color }}
          />
        ),
      }))
    }
    if (kind === "dates") {
      return [
        { value: "dueDate", label: "Due date" },
        { value: "createdDate", label: "Created date" },
        { value: "updatedDate", label: "Updated date" },
        { value: "startedDate", label: "Started date" },
        { value: "completedDate", label: "Completed date" },
        { value: "triagedDate", label: "Triaged date" },
        { value: "timeInStatus", label: "Time in current status" },
      ].map((o) => ({
        ...o,
        icon: <DateOptionIcon kind={o.value} />,
      }))
    }
    if (kind === "labels") {
      return labels.map((l) => ({
        value: l.id,
        label: l.name,
        icon: (
          <span
            aria-hidden="true"
            className="size-2.5 shrink-0 rounded-full"
            style={{ backgroundColor: l.color }}
          />
        ),
      }))
    }
    if (kind === "project") {
      return [
        { value: "none", label: "No project" },
        ...projects.map((p) => ({ value: p.id, label: p.name })),
      ]
    }
    return []
  }, [kind, members, labels, projects])
}
