  ...
  if (channelMatch) {
    ...
    if (channel) {
      return {
        ...
        pinned: channel.pinnedMessageIds.map((id) => store.getMessageById(id)),
      }
    }
  }
...
function getAvailableActions(): string[] {
  ...
  if (
    _rlState.currentPage === "/activity" ||
    _rlState.currentPage === "/mentions"
  ) {
  ...
  if (
    _rlState.currentPage === "/people" ||
    _rlState.currentPage.match(/^\/people\/.+/)
  ) {
    return [...base, "open_dm"]
  }
...
function executeAction(action: Record<string, unknown>): {
  ...
  switch (action.action) {
    ...
    case "open_dm": {
      const result = store.openDirectMessage(action.participantIds as string[])
      reward = result.success ? 0.5 : -0.5
      success = result.success
      break
    }
    ...
    case "mark_notification_read": {
      const result = store.markNotificationRead(action.notificationId as string)
      reward = result.success ? 0.2 : -0.5
      success = result.success
      break
    }
    ...
    case "set_preference": {
      const result = store.setPreference(action.key as string, action.value)
      reward = result.success ? 0.2 : -0.5
      success = result.success
      break
    }
