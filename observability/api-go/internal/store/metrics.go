...
func (s *Store) RecordMetricEvent(ctx context.Context, metricID, traceID, projectID string, passed *bool, score *float64, label *string) (*models.MetricEvent, error) {
	...
	_, err := s.Pool.Exec(ctx, `
		INSERT INTO metric_events (id, metric_id, trace_id, project_id, passed, score, label, evaluated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
	`, ev.ID, ev.MetricID, traceID, projectID, ev.Passed, ev.Score, ev.Label, ev.EvaluatedAt)
	if err != nil {
		return nil, err
	}
	return ev, nil
...
func (s *Store) ListMetricEvents(ctx context.Context, metricID string, limit int) ([]models.MetricEvent, error) {
	...
	rows, err := s.Pool.Query(ctx, `
		SELECT me.id, me.metric_id, m.name, me.trace_id, me.passed, me.score, me.label, me.evaluated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ev.ID, &ev.MetricID, &ev.MetricName, &ev.TraceID, &ev.Passed, &ev.Score, &ev.Label, &ev.EvaluatedAt); err != nil {
			return nil, err
		}
		out = append(out, ev)
	}
