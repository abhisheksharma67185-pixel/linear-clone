...
# GC37 — Delete opens a confirmation dialog with the correct title
...
# 3. Hover first row
# 4. Click Delete
# Plan assertion: Dialog title reads 'Delete goal permanently?' and a 'Delete permanently' button is visible
...
async def run_test():
    ...
    try:
        ...
        await expect(page.get_by_role("heading", name="Delete goal permanently?")).to_be_visible()
        await expect(page.get_by_role("button", name="Delete permanently")).to_be_visible()

        print("GC37: PASS")

    except Exception as e:
        print(f"GC37: FAIL — {type(e).__name__}: {e}")
        raise
    finally:
        if context:
            await context.close()
