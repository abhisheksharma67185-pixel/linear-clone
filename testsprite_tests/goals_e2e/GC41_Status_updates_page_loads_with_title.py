...
# GC41 — Status updates page loads with title
# Route: /goals/status-updates
#
# Plan steps:
# 1. Login
# 2. Navigate to /goals/status-updates
# Plan assertion: Heading reads 'Status updates'
...
async def run_test():
    ...
    try:
        ...
        await expect(page.get_by_role("heading", name="Status updates", level=1)).to_be_visible()

        print("GC41: PASS")

    except Exception as e:
        print(f"GC41: FAIL — {type(e).__name__}: {e}")
        raise
    finally:
        if context:
            await context.close()
