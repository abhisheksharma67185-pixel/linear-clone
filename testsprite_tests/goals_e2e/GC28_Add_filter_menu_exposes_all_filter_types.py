...
# GC28 — Add filter menu exposes all filter types
...
# 3. Open 'Add filter'
# Plan assertion: Menu lists Tag, Status, Owner, Team, Starred, Metric, Reporting line
...
async def run_test():
    ...
    try:
        ...
        await page.get_by_role("button", name="Add filter", exact=True).click()
        await asyncio.sleep(1)
        for label in ("Tag", "Status", "Owner", "Team", "Starred", "Metric", "Reporting line"):
            await expect(page.get_by_text(label, exact=True).first).to_be_visible()

        print("GC28: PASS")

    except Exception as e:
        print(f"GC28: FAIL — {type(e).__name__}: {e}")
        raise
    finally:
        if context:
            await context.close()
