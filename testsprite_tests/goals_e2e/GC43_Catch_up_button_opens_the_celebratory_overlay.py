...
# GC43 — 'Catch up' button opens the celebratory overlay
...
# 3. Click 'Catch up'
# Plan assertion: Catch-up overlay becomes visible (full-screen modal / checkmark animation)
...
async def run_test():
    ...
    try:
        ...
        await page.get_by_role("button", name="Catch up", exact=True).click()
        await expect(page.get_by_role("heading", name="You're all caught up")).to_be_visible()

        print("GC43: PASS")

    except Exception as e:
        print(f"GC43: FAIL — {type(e).__name__}: {e}")
        raise
    finally:
        if context:
            await context.close()
