...
# GC47 — Status summary grid shows all six categories
# Route: /goals/status-updates
#
# Plan steps:
# 1. Login
# 2. Navigate to /goals/status-updates
# Plan assertion: Six labels visible: 'On track', 'At risk', 'Off track', 'No update', 'Cancelled', 'Completed'
...
async def run_test():
    ...
    try:
        ...
        for label in ("On track", "At risk", "Off track", "No update", "Cancelled", "Completed"):
            await expect(page.get_by_text(label, exact=True).first).to_be_visible()

        print("GC47: PASS")

    except Exception as e:
        print(f"GC47: FAIL — {type(e).__name__}: {e}")
        raise
    finally:
        if context:
            await context.close()
