...
# GC38 — Cancel closes the delete dialog without removing the row
...
# 4. Click Cancel
# Plan assertion: Dialog closes and the row is still present in the list
...
async def run_test():
    ...
    try:
        ...
        # Capture first row's name before opening dialog
        name_before = (await page.locator('div:has(> button:has-text("Restore"))').first.inner_text()).splitlines()[0]
        await page.get_by_role("button", name="Delete").first.click()
        await page.get_by_role("button", name="Cancel", exact=True).click()
        await asyncio.sleep(1)
        # Row should still be present
        await expect(page.get_by_text(name_before, exact=False).first).to_be_visible()

        print("GC38: PASS")

    except Exception as e:
        print(f"GC38: FAIL — {type(e).__name__}: {e}")
        raise
    finally:
        if context:
            await context.close()
