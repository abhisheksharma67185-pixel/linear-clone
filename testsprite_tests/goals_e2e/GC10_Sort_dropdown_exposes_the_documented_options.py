...
# GC10 — Sort dropdown exposes the documented options
# Route: /goals
#
# Plan steps:
# 1. Login
# 2. Navigate to /goals
# 3. Open 'Sort by' dropdown
# Plan assertion: Menu shows Name, Status, Target date, Following, Last updated, Follower count
...
async def run_test():
    ...
    try:
        ...
        # Sort trigger reads the current sort field; default is "Sort by name"
        await page.get_by_role("button", name=re.compile(r"^Sort by ")).click()
        for label in ("Name", "Status", "Target date", "Following", "Last updated", "Follower count"):
            await expect(page.get_by_role("menuitem", name=label, exact=True)).to_be_visible()

        print("GC10: PASS")

    except Exception as e:
        print(f"GC10: FAIL — {type(e).__name__}: {e}")
        raise
    finally:
        if context:
            await context.close()
