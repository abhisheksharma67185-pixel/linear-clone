...
# GC33 — Archived page loads with title 'Archived'
# Route: /goals/archived
#
# Plan steps:
# 1. Login
# 2. Navigate to /goals/archived
# Plan assertion: Page heading reads 'Archived'
...
async def run_test():
    ...
    try:
        ...
        await expect(page.get_by_role("heading", name="Archived", level=1)).to_be_visible()

        print("GC33: PASS")

    except Exception as e:
        print(f"GC33: FAIL — {type(e).__name__}: {e}")
        raise
    finally:
        if context:
            await context.close()
