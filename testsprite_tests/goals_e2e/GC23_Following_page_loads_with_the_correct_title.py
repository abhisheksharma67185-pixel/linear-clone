...
# GC23 — Following page loads with the correct title
# Route: /goals/following
#
# Plan steps:
# 1. Login
# 2. Navigate to /goals/following
# Plan assertion: Page heading reads 'Following'
...
async def run_test():
    ...
    try:
        ...
        await expect(page.get_by_role("heading", name="Following", level=1)).to_be_visible()

        print("GC23: PASS")

    except Exception as e:
        print(f"GC23: FAIL — {type(e).__name__}: {e}")
        raise
    finally:
        if context:
            await context.close()
