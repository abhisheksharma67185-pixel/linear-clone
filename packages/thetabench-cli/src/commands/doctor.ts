...
import { command, header, printJson, resolveBaseUrl, truncate } from "../util.js";
...
const PROBES: Probe[] = [
  ...
  { name: "sim/snapshot", method: "GET", path: "api/sim/snapshot", allow4xx: true },
...
async function probe(baseUrl: string, p: Probe): Promise<ProbeResult> {
  ...
  try {
    ...
    return { name: p.name, ok: true, status: 200, latencyMs: Date.now() - t0, message: "" };
  } catch (e) {
  ...
  program
    ...
    .action(
      command(async (opts: Record<string, string | boolean>) => {
        ...
        const summary = passed === total ? ok(`All ${total} probes passed`) : errColor(`${passed}/${total} probes passed`);
