  ...
  program
    ...
    .action(
      command(async (opts: Record<string, string>) => {
        ...
        let actionSpace: Awaited<ReturnType<typeof fetchActionSpace>> | null = null;
        try {
        ...
        while (!done && steps < ep.task.max_steps) {
          ...
          const available = (obs.observation.availableActions as string[] | undefined) ?? [];
          p.note(
            available.length ? available.join(", ") : muted("(no available actions)"),
            "Available actions",
          );
          ...
          try {
            ...
            const rewStr = res.reward >= 0 ? ok(res.reward.toFixed(4)) : warn(res.reward.toFixed(4));
            stepSpin.stop(
        ...
        if (task.type === "retrieval" || task.type === "action_retrieval" || task.domain === "impossible") {
          const r = await p.text({
            message: "Agent response (required for retrieval/impossible tasks)",
            placeholder:
              task.domain === "impossible" ? "This task is not possible because…" : "e.g. $29.99",
          });
          if (p.isCancel(r)) {
        ...
        try {
          ...
          finishSpin.stop(`Done. status=${res.status}  score=${res.score.toFixed(3)}`);
          ...
          p.note(
            `score: ${res.score.toFixed(3)}   reward: ${res.total_reward.toFixed(3)}\n` +
              ...
              (passed !== undefined && total !== undefined ? `checks: ${passed}/${total}\n` : "") +
              (res.judge_result
