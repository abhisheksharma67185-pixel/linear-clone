...
function pickAction(
  ...
  if (availableActions.includes("navigate")) {
    ...
    const candidates = ["/board", "/backlog", "/projects", "/orders", "/products", "/customers", "/"];
...
async function runOne(
  ...
  try {
    ...
    const response =
      task.type === "retrieval" || task.type === "action_retrieval" || task.domain === "impossible"
    ...
    return {
      ...
      passed: false,
      walltimeSec: (Date.now() - t0) / 1000,
      error: e instanceof HttpError ? `${e.status}: ${e.message}` : e instanceof Error ? e.message : String(e),
    };
  }
}

export function registerBaselineCommand(program: Command): void {
  const baseline = program.command("baseline").description("Run a built-in rule-based agent");

  baseline
    .command("run")
    .description("Run the baseline agent against a set of tasks and save a results JSON")
    .requiredOption("--url <site-url>", "Site base URL")
    .option("--tasks <ids>", "Comma-separated task IDs, or 'all' to iterate everything", "all")
    .option("--output <file.json>", "Where to write the result JSON", "baseline-results.json")
    .option("--max-steps <n>", "Per-task step budget before forcing finish", "25")
    .option("--limit <n>", "Run at most N tasks (safety cap when --tasks=all)", "10")
    .addHelpText(
      "after",
    ...
    .action(
      command(async (opts: Record<string, string>) => {
        ...
        loader.succeed(`Loaded ${all.tasks.length} task definitions from ${baseUrl}`);

        let selected: TaskListItem[];
        if (opts.tasks === "all" || !opts.tasks) {
          selected = all.tasks.slice(0, limit);
        } else {
          const ids = new Set(opts.tasks.split(",").map((s) => s.trim()).filter(Boolean));
          selected = all.tasks.filter((t) => ids.has(t.id));
          const missing = [...ids].filter((id) => !selected.some((t) => t.id === id));
          if (missing.length > 0) {
            process.stderr.write(
              errColor(`warning:`) + ` unknown task IDs: ${missing.join(", ")}\n`,
            );
          }
        }

        if (selected.length === 0) {
          throw new Error("No tasks to run (check --tasks / --limit and that the site has tasks).");
        }
        ...
        for (let i = 0; i < selected.length; i++) {
          ...
          spinner = ora({ text: `[${i + 1}/${selected.length}] ${task.id} — ${task.domain}/${task.difficulty}` }).start();
          const result = await runOne(baseUrl, task, maxSteps);
          if (result.error) {
            spinner.fail(`${task.id} — ${result.error}`);
          } else {
            const mark = result.passed ? ok("passed") : muted(`score=${result.score.toFixed(2)}`);
            spinner.succeed(`${task.id} — ${mark}  steps=${result.steps}  reward=${result.totalReward.toFixed(2)}`);
          }
          results.push(result);
        }
        ...
        process.stdout.write(
          renderTable(rows, {
            head: ["", "Task", "Domain", "Type", "Steps", "Score", "Reward", "Status"],
        ...
        // Write JSON
        const outPath = pathResolve(process.cwd(), opts.output ?? "baseline-results.json");
        writeFileSync(
          outPath,
          JSON.stringify(
            {
              ...
              avgReward:
                results.length ? results.reduce((s, r) => s + r.totalReward, 0) / results.length : 0,
              results,
            },
            null,
            2,
