...
export function registerEpisodeCommand(program: Command): void {
  const episode = program
    .command("episode")
    .description("Control a running episode")
    ...
    .action(
      command(
        async (taskId: string, opts: Record<string, string | boolean>) => {
          ...
          const res = await startEpisodeRemote(baseUrl, {
            task_id: taskId,
            seed,
          })
      ...
      command(async (opts: Record<string, string | boolean>) => {
        ...
        process.stdout.write(
          renderKeyValue([
            ...
            [
              "availableActions",
              Array.isArray(obs.availableActions)
                ? obs.availableActions.join(", ")
                : null,
            ],
  ...
  episode
    ...
    .description(
      'Execute one action; <action-json> is e.g. \'{"action":"navigate","target":"/board"}\''
    )
    .option("--url <site-url>", "Site base URL")
    .option("--json", "Output JSON instead of formatted text")
    .addHelpText(
    ...
    .action(
      command(
        async (actionJson: string, opts: Record<string, string | boolean>) => {
          const baseUrl = resolveBaseUrl(opts.url as string | undefined)
          ...
          if (
            ...
            throw new Error(
              '<action-json> must be a JSON object, e.g. \'{"action":"navigate","target":"/board"}\''
            )
          }
          ...
          process.stdout.write(
            renderKeyValue([
              ...
              [
                "currentPage",
                (res.observation.currentPage as string | undefined) ?? null,
              ],
              [
                "lastAction",
                (res.info?.lastAction as string | undefined) ?? null,
              ],
  ...
  episode
    ...
    .option(
      "--response <text>",
      "Agent response (required for retrieval tasks)"
    )
    .option("--url <site-url>", "Site base URL")
    .option("--json", "Output JSON instead of formatted text")
    .addHelpText(
    ...
    .action(
      command(async (opts: Record<string, string | boolean>) => {
        const baseUrl = resolveBaseUrl(opts.url as string | undefined)
        const res = await finishEpisodeRemote(
          baseUrl,
          opts.response as string | undefined
        )
        ...
        process.stdout.write(
          renderKeyValue([
            ...
            [
              "judge",
              judge
                ? `${judge.passed ? ok("PASS") : warn("FAIL")} (${judge.matchType})`
                : null,
            ],
          ])
        )
        ...
        if (res.eval?.checks?.length) {
          ...
          for (const check of res.eval.checks) {
            ...
            const weight =
              typeof check.weight === "number" ? ` (w=${check.weight})` : ""
            process.stdout.write(
              `    ${marker} ${check.message}${muted(weight)}\n`
            )
          }
          process.stdout.write("\n")
        }
