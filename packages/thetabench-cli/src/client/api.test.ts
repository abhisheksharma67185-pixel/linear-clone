...
describe("typed API wrappers — URL/method/body construction", () => {
  ...
  it("fetchTasks → GET /api/sim/tasks with no query when filters are empty", async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ total: 0, filtered: 0, tasks: [] }));
  ...
  it("startEpisodeRemote → POST /api/sim/config with the payload", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        episode_id: "ep1",
        task: { id: "t1", title: "x", goal: "y", type: "action", difficulty: "easy", max_steps: 10, curriculum_stage: 1 },
        status: "running",
        initial_snapshot: {},
      }),
    );
  ...
  it("finishEpisodeRemote omits agent_response when not provided (sends '{}')", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        episode_id: "e", task_id: "t", status: "complete", steps: 1, score: 0, total_reward: 0, wall_time_seconds: 0,
      }),
    );
  ...
  it("finishEpisodeRemote forwards agent_response when provided", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        episode_id: "e", task_id: "t", status: "complete", steps: 1, score: 1, total_reward: 0, wall_time_seconds: 0,
      }),
    );
  ...
  it("evaluateEpisodeRemote → POST /api/sim/evaluate with empty body", async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ passed: 1, total: 1, checks: [] }));
  ...
  it("fetchObservation → GET /api/rl", async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ observation: { currentPage: "/" }, info: {} }));
  ...
  it("stepEnvironment → POST /api/rl with the action object", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({ observation: {}, reward: 0.5, done: false, truncated: false }),
    );
    await stepEnvironment(BASE, { action: "navigate", target: "/board" });
    const init = fetchMock.mock.calls[0]![1] as RequestInit;
    expect(fetchMock.mock.calls[0]![0]).toBe(`${BASE}/api/rl`);
    expect(init.method).toBe("POST");
    expect(JSON.parse(init.body as string)).toEqual({ action: "navigate", target: "/board" });
  });

  it("resetEnvironment → POST /api/rl/reset with empty body", async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse({ message: "ok", observation: {} }));
