...
export const fetchHealth = (baseUrl: string) =>
  getJson<HealthResponse>(baseUrl, "api/health")
...
export const submitLeaderboard = (
  baseUrl: string,
  entry: Partial<LeaderboardEntry>
) =>
  postJson<{ id: string; rank: number; message: string }>(
    baseUrl,
    "api/sim/leaderboard",
    entry
  )
...
export const resetEnvironment = (baseUrl: string) =>
  postJson<{ message: string; observation: unknown }>(
    baseUrl,
    "api/rl/reset",
    {}
  )
